...
<ol>
<li>We wanted to have a source code management system so that we could contribute and update patches to [[ImageJ]].

Originally, this was CVS, now it is [[Git]], and in the meantime [[Wayne Rasband]], the author of ImageJ, also adopted a routine involving Git. So basically, this goal was reached.</li>
...
The real first script is a notifier which scrapes the [http://imagej.nih.gov/ij/notes.html News section of the ImageJ 1.x site] every hour or so and sends me an email when it changed. Whenever I get such a mail I run three scripts in succession, which are all stored in the 'tools' branch of ImageJA.git:
...
These scripts have gone through a long history ([[User:Schindelin|I]] started the stuff in October 2005, when I tried to get ahold of as many as possible previous source packages and committed them as history of ImageJ to document as much as possible how things developed).
...
<ol>
<li>Changes that we provided for traditional reasons but that will be obsolete with IJ2, such as the "convert to 8-bit" in the {{bc|File|Import|Image Sequence...}} support code or the "fuzzy matching" in the Command Finder.

Roughly half of these changes I will just forget, while the other half of changes will persist in the IJ1-based Fiji through [[Javassist]].
</li>
<li>Changes that actually should live as add-ons to ImageJ, such as the {{bc|Plugins>Install Plugin...}} plugin. Of course, having it as a Fiji plugin will move its position in the menu, but that does not matter that much.</li>
...
In general, this should clean up a lot of unnecessary modifications in IJ1 and let us (read: [[User:Schindelin|me]]) concentrate much more on IJ2.
