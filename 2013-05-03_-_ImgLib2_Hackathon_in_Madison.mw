__NOTOC__
From Saturday, April 27, 2013 through Friday, May 3, 2013, LOCI in Madison hosted Tobias Pietzsch and Stephan Preibisch, two [[ImgLib2]] main developers, for a hackathon. Here is a brief summary of overall accomplishments and topics of discussion:

* Initial version of SCIFIOCellImg
* Much discussion and ideas and proofs of concept for better OPS
* Start on [https://github.com/imglib/imglib/commit/e4e26aa05e0f42ff1a90e8c6d67235431471de44 better imglib2 type hierarchies for metadata], particularly units
* Decided on histogram unification and [https://github.com/imglib/imglib2/commit/79bbc2008eeec6f221c16b6f84782daca2b3f496 merged to master]
* Deprecated [[ImgLib1]] more fully, purging it from the [https://github.com/imglib/imglib2 imglib repository]
** See the [http://thread.gmane.org/gmane.comp.java.imagej.devel/1477 deprecation announcement thread] for details
...
* Together with Johannes we removed imglib1 from the imglib git repository, it now lives in [https://github.com/fiji/legacy-imglib1 legacy-imglib1]. It could not go into SPIM_Registration directly due to circular dependencies with the 3d-viewer.
...
* We have been working on a new Type hierarchy that reflects the opposed read and write capabilities for integer/floating point based Types. The repository [https://github.com/StephanPreibisch/funwithtypes lives on GitHub]. Until now it does not work though as it seems very hard to express it using Java generics. I hope that is not true ...
...
Work was done on a branch called <code>img-metadata</code>, culminating in [https://github.com/imglib/imglib/commit/d3f8e640fec2b91bba761930b46d13170486b9f5 d3f8e64]. The branch then sat for several months before being [https://github.com/imglib/imglib/commit/e4e26aa05e0f42ff1a90e8c6d67235431471de44 merged to master] on August 12, 2013.
...
* We had a lot of discussions revolving about Ops and Expressions and performance. My current thinking is that we need to separate Ops from Expressions: to make developing, understanding and maintaining Ops as easy as possible, we have to separate the concerns better.<br/>And it is simply easier for everybody if the Ops do one thing, and one thing only, but that one well: evaluate an operation, working on Type variables. Expressions, however, need to know about temporary variables, input and output variables and about being clever not to copy data around too much. But that's the Expressions' job, not the Ops'.
...
The new roi discussion led to an attempt to see if the new interface design had any gotcha limitations. To that end I spent much of my time exercising the new interface by implementing it for some of the most general PointSet implementations. The test interface was laid out [https://github.com/imglib/imglib/blob/be367d9aad54448efa7e2955b791ac5026e86c34/ops/src/main/java/net/imglib2/ops/sandbox/NewPointSet.java here as NewPointSet].. I have now created four implementations of this interface that can be found [https://github.com/imglib/imglib/tree/be367d9aad/ops/src/main/java/net/imglib2/ops/sandbox here]. So far I have not run into any show stopper gotchas. The new implementations are sometimes slower than the old implementations but that may be addressable with additional attention.

I also presented work I had done on a set of histogram classes in Imglib2 that could be used to unify the various histogram implementations currently found there. Suggestions were fielded, some changes were made, and the code has been [https://github.com/imglib/imglib/commit/79bbc2008eeec6f221c16b6f84782daca2b3f496 merged to master].

In regards to data type hierarchy experiments I tried to determine the ramifications of supporting BigInteger and BigDecimal data types in Imglib2 for ImageJ2. As part of this work I mocked up a version of the Haskell numeric type hierarchy so that it could inform any future development of data types. It can be found on the types-experiment branch [https://github.com/imglib/imglib/blob/faaeaae7f5fe9c66520ebdba6f50efd9cb6d18f9/ops/src/main/java/net/imglib2/ops/sandbox/types/Types.java here] and [https://github.com/imglib/imglib/blob/faaeaae7f5fe9c66520ebdba6f50efd9cb6d18f9/ops/src/main/java/net/imglib2/ops/sandbox/types/Types2.java here].
...
* SCIFIO
** Converted [https://github.com/hinerm/bioformats/blob/scifio-cell-image/components/scifio-devel/src/ome/scifio/io/img/ImgSaver.java ImgSaver] and [https://github.com/hinerm/bioformats/blob/scifio-cell-image/components/scifio-devel/src/ome/scifio/io/img/ImgOpener.java ImgOpener] to use the SCIFIO API.
** Extended the imglib2 cell components with [https://github.com/hinerm/bioformats/tree/scifio-cell-image/components/scifio-devel/src/ome/scifio/io/img/cell SCIFIO cell classes]. Currently, these components create 2D cells using the optimal tile height/width, as determined by the current SCIFIO reader. The cells are cached per-dataset using soft references to determine when cells are disposed. 
* ImageJ2
** Updated [https://github.com/imagej/imagej/blob/scifio-cells/core/io/src/main/java/imagej/io/DefaultIOService.java DefaultIOService] and [https://github.com/imagej/imagej/blob/scifio-cells/plugins/commands/src/main/java/imagej/core/commands/io/SaveAsImage.java SaveImg] to use the updated SCIFIO ImgOpener and ImgSaver. This also means that IJ2 can read/write all the formats currently supported in SCIFIO.
...
See the [https://imagej.net/pipermail/imagej-devel/2013-May/001488.html thread on imagej-devel].
