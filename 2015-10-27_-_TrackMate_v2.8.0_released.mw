...
This version ships mainly small utilities, improvement and bugfixes. But more importantly, I would like to highlight several contributed [[How_to_write_your_own_edge_feature_analyzer_algorithm_for_TrackMate|TrackMate modules]] since the last release. 
...
The source code is found on [https://github.com/chicoronny/RonnyTrackMate his github page].



=== Thorsten Wagner Find maxima detector. ===

Thorsten Wagner is working on a detector that emulates the Find maxima tool of ImageJ. The module is not totally ready, but you can find a working fork of TrackMate [https://github.com/thorstenwagner/TrackMate here] that ships the functionality.



=== Benoit Lombardo analyzers. ===

Benoit Lombardo wrote a spot analyzer that compute mean intensities in all the channels of an image, when you have a multi-channel image. You can find it on the [https://github.com/tinevez/TrackMate-extras TrackMate extras] page.
...
{{#widget:YouTube|id=IZenKcv6R4Y|width=640}}


=== The Block LoG detector. ===

TrackMate initial design always favored speed over memory consumption. This can be a problem for applications with large images, particularly for the detection of spots. For instance, when the LoG detector processes a single frame, it first copies it to a <code>float</code> image, then takes its complex FFT. This has a major impact when working with large images and using multithreading to process several frames at once.
...
[[File:TrackMate BlockLogDetector.png|600px]]
...
[[File:TrackSchemeHonorsRadiusRatio.png|600px]]
...
Now, the quality value returned by the LoG detector is such
that:
...
retrieve the same quality value value than before scaling.  However, I (JYT)
could not derive the exact formula if the image is scaled differently across X,
Y and Z.
...
This bug is now fixed in <code>imglib2-algorithm-0.3.3</code>. The bias is now gone, and you should see a marked improvement on the localization accuracy. 

This bug was discovered by both Leanna Owen and Jan Eglinger, and was discussed in part [http://forum.imagej.net/t/accuracy-of-subpixel-localization-in-log-detector/ here].
...
{{Person|JeanYvesTinevez}}
[[User:CCCT|CCCT]] ([[User talk:CCCT|talk]]) 10:53, 27 October 2015 (CDT)
