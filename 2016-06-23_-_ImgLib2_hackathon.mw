...
We invented <code>MetaViews</code>, a class analogous to {{Javadoc | project=ImgLib2 | package=net/imglib2/view | class=Views}} but for metadata elements. These elements are then aggregated into a <code>MetaSpace</code>, which provides mutators and accessors for working with the space. For example, you might attach a physical calibration to a spatial axis, or a custom variable-width timestamp to each point along the time axis.

On top of this, we are actively developing a (tentatively named) <code>RichImage</code> structure, consisting of image pixels (e.g., a <code>RandomAccessible</code>) together with a <code>MetaSpace</code> which describes it. The [[SCIFIO]] library will be updated to produce a <code>Dataset</code>, a collection of <code>RichImage</code> elements, which are populated with elements directly from the source data format.

=== Image enrichers ===
For the various use cases above (e.g., registration of tiles in a larger space), we will have a pipeline which calls <code>ImageInspector</code> plugins that examine the <code> RichImages</code> and decide whether they can be improved with available <code>ImageEnricher</code> plugins. (Again: these are working names only.) For instance, the <code>Dataset</code> image collection might consist of a bag of 2D tiles with associated affine transforms; a <code>MosaicEnricher</code> could then apply those transforms virtually to offer the larger 2D mosaic image as a <code>RandomAccessible</code> which returns lists of tile values at each sample. A <code>FusionEnricher</code> could operate on any <code>RandomAccessible</code> which returns a list, blending the values of that list using a given <code>Converter</code>. (In practice, the enrichers will very likely be more general than these examples, but hopefully they serve here to illustrate some of the possibilities.)
