...
The [https://fiji.sc/Jython_Scripting#Using_a_jython_script_as_a_plugin Jython plugin installation rules] apply. You must [https://dl.dropboxusercontent.com/u/5200940/3view_import_list.zip download] and copy 3view_import_list.py and MakeImportList4TrakEM2.py together in the same directory. The third file, TrakEM2_import_list.py is optional (see point 3 below). If you want to call the script from your own code, only the second file is necessary. As a suggestion only, you can follow these steps:

# Copy 3view_import_list.py and MakeImportList4TrakEM2.py into Fiji directory \Fiji.app\plugins\Scripts\Plugins\Utilities.
# To make the plugin appear in the menu, you have the option of selecting  {{bc | Plugins | Scripting | Refresh Jython Scripts}}, {{bc | Help | Refresh Menus}} or restarting Fiji. Then the menu entry ‘3View import list’ will appear under {{bc | Plugins | Utilities}} (the first Utilities folder...)
...
# If you run the slightly modified version, ‘TrakEM2 import list’, you can choose another file format other than .dm3, choose another folder pattern (these are [[wikipedia:Regular expression|Regular Expression]] patterns) or a different file name for the final text file. However, the image files still need to have metadata for stage coordinates and pixel dimension (size), in the same units, for the automatic calculations to work. Currently I don’t have any ‘non-automatic’ version that will take all required information for the user.<br />Note 1: The numbering in the files must have 4 digits. I ''may'' fix this in the future.<br />Note 2: The file format input is case sensitive: '.tif' is not the same as '.TIF'
...
*[https://fiji.sc/Jython_Scripting This other page] prepared by Albert Cardona
