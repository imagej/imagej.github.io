{{DevelopMenu | tutorials}}{{TOC}}This tutorial is designed to guide developers through the options, processes and motivations for adding Ops outside the core {{GitHub|org=imagej|repo=imagej-ops|label=imagej-ops}} project. Because this tutorial caters to external developers, in this tutorial we show how a [[BAR]] function could be converted to an Op.

= Make your first Op =

At the most fundamental level, an Op is a SciJava [https://github.com/scijava/scijava-common/blob/scijava-common-2.47.0/src/main/java/org/scijava/plugin/Plugin.java Plugin] encapsulating a piece of functionality, which can be discovered and used by a central [https://github.com/imagej/imagej-ops/blob/imagej-ops-0.18.0/src/main/java/net/imagej/ops/OpService.java OpService].
...
[https://github.com/tferr/Scripts/blob/master/BAR/src/main/java/bar/DefaultGCD.java On GitHub]

== Use your Op ==

With these two components, you can start using your Op - for example, in the [[Scripting|script editor]]:
...
Calling our Ops by name is not type-safe, and importing each interface is tedious. If you are going to provide a collection of Ops, a useful way to package them is within a custom [http://javadoc.scijava.org/ImageJ/net/imagej/ops/Namespace.html Namespace].
...
[https://github.com/tferr/Scripts/blob/master/BAR/src/main/java/bar/BAR.java On GitHub]
...
SciJava [https://github.com/scijava/scijava-common/blob/scijava-common-2.47.0/src/main/java/org/scijava/service/Service.java Services] are a general workhorse in a given SciJava context. There is a single instance of each Service created per context, so they are a common container for static utility style methods.
...
[https://github.com/tferr/Scripts/blob/master/BAR/src/main/java/bar/BARService.java On GitHub]
...
The ImageJ [[Scripting|script editor]] automatically collects scripts located in <code>src/main/resources/script_templates</code>. For example, if we create a file:
...
[https://github.com/tferr/Scripts/blob/master/BAR/src/main/resources/script_templates/BAR/GCD.py On GitHub]
...
However, all of this is done in the core {{GitHub|org=imagej|repo=imagej-ops|label=imagej-ops}} project and could be deduced by careful study. If you do take the time to independently attempt any of these topics, consider documenting your experience and sharing it as a tutorial on this wiki.
