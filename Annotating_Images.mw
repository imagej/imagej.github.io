...
# Open the image with the original calibration
# Run the "Copy Pixel Size" plugin. 
# In the “''from''" drop-down box select the original image; in the “''to''" box, select the processed image. Click OK.
# In the new dialog, enter a value by which the receiving image has been scaled.
...
# Using the line selection tool, draw a line along the length of the feature or scale bar. Do not choose "''Edit/Draw''" after adding the line.
# Run the menu command “''Analyze/Set scale…”''.
# Enter the dimensions of the object/scale bar in the “''known distance''” box and set the units in the "''Unit of length''" box.
# Do not check ''Global'' unless you want all your images to have this calibration. Click OK.
...
Adding text to single frame images should be done in CorelDRAW or a similar application so the text does not become part of the image and can therefore be edited at any time. Adding text to an image/movie is irreversible so it should not be done on raw data. Images can be copied to and pasted from the system clipboard between programs using the System Clipboard plugin (Plugin documentation found [https://imagej.net/plugins/clipboard.html here])or by using the ''"Copy to System"'' and ''"Paste"'' options in the ''Edit'' menu. In the case that an image is not pasting accurately, use "''Edit/Paste Special''" in the receiving application, then select "''Device independent bitmap''".
...
Adding text to a stack is a slightly different process. You can draw a text box by following the same steps, but on a stack, keyboard shortcut {{key|Ctrl}}+{{key|B}} will overlay the text on the current slice, while shortcut {{key|Ctrl}}+{{key|D}} will draw the text on every slice of the stack.
...
[[File:time_stamper.png|166x237px]]
...
Also, bear in mind that the interval doesn't necessarily need to be "time". The Time Stamper can also be used to add "z-axis depths" to z-stacks and "angle of rotation" to rotating volume movies (° = {{key|Alt}}+{{key|2}}-{{key|4}}-{{key|8}}; µ = {{key|Alt}}+{{key|2}}-{{key|3}}-{{key|0}}).
...
The [[Series Labeler]] plugin, allows quite flexible text labeling for movies and z stacks, with preview functionality, so you can see what you are going to get. 
