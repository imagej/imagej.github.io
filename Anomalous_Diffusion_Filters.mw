...
| source                 = {{GitHub | org = CSIM-Toolkits | repo = ImageJ/tree/master/plugins/AnomalousDiffusionFilters | label = GitHub CSIM-ImageJ AnomalousDiffusionFilters repository}}
| released               = February 03<sup>rd</sup>, 2015
| latest version         = February 03<sup>rd</sup>, 2015
| status                 = experimental, active
| category               = [[:Category:Plugins|Plugins]], [[:Category:Filtering|Filtering]]
}}
== Anomalous Diffusion Filters ==

[[Image:Filters-examples.png|thumb|500px|Filtering examples with the T1w MRI stack available in Fiji examples images.]]

A PlugInFilter for the two different methods for image filtering: Anisotropic Anomalous Diffusion and Isotropic Anomalous Diffusion. Both methods description can be found in the Physics in Medicine and Biology article [http://dx.doi.org/10.1088/0031-9155/60/6/2355 weblink] and have a discrete solution of generalized diffusion heat equation (also know as a porous media equation). 
...
In the case of isotropic filtering, the diffusion intensity is set as a fixed parameter for all filtering process, given by the Generalized diffusion coefficient value (<math>D_q</math>). This approach is similar with the [[wikipedia:Gaussian blur|Gaussian blur]] and have the physical process, where all the image space have the same diffusion intensity applied. A further idea, the anisotropic filtering set a local diffusion intensity based on the neighborhood characteristic. The local gradient magnitude is the general parameter to set what will be the filter behavior in that specific region of the image, given by its pixel neighbors. This kind of solution for the local diffusion intensity (gradient magnitude) is similar with the Perona and Malik anisotropic filter and for this case the Condutance parameters plays the edge detection role. See the [[wikipedia:Anisotropic diffusion|Classic Anisotropic Filter]] to get more information about the condutance parameter.

== Indicated Usage ==

Some studies were already made with different MRI imaging techniques and the both isotropic and anisotropic filters shown a good performance with some specific imaging modalities. Diffusion tensor imaging (DTI) and structural MRI images such as T1 and T2 weighted images show a better filtering efficiency with the AAD filter. For the IAD method, we indicate the use of higher SNR images such as T1 weighted images, where you do not need to decrease so much noise presented in the original image. A short analysis with functional MRI and Diffusion Weighted Imaging with the IAD methods was made and publish in international conferences ([http://www.ncbi.nlm.nih.gov/pubmed/25570699 fMRI], [http://www.ncbi.nlm.nih.gov/pubmed/24110614 DWI]) and should be appropriate use this kind of filtering method to enhance image quality in these cases.
