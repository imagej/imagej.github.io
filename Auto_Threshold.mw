{{ComponentStats:sc.fiji:Auto_Threshold}}This plugin binarises 8 and 16-bit images using various '''global''' (histogram-derived) thresholding methods. The segmented phase is always shown as white (255).

For '''local''' thresholding rather than global, see  the [[Auto Local Threshold]] plugin.

== Installation ==
'''ImageJ''': requires v1.42m or newer. Download [http://maven.imagej.net/service/local/artifact/maven/redirect?r=releases&g=sc.fiji&a=Auto_Threshold&v=RELEASE&e=jar Auto_Threshold-X.Y.Z.jar] and copy it into the ImageJ/plugins folder and either restart ImageJ or run the '{{bc | Help | Update Menus}}' command. After this a new command should appear in '{{bc | Image | Adjust | Auto Threshold}}'.
...
'''1.''' This  plugin is accessed through the {{bc | Image | Auto Threshold}} menu entry, however the thresholding methods were also partially implemented in ImageJ's thresholder applet accessible through the {{bc | Image | Adjust | Threshold...}} menu entry.
While the Auto Threshold plugin can use or ignore the extremes of the image histogram (Ignore black, Ignore white) the applet cannot: the 'default' method ignores the histogram extremes but the others methods do not. This means that applying the two commands to the same image can produce apparently different results. In essence, the Auto Threshold plugin, with the correct settings, can reproduce the results of the applet, but not the way round.

'''2.''' From version 1.12 the plugin supports thresholding of 16-bit images. Since the Auto Threshold plugin processes the full greyscale space, it can be slow when dealing with 16-bit images. 
Note that the ImageJ thresholder applet also processes 16-bit images, but in reality ImageJ first computes a histogram with 256 bins. Therefore, there might be differences in the results obtained on 16-bit images when using the applet and the true 16-bit results obtained with this plugin.
Note that for speeding up, the histogram is bracketed to include only the range of bins that contain data (and avoid processing empty histogram bins at both extremes).

'''3.''' The result of 16 bit images and stacks (when processing all slices) is an 8 bit container showing the result in white [255] to comply with the concept of "binary image" (i.e. 8 bits with 0 and 255 values).
However, for stacks where only 1 slice is thresholded, the result is still a 16 bit container with the thresholded phase shown as white [65535]. This is to keep the data untouched in the remaining slices.
The "Try all" option retains the 16 bit format to still show the images with methods that might fail to obtain a threshold.
Images and stacks that are impossible to threshold remain unchanged.
...
E.g. the Triangle method applied to an 8 bit image and to the same image converted to 16 bits ''with scaling'' can result in different threshold values. This is because the scaling from 8 to 16 bits creates empty bins in between the scaled grey values. The Triangle method (based on a geometric approach) finds those artefactual gaps in the new 16 bit histogram which satisfy the method constraints, but which would not exist in the original 8 bit image. This cannot be prevented (for example by detecting empty histogram bins) as it would interfere with the analysis when real empty bins (as oppose to artefactual ones) exist in the image.
...
=== Default ===
This is the original method of auto thresholding available in ImageJ, which is a variation of the IsoData algorithm (described below). The '''Default''' option should return the same values as the '{{bc | Image | Adjust | Threshold | Auto}}', when selecting '''Ignore black''' and '''Ignore white'''.
...
| url = http://www.ktl.elf.stuba.sk/study/vacso/Zadania-Cvicenia/Cvicenie_3/TimA2/Huang_E016529624.pdf
...
Ported from Antti Niemist&ouml;'s [[MATLAB]] code. See [https://github.com/carandraug/histthresh here] for an excellent slide presentation and his original [[MATLAB]] code.
...
Modified from Johannes Schindelin's plugin [http://wbgn013.biozentrum.uni-wuerzburg.de/ImageJ/triangle-algorithm.html Triangle_Algorithm].
