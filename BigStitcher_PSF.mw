...
[[File:BigStitcher_psf_menu.png|center|500px]]
...
'''Extracting''' PSFs is the process of taking previously [[BigStitcher_Interest_points|detected interest points]] and creating PSFs from them. Clicking this menu entry will bring up the PSF extraction dialog:

[[File:BigStitcher_psf_extraction.png|center|500px]]

Here, you have a few options:

*'''Interest Points''': which interest points to use for PSF creation.
*'''Use corresponding interest points''': check this to only use interest points that have a correspondence in another image. This only works if you have already performed [[BigStitcher_Registration|registration]] with those points. This is useful to quickly exclude spurious detections, but for the best results, you might want to [[BigStitcher_Interest_point_management#Remove_interactively|manually curate a set of points that only consists of beads]].
...
{{Notice|We will save the extracted PSFs as TIFF files in your project directory once you click '''Save''' in the main window. This is necessary for subsequent steps such as deconvolution, so make sure to save.}}
...
[[File:BigStitcher_psf_assignment_advanced.png|center|500px]]
