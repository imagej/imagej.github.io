...
{{Warning|Clicking '''Apply''' will overwrite the transformations stored in your dataset (except for the pixel calibration, the translation from the tile configuration and, optionally, a rotation)}}

==== Tile Configuration file format ====

'''Tile configurations''' are just plain text files that adhere to a simple format:

* The file has to contain a header line specifying the number of dimensions in the form: <source> dim=3 </source>
* The locations if Views are specified by lines of the following form (the position has to have the same dimensionality specified in the header): <source> ViewSetupID;TimePointID;(position_x, position_y, ... ) </source>
* The ''time point ID'' can be left blank - in that case, the transformation for the view will be used ''at all time points'': <source> ViewSetupID;;(position_x, position_y, ... ) </source>
* Lines starting with '''#''' (comments) are ignored: <source># this is a comment. </source>
