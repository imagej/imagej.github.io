...
You can access it via ''Plugins {{arrow}} BigDataViewer {{arrow}} Big Warp'', or by modifying {{GitHub|org=saalfeldlab|repo=bigwarp|path=scripts/bigwarp_fiji_demo.bsh|label=this example script}}.  If this is not visible in your installation, try updating Fiji with ''Help {{arrow}} Update Fiji.''
...
The warped moving image can be exported as an in-memory or [https://imagej.net/docs/guide/146-8.html virtual] ImagePlus.  A virtual ImagePlus is generally faster to generate but slower to browse, whereas an in-memory ImagePlus will be slower to generate but faster to browse.
...
The "normal" BigWarp plugin accessible through ''Plugins {{arrow}} Big Data Viewer {{arrow}} BigWarp'' works well for small- and medium-sized images but not for very large volumes (that do not fit into memory).  For very large volumes, we recommend first [https://imagej.net/BigDataViewer#Exporting_Datasets_for_the_BigDataViewer converting the volume to bigdataviewer's xml/hdf5 format], then using [https://raw.githubusercontent.com/saalfeldlab/bigwarp/master/scripts/BigWarp_ImagePlus_or_Xml.groovy this script] to run BigWarp using the xml/h5 file(s). See also [https://forum.image.sc/t/issue-with-big-warp/31472 this forum post.]

[https://raw.githubusercontent.com/saalfeldlab/bigwarp/master/scripts/BigWarp_N5.groovy This script] enables you to run BigWarp using images stored using [https://github.com/saalfeldlab/n5 N5].
...
# Wan et al. [https://www.sciencedirect.com/science/article/pii/S0092867419309584 "Single-Cell Reconstruction of Emerging Population Activity in an Entire Developing Circuit."] Cell 179(2) 2019.
# Hoffman, Shtengel et al. [https://science.sciencemag.org/content/367/6475/eaaz5357 "Correlative three-dimensional super-resolution and block-face electron microscopy of whole vitreously frozen cells."] Science 367 (6475) 2020.
