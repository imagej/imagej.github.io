...
== Introduction ==
[[Image:Watershed-flooding-graph.png|thumb|250px|right|Schematic overview of watershed flooding in 1D]]Classic Watershed is an ImageJ/Fiji plugin to perform watershed segmentation of grayscale 2D/3D images using flooding simulations as described by Pierre Soille and Luc M. Vincent (1990)<ref name="Soille1990>{{ cite conference
...
<gallery widths=200px heights=200 mode="nolines">
Image:Blobs-blur.png|Gaussian-blurred blobs image used as input (radius = 3).
Image:Blobs-watershed-no-preprocessing.png|Watershed segmentation on original image (Min h = 0, Max h = 150)
Image:Blobs-blur-watershed.png|Watershed segmentation on Gaussian-blurred original image (radius = 3, Min h = 0, Max h = 150)
</gallery>

== Installation ==

The Classic Watershed plugin is part of the [[MorphoLibJ]] library. To install it, you just need to [[How_to_follow_a_3rd_party_update_site#Add_update_sites | add]] the IJPB-plugins update site:
...
'''Note''': Classic Watershed is only one of the plugins included in the [[MorphoLibJ]] suite. By following these installation steps, you will be installing as well the rest of plugins in the suite.
