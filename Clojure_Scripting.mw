{{Learn | languages}}[[wikipedia:Clojure|Clojure]] is a dialect of the [[wikipedia:Lisp (programming_language)|Lisp programming language]]. Clojure is a general-purpose programming language with an emphasis on functional programming.

= Clojure tutorial for ImageJ =

<p>Check out [http://clojure.org clojure web site] and particularly the chapter on [http://clojure.org/java_interop Java interoperability].</p>
...
*[http://en.wikibooks.org/wiki/Clojure_Programming/Examples/Cookbook Clojure Cookbook].
*[http://clojure.org/api The Clojure API] (listing of all available functions, with explanations).
*[http://clojure.org/cheatsheet Clojure cheat sheet]: a summary of all the essentials.

== Using Clojure inside Fiji ==
Go to {{bc|Plugins|Scripting|Clojure Interpreter}}. The prompt accepts any clojure code. See also Fiji's [[Script Editor]].

See [[Scripting Help]] for details on keybindings and how to use the interpreter. {{key|Ctrl}}+{{key|)}} will add all necessary ending parenthesis.
...
To create scripts, just save them as .clj text files (with an underscore in the name) in any folder or subfolder of Fiji's plugins folder, and run {{bc|Plugins|Scripting|Refresh Clojure Scripts}} to update the menus (it's done automatically at start up as well).
...
* If it was the last executed command, just type {{key|Shift}}+{{key|R}} (shortcut to "Process - Repeat Command").
...
*[https://github.com/funimage/funimage/ Funimage] provides a library for convenient Clojure coding within Fiji. Alleviating much of the need for type-hinting, and some of the burdens involved in handling more complicated data structures, such as those from ImgLib2.

See the {{ListOfUpdateSites}} for information on setting up the Funimage update site.
...
* The <b>[https://imagej.net/developer/api/ij/ImagePlus.html ImagePlus]</b>, which wraps the ImageProcessor and contains properties and pointers to the ROI (region of interest) and the ImageWindow that may be displaying the image.
* The <b>[https://imagej.net/developer/api/ij/process/ImageProcessor.html ImageProcessor]</b>, which is an abstract class enabling the high-level manipulation of and access to pixels. Its subclasses each wraps a different kind of data type:
** ByteProcessor - byte[]
** ShortProcessor - short[]
** FloatProcessor - float[]
** ColorProcessor - int[] (byte-packed ARGB, but Alpha channel is ignored)
* The <b>[https://imagej.net/developer/api/ij/ImageStack.html ImageStack]</b> which contains unfortunately not an array of ImageProcessor, but an Object[] containing an homogeneous list of equal length byte[], or float[], etc.

For extensive documentation, see the [http://albert.rierol.net/imagej_programming_tutorials.html#ImageJ%20programming%20basics Anatomy of an ImageJ image] ImageJ programming basics tutorial.
...
ImageJ though provides a mid-level resizing method, via the [https://imagej.net/developer/api/ij/plugin/CanvasResizer.html CanvasResizer] class:
...
With [[Imglib]], pixels are stored in native arrays of primitives such as int, float, double, etc. (or other more interesting forms, such as [http://download.oracle.com/javase/1.5.0/docs/api/java/awt/Shape.html Shape]. Such pixels are accessed with intermediate proxy objects that the JIT is able to completely remove out of the way.

From Clojure, there are many ways in which to access the pixels. Here we list some examples of the pixels accessed as a Collection of accessor [https://fiji.sc/javadoc/mpicbg/imglib/type/Type.html Type] objects.
...
What's more, the [https://fiji.sc/javadoc/script/imglib/math/Compute.html Compute]/inFloats method runs in parallel, with as many processors as your machine has cores. If you'd rather not execute the operation in parallel, add the desired number of threads as an argument to <i>inFloats</i>.

All mathematical operations listed in java.lang.Math have a corresponding constructor for execution in Compute/inFloats. See the documentation for the [https://fiji.sc/javadoc/script/imglib/math/package-frame.html script.imglib.math package].
...
(Code adapted from a Common Lisp version by [http://nklein.com/2011/02/calculating-the-mean-and-variance-with-one-pass/ Patrick Stein].)
...
When done, just run the {{bc|PlugIns|Scripting|Refresh Clojure Scripts}} plugin.
...
* {{GitHub|repo=fiji|path=plugins/Examples/Multithreaded_Image_Processing.clj|label=Multithreaded_Image_Processing.clj}}: illustrate, with macros (via [http://clojure.org/macros#toc9 defmacro]), how to automatically multithread the processing of an image using arbitrary subdivision of the image, such as one line per thread, for as many threads as cores the CPU has.
* {{GitHub|repo=fiji|path=plugins/Examples/blend_two_images.clj|label=blend_two_images.clj}}: illustrates how to open two images from an URL, and blend the gray image into each channel of the color image.
* {{GitHub|repo=fiji|path=plugins/Examples/celsius_to_fahrenheit.clj|label=celsius_to_fahrenheit.clj}}: illustrates the usage of a Swing GUI, and how to instantiate anonymous classes from an interface (via [http://clojure.org/java_interop#toc20 proxy] Clojure function). This example is taken from the [http://clojure.org/jvm_hosted Clojure website].
* {{GitHub|repo=fiji|path=plugins/Examples/random_noise_example.clj|label=random_noise_example.clj}}: illustrates how to declare a function inside a closure (for private access to, in this case, a unique instance of a random number generator), and then fill all pixels of a ByteProcessor image with a random byte value.
* {{GitHub|repo=fiji|path=plugins/Examples/Command_Launchers/Command_Launcher_Clojure.clj|label=Command_Launcher_Clojure.clj}}: illustrates how to create a GUI with a KeyListener, so that the typed text changes color from red to black when it matches the name of an ImageJ command. This example is also under the [[Scripting comparisons]], along equivalent versions written in Java, [[Jython Scripting|Jython]], [[Javascript_Scripting|Javascript]] and [[JRuby_Scripting|JRuby]].
* {{GitHub|repo=fiji|path=plugins/Analyze/Dynamic_ROI_Profiler.clj|label=Dynamic ROI Profiler}}: illustrates how to add a MouseMotionListener and a WindowListener to an ImageWindow of an open image. Reads out the ROI (Region Of Interest), and if it's a line, polyline or rectangle, plots the profile of pixel intensity along the line. As the mouse moves or edits the ROI on the image, the profile is updated.
...
Below, the sequence <i>fibs</i> is defined in such a way that it contains all possible [[wikipedia:Fibonacci number|fibonacci numbers]]. Since such sequence is infinite, we declared it as <i>lazy</i> sequence, that creates new elements only when they are asked for.
...
Using ahead of time (AOT) compilation with [http://clojure.org/compilation  gen-class], any clojure code can be compiled to a java class. Such class can then be used from java code, or from any scripting language like [[Jython Scripting|jython]], [[JRuby Scripting|jruby]], [[Javascript Scripting|javascript]], and [[:Category:Scripting|any other]].

One way to do so is to place a [http://clojure.org/compilation gen-class] declaration in a namespace block.
...
The following clojure example contains a namespace declaration that includes some imports and also the gen-class.
...
Clojure supports thread concurrency without explicit locks. Compared to java code, this is a gigantic step forward: locks, and particularly multiple locks, are very hard to get right and very, very hard to debug properly (but see [http://albert.rierol.net/java_tricks.html#How%20to%20debug%20a%20multithreaded%20java%20program debugging multithreaded java programs]).

The most basic building blocks are [http://clojure.org/refs<i>references</i>], which are created with the <i>ref</i> function, and modified within transaction blocks (defined by <i>dosync</i>) using <i>commute</i> or <i>alter</i> functions (and [http://clojure.org/refs others]).
...
Some explanations on the above macro syntax (see also [http://clojure.org/macros clojure's macro syntax page]):
...
The above is an extract from a clojure GUI for XMMS2, available at github {{GitHub|org=acardona|repo=xmms2-clj|label=xmms2-gui}}.
