{{BigNotice | See [[:Category:Colocalization]] for pages about colocalization.}}
{{Learn | techniques}}
== What is colocalization? ==

Suppose you are given some images by a colleague, or have some images of your own, and you want to measure the amount of colocalisation between two of the dyes or stains in the images. First you have to define what you mean by colocalisation, and that is not trivial. For one place to start reading about colocalisation and for how to correctly capture quantitative fluorescence microscopy images suitable for colocalisation analysis, look here: [http://www.biodip.de/wiki/Teaching_Material#IPF_Image_Processing_Courses Image Processing Courses at BioDIP, Dresden]. A recent review is [http://ajpcell.physiology.org/content/300/4/C723 by Dunn in 2011].
...
# '''Pearson's correlation coefficient.''' It is not sensitive to differences in mean signal intensities or range, or a zero offset between the two components. The result is +1 for perfect correlation, 0 for no correlation, and -1 for perfect anti-correlation. Noise makes the value closer to 0 than it should be. 
...
[[Image:SpiralsRGY.png|512px|center]]

Most people might think that the image contains 4 distinct colours: 2 sets of thin spirals are in dark red and dark green, and 2 thick prominent spirals of yellow-green and yellow. However, the yellow and yellow-green actually have <b>exactly the same color!</b> You can verify this yourself by calling {{bc | File | Open Samples | [[Spirals (Macro)]]}} in Fiji.

[[Image:GreenYellowSimultColourIllusion.png|512px|center]]
...
[[Image:Ch1coloc.gif]]
[[Image:Ch2coloc.jpg]]
[[Image:Scatterplotcoloc.jpg]]
[[Image:ColocQuadrants.jpg]] 
...
'''Open this sample data file [https://fiji.sc/samples/colocsample1bRGB_BG.tif colocsample1bRGB_BG.tif].''' Then use the "Image-Color-Split Channels" menu command to get a separate z stack for the 2 dyes (you can throw the blue one away!).
...
There are several plugins available for performing colocalization analysis. In addition to the options described below, see also the [[:Category:Colocalization|index of pages related to colocalization]].
...
[[JaCoP]] is a compilation of co-localization tools:
...
See the [[JaCoP]] page for full details.
...
See the [http://punias.free.fr/ImageJ/colocalization-finder.html Colocalization Finder web page] for further details.
...
# Is the image data noisy?
# Has there been lossy compression?
# Is the intensity information saturated / clipped / overexposed?
# Is there a problem with uniform background / detector offset?
...
# Significant noise (uncertainty in the pixel values - usually from detection of too few photons) means the methods we will use will significantly underestimate the true colocalization, or even completely fail to give the "right" result.
# Lossy compression messes up the intensity information of the pixels, causing the colocalization result to be more or less wrong. 
# Intensity clipping/saturation is bad news. Pixel intensity correlation measurements rely on the pixel intensities being true and not clipped to 255 when they were really higher! See the [[Detect Information Loss]] tutorial for details on how to detect such problems.
# We should look for wrong offset / high background (since this confuses the auto threshold method, since zero signal is not zero pixel intensity but some larger number)
# Spatial resolution
...
Linear regression solution: If the image data are close to ideal, with comparable mean intensity in both channels and no problems exist with offset/background, then by definition, the gradient of the regression line will be close to unity (1) and the intercept will be close to zero. These are both good things for the quality of the result. This information might be less useful in other situations, e.g. where there is only sparse data in one channel, or there are multiple colocalising populations of signals (several clearly independent patches/blobs in the scatterplot).
           ...
           <li>
The % volume and intensity colocalised values are pretty useless, unless you use a biologically relevant region of interest, as they are very dependent on how much stuff and how much background there is in a certain image, and you will get totally different values for different images of the same sample, that happen to have different areas of background.
In images that have no empty regions, like tissue samples, it might then be a useful number. Maybe % intensity above threshold colocalised might be a useful value in some cases? See section 6.3 at http://www.uhnresearch.ca/facilities/wcif/imagej/colour_analysis.htm for more explanation of what the different measurements exactly are, or look at the code. 
           </li>
...
<span style="color: red">Note: This plugin is no longer actively developed or supported. Use [[Coloc 2]] instead, which does the same thing, only more correctly, and as described in the original publication by Costes, instead of making a nasty assumption and shortcut.</span>

The [[Colocalization Test]] plugin performs the Costes test for statistical significance (which you should ALWAYS do after calculating the thresholded Manders coefficients and the scatterplot). It is in the menus at {{bc | Analyze | Colocalization | Colocalization Test}}
