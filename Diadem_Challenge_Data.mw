This page describes how to visualize the various data sets provided as part of the [http://www.diademchallenge.org/ Diadem Challenge] in Fiji, and in particular viewing the traced neurons with [[Simple Neurite Tracer]].  The most important point about this is that the SWC files distributed as part of the challenge all use co-ordinates that are image-coordinates rather than world co-ordinates.  (This isn't how I interpret [http://dx.doi.org/10.1016/S0165-0270(98)00091-0 the paper that describes the SWC format], but in practice different software uses different conventions, so one has to deal with both.)  This means that you should always select the "ignore image calibration" option when importing the SWC file into Simple Neurite Tracer.  The other key point is that the image data are distributed with each slice in a separate TIFF file and don't specify the slice separation in those files.  So, after importing the sequence you will need to correct the calibration according to information on the Diadem challenge website or they will look odd when rendered in 3D.  I describe these steps in more detail below.
...
Unfortunately these TIFF files do not have the separation of these slices encoded correctly, so you should correct this manually.  If you go to {{bc | Image | Properties}} you will see that the pixel width, pixel height and voxel depth are all set to the same value (0.3296485).  The [http://www.diademchallenge.org/olfactory_projection_fibers_readme.html page with information about the data set] tells us that the z-separation should be three times the x and y separation, so correct the voxel depth to 3 times 0.3296485, like this:

[[Image:Diadem_Challenge_Data-4.png]]

... and then press "OK".  To save yourself from having to re-enter this calibration information, and to be able to open the stack from the "Recent Files" menu, etc. I think it's helpful to now save this stack as a single TIFF.  So, I would go to "{{bc | File | Save As | Tiff ...}}" and save this as OP_1.tif in the directory above the one with individual slices.

Now start the tracing plugin by going to {{bc | Plugins | Segmentation | Simple Neurite Tracer}}.  If you keep the default options, you should see a display like this:
...
Firstly, however, it will help to load each image sequence and save out the stack to a single TIFF image stack file.  For each of the 6 directories, do the following: go to {{bc | File | Import | Image sequence}} and select the first file in the directory (01.tif).  After opening with the default options, save this out into the parent directory with the following names:
...
Now you can stitch together the images using these offsets by going to {{bc | Plugins | Stitching | Stitch Collection of Images}}, browsing to the TileConfiguration.txt file you've just created and making sure you uncheck the box "compute overlap".  It should look something like this:
...
Next, you should correct the Z-calibration of the stitched stack.  The data set page tells us that the z separation is 3.03 pixels, so open up {{bc | Image | Properties}} and set the Voxel Depth to 3.03 times the Pixel Width:

[[Image:Diadem_Neocortical_Layer_6_Axons-3.png]]

Now you should save the stitched stack, and work from that from now on.  (Since the stitched stack isn't very large (156MB) one wonders why this wasn't distributed already stitched.)  Save the stack with "{{bc | File | Save As | Tiff...}}".

To load the SWC files, start up the tracer with {{bc | Plugins | Segmentation | Simple Neurite Tracer}}.  You should see something like this after it has started, with the stack rendered in the 3D Viewer:
...
To load the files as a virtual stack, go to "{{bc | File | Import | Image Sequence ...}}" and select the first file in the sequence, "01.tif".  You will be presented with the import dialog, in which you should make sure that "Use virtual stack" is selected.
...
After loading this you need to convert this to 8-bit.  You can do this in any number of ways, and if you're actively working on a tracing task with this data you will probably want to do this in such a way that picks out the brown coloured neuronal processes.  However, a simple way just for viewing purposes is to use the {{bc | Image | Color | RGB to Luminance}} option.  (Another option is to use {{bc | Image | Type | 8-bit}}.)  This will produce results like this:

[[Image:Diadem_Cerebellar-3.png|750px]]

You can close the original virtual stack now.

It is a good idea to invert this image, since the 3D Viewer will make the low values of the image more transparent, and Simple Neurite Tracer assumes that neurons are brighter than their background.  So, go to {{bc | Edit | Invert}} and select "Yes" in response to the question about whether to process all 34 images.  The results should look like this:

[[Image:Diadem_Cerebellar-4.png|750px]]

Before saving this out as a single 862MB stack, it is a good idea to correct the calibration of the image.  The [http://www.diademchallenge.org/cerebellar_climbing_fibers_readme.html data set web page] tells us that the separation between slices is 8.08 pixels.  So, if you bring up the {{bc | Image | Properties}} dialog, then you can correct the voxel depth to be 8.08 times the pixel width:

[[Image:Diadem_Cerebellar-5.png]]

Now save this as a single TIFF stack called CF_1.tif by going to "{{bc | File | Save As | Tiff ...}}", saving it in the directory above the one with the slices, to avoid confusion.

You can now start the tracer with {{bc | Plugins | Segmentation | Simple Neurite Tracer}}.  This may take some time, since there is a lot of data to load in to the 3D viewer.  The result should look like this:

[[Image:Diadem_Cerebellar-6.png|750px]]

You may wish to adjust the transparency in the 3D viewer by selecting the image volume (left click on the image in the 3D viewer) and then going to "{{bc | Edit | Attributes | Change Transparency...}}":
...
As with the other stacks, you should load the image stack using {{bc | Import | Image Sequence}} and selecting the "Use virtual stack" option:

[[Image:Diadem_Hippocampal_CA3_Interneuron-2.png]]

That should load in the image data:

[[Image:Diadem_Hippocampal_CA3_Interneuron-3.png|750px]]

Now convert it to 8-bit with {{bc | Image | Type | 8-bit}}.  (The same comments as above regarding converting to 8-bit apply here as well.)

[[Image:Diadem_Hippocampal_CA3_Interneuron-4.png|750px]]

... and invert the image with {{bc | Edit | Invert}}, saying "Yes" to the question about whether to process all 110 slices:

[[Image:Diadem_Hippocampal_CA3_Interneuron-5.png|750px]]
...
And then I would save this file as a TIFF stack (with {{bc | Save As | Tiff...}}) so that you don't have to go through these steps again.  That will need about 2GB of disk space, and will only work if your filesystem supports single files of that size.

If you now start {{bc | Plugins | Segmentation | Simple Neurite Tracer}} with its default options you should (eventually!) see something like the following:

[[Image:Diadem_Hippocampal_CA3_Interneuron-7.png|750px]]
...
[[Image:Diadem_Hippocampal_CA3_Interneuron-9.png|750px]]

== Neuromuscular Projection Fibers ==

There are a number of caveats regarding these image data and reconstructions which are [http://www.diademchallenge.org/neuromuscular_projection_fibers_readme.html listed on its web page].

These 152 image stacks are in total so large that stitching them together and dealing with the stitched stack is not likely to be helpful.  Instead, I would suggest that you just stitch together those images that you need to cover a particular SWC file.  Since there are so many input stacks, I've written a clojure script for Fiji to help process these, which you can [http://gist.github.com/508411 view and download via github].  On running this script you will be asked to select a directory - you should select the directory that contains all of your unpacked image stack directories (probably called "000", "001", etc.)  For each of these directories it will then:
...
Finally, the script will output a TileConfiguration.txt file in the same directory.  Supposing one had vast amounts of RAM, this file could be used unaltered with {{bc | Plugins | Stitching | Stitch Collection of Images}}, as above.  However, the file is still useful even with modest amounts of RAM, since you can copy this file to a new name and edit out all but a few of the images that are listed there.  For example, here are image stacks 000 to 007 stitched together:

[[Image:Neuromuscular-0-7-stitched.png|750px]]

That script may be useful as a basis for a smarter selection of images to stitch for particular reconstructions.

You should then be able to adjust the SWC import options in order to import the reconstructions in the correct positions - you will need to understand the translations described for the reconstructions  [http://www.diademchallenge.org/neuromuscular_projection_fibers_readme.html on the web page] in order to get this right.  (I haven't had time to do this accurately myself yet.)
