...
Please note that the elastic alignment and montage plugin available through Fiji, is based on a publication. If you use it successfully for your research please cite our work:

S. Saalfeld, R. Fetter, A. Cardona and P. Tomancak (2012) "Elastic volume reconstruction from series of ultra-thin microscopy sections, ''Nature Methods'', 9(7), 717-720 [http://www.nature.com/nmeth/journal/vaop/ncurrent/full/nmeth.2072.htmll Webpage] [[Media:Saalfeld_nmeth.2072.pdf|PDF]] [[Media:Saalfeld_nmeth.2072-S1.pdf|Supplement]]

Supplementary videos demonstrating the performance of the method are available [http://fly.mpi-cbg.de/elastic here].

==Introduction==

[[Image:Aligned-series-crop-512.gif|thumb|right|166px|Example 2: Example for elastic alignment and montaging.  7 serial TEM sections of the neuropil of a ''Drosophila melanogaster'' first instar larva, detail at 100% pixel resolution. Image courtesy of {{Person:Albertcardona}}.]][[Image:Aligned-series-512.gif|thumb|right|166px|Example 1: Example for elastic alignment and montaging.  7 serial TEM sections of the neuropil of a ''Drosophila melanogaster'' first instar larva, downscaled by a factor of 12.  Image courtesy of {{Person:Albertcardona}}.]]
...
[[Image:Correlation.png|thumb|right|361px|Fig. 2: Match filter based on the correlation surface. Starting from an approximate alignment (e.g. affine), for each vertex of the spring mesh, an offset is searched calculating the PMCC coefficient r of a block at all possible x,y translations in a given local vicinity over the overlapping image. The above windows display six examples of such correlation surfaces for translations in a square region with the origin in the middle. The [[wikipedia:Pearson product-moment correlation coefficient|PMCC coefficent]] r is grey-coded in the range from -0.7 to 0.7. The candidate for the translational offset is the translation with maximal r. Candidates are rejected if either r was too low (not similar), there was more than one maximum with very similar r (ambiguous), the maximum is not well localized in both dimensions (an edge pattern that fits everywhere alongside the edge). ]]
Corresponding locations are searched through block matching.  Initialized from an approximate linear pairwise alignment that is estimated using [[Feature Extraction|local image features]], the local vicinity around each vertex is inspected for an optimal match.  We use the The [[wikipedia:Pearson product-moment correlation coefficient|PMCC coefficent]] ''r'' of a patch around the vertex and the overlapping patch in the other image as the quality measure for a match.  The location with maximal ''r'' specifies the offset of the vertex relative to the initial linear alignment.

We perform block matching at a reasonably down-scaled version of the images.  The ideal scaling factor depends on the application and quality of the signal.  To overcome the reduced accuracy of the estimated offset, we use Brown's method<ref name="BrownL2002">{{cite conference
...
See also [[Test Block Matching Parameters]] plugin
...
[[Image:Filters.jpg|thumb|right|361px|Fig. 4: The effect of block matching filters. (ssTEM image data courtesy of [http://janelia.org/people/scientist/richard-fetter Richard Fetter])]]
;Correlation Filters
...
:Find documentation at the [[Feature Extraction]] page.  The '''approximate transformation''' model as specified in this dialog is used for geometric consensus filter and pairwise pre-alignment only.  It should approximate the actual deformation as closely as possible.  That is, you should select an '''affine transformation''' in almost any case. 
