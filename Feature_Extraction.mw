...
| source                 = {{GitHub|org=axtimwalde|repo=mpicbg|label=on GitHub}}
| released               = 2008
...
The plugins "Extract SIFT Correspondences" and "Extract MOPS Correspondences" identify a set of corresponding points of interest in two images and export them as PointRoi.  Interest points are detected using the [[wikipedia:Blob detection|Difference of Gaussian detector]] thus providing similarity-invariance.  Corresponding points are best matches from local feature descriptors that are consistent with respect to a common geometric transformation.

The plugins use the [[wikipedia:Scale-invariant feature transform|Scale Invariant Feature Transform]] (SIFT) and [http://research.microsoft.com/research/pubs/view.aspx?msr_tr_id=MSR-TR-2004-133 Multi-Scale Oriented Patches] (MOPS) for local feature description.  The thus established matches are filtered using the [[wikipedia:RANSAC|Random Sample Consensus]] (RANSAC).
...
:The '''SIFT'''-descriptor consists of ''n''&times;''n'' gradient histograms, each from a 4×4px block. n is this value.  Lowe (2004) uses ''n''=4.  We found larger descriptors with ''n''=8 perform better for Transmission Electron Micrographs from serial sections.
