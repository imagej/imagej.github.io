...
{{FijiMenu}}Distributing your software component as part of [[Fiji]] is an effective way to immediately and easily put it into the hands of many users, as well as to actively participate in the community of ImageJ software development. However, doing so comes with a few corresponding rules.
...
A "core" [[Fiji]] project is one distributed on the [http://update.fiji.sc Fiji update site]. Such projects are subject to the requirements discussed below. Conversely, if you distribute your [[ImageJ]] extension on a separate update site, this page does not apply.
...
There are many corollaries to this wisdom, the most prominent: if you write software in your endeavor to discover new insights, [[Open Source]] is the way that brings you farthest. Withholding the source code—like any other method to obstruct other researchers' work, e.g. refusing to share materials and methods—will [[Why Closed-Source Is Wrong|invariably have the opposite effect]] in the long run. Likewise, working with interested parties to improve one's project will invariably lead to a much better and stronger result.
...
The following criteria apply to projects hosted in the [https://github.com/fiji fiji organization]:
...
** Extending the [[Architecture#Maven_component_structure|pom-scijava parent POM]]
* Components use the groupId <code>sc.fiji</code>.
* Components are [[Versioning|versioned according to SemVer]].
* The project uses [[Issues|GitHub Issues]] for issue tracking.
* The project has a dedicated page here on the ImageJ wiki.
* The [[Governance|Fiji maintainers]] may make commits and [[Releases|release new versions]] of the component as needed, so that Fiji as a whole continues to work as intended.
* The <code>master</code> branch is considered ''release ready'' at all times, meaning it compiles with passing tests, and is ready for downstream consumption.

=== External projects ===

Projects that reside outside the [https://github.com/fiji fiji organization] are not subject to the requirements above. But it is then the project maintainer's responsibility to ensure the project continues to function properly in up-to-date installations of Fiji. This might entail code changes as ImageJ and Fiji evolve.

== Continuous integration: Travis CI ==

To verify that the Fiji components build without problems, and that all regression tests pass, every Fiji project's source code repository is connected to a [[Travis CI]] job that builds and tests the source code, and deploys the [[#Maven artifacts|Maven artifacts]], whenever a new revision is made available.

Have a look at the [[Travis]] page for instructions on setting it up.
...
In general, if the rest of the Fiji distribution upgrades to a new major version of a library on which your component depends, your component must also be upgraded to use the new version. Such decisions are typically reached after [[Communication|discussion on public channels]].
...
Versioning through the pom.xml has several advantages to facilitate [[Reproducibility|reproducible builds]], including:
...
* From the POM. This is the most reliable option. For convenience, [https://github.com/scijava/scijava-common scijava-common] provides a utility class to assist in version retrieval: [https://github.com/scijava/scijava-common/blob/scijava-common-2.39.0/src/main/java/org/scijava/util/VersionUtils.java#L51 VersionUtils].
* Alternatively, the [http://docs.oracle.com/javase/7/docs/api/java/lang/Package.html#getSpecificationVersion%28%29 specification] or [http://docs.oracle.com/javase/7/docs/api/java/lang/Package.html#getImplementationVersion%28%29 implementation] version can be used - for example, as in the [https://github.com/fiji/LSM_Reader/commit/a6b26290ad71667efd75d77f3eef95d445d6eaff LSMReader]. Core Fiji libraries follow a convention of setting these versions to match the pom version, and they are set at the manifest level to ensure they are they same for all packages in a given component. '''However''', the two versions are allowed to differ - each package is allowed its own specification and implementation version! Furthermore, classes in a default package will not be able to retrieve either version. So these functions can not be relied on as a general solution.
...
* All of Fiji's components are deployed by [[Travis CI]] to the [[Architecture#Maven_repositories|ImageJ Maven repository]] or to [http://oss.sonatype.org/ OSS Sonatype]. That way, all Fiji components can be added easily as dependencies to downstream projects.
...
To leverage the power of [[open source]], the default for discussions should be to use [[Communication|public channels]]. In other words, the question to ask should be "Is there any good reason why this conversation should be private?" instead of the opposite.
...
| colspan=1 style="text-align: center" | '''[[Maven]]'''
|-
| '''Classification'''
| '''Component'''
| '''Core?<sup>1</sup>'''
