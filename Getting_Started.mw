...
[[Image:Oval_Tool_Option_Dialog.png]]

If there is a small red arrow in the lower right corner of the tool icon, you can right-click ({{key|Ctrl}}+{{key|click}} on an Apple mouse) and select an alternative selection tool (e.g. a circular brush selection tool which shares the icon with the ellipse selection tool). Example:

[[Image:Alternative_Tools.png]]
...
[[Image:Status-bar-with-progress.png]]
...
[[Image:Default-status-message.png]]
...
Whenever you open an image, be it via {{bc | File | Open}}, Drag 'n Drop or {{bc | File | Open Samples}}, ImageJ will open an image window.
...
If your image does not have meta-data about the real resolution, you can set the resolution explicitely with {{bc | Image | Properties...}} or by following the [[SpatialCalibration|tutorial on spatial calibration]].
