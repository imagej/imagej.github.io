...
MaMuT is a Fiji plugin that depends on its components to run. Download Fiji from [https://fiji.sc/#download its website] if you do not have it already.  

MaMuT is not shipped with Fiji by default; you have to install it in Fiji. Thanks to the ImageJ [[Updater|'''Updater''']], there is not much to do. Simply follow the instructions on how to [[How_to_follow_a_3rd_party_update_site|'''follow a 3rd party site''']], and subscribe to the MaMuT update site. Here is briefly how to do it. 

Got to {{bc | Help | Update...}} and click on the '''Manage update sites''' button. In the window that appear, find the '''MaMuT''' checkbox and tick it. Then close the window. In the files to update list, there should be a '''plugins/MaMuT_.jar''' appearing. Click on '''Apply changes''' button, then restart Fiji.

[[Image:bdv-MaMuT_UpdateSite.png]]
...
* they are [[BigDataViewer#Exporting_from_ImageJ_Stacks|'''opened as an ImageJ stack''']], or
* they come from a [[BigDataViewer#Integration_with_Fiji.27s_SPIMage_Processing_Tools|'''SPIM processing pipeline''']].
...
Once you have prepared your images for opening in the BigDataViewer, you should have a <tt style="font-size: 1.25em;">.xml</tt> file and a possibly very large <tt style="font-size: 1.25em;">.h5</tt> file on your computer. The <tt style="font-size: 1.25em;">.xml</tt> file must be the output of the BigDataViewer data preparation. It should start with the following lines:
...
Instead of preparing your own dataset, you can also download an example dataset for this tutorial [http://bds.mpi-cbg.de/mamut-example here].
...
[[Image:bdv-MaMuT_MenuItems.png]]

A file explorer window opens. Browse to the <tt style="font-size: 1.25em;">.xml</tt> file of the BDV file pair. After a little while, the MaMuT main GUI window opens:

[[Image:bdv-MaMuT_MainGUIWindow_beginning.png|200px]]
...
If you are familiar with the BigDataViewer, there is a few useful things to know. When we launch a new MaMuT annotation, we actually import some of the settings of the BigDataViewer in the session. For instance, in the BigDataViewer you can save and restore settings using the {{bc | File | Save settings}} and {{bc | File | Load settings}} menu item. This creates and loads a XML file typically named <tt style="font-size: 1.25em;">bdv_file.settings.xml</tt> if the BigDataViewer file is named <tt style="font-size: 1.25em;">bdv_file.xml</tt>. If this settings file is present in the same folder than the BigDataViewer file when launching a new MaMuT annotation, then MaMuT imports the brightness adjustments and bookmarks found in this file. The manual transformation and the groups are however not imported.

=== Saving to a MaMuT file. ===

To save the annotation to a MaMuT file, press the '''Save''' button on the main GUI window. This will let you save to a file which is typically named <tt style="font-size: 1.25em;">bdv_file-mamut.xml</tt> if the BigDataViewer file was named <tt style="font-size: 1.25em;">bdv_file.xml</tt>.
...
* <tt style="font-size: 1.25em;">MaMuT_Parhyale_demo.h5</tt>: this is typically a very large file that contains the raw image data. 
* <tt style="font-size: 1.25em;">MaMuT_Parhyale_demo.xml</tt>: this is the BigDataViewer file. It is a XML file that lists where is the image data and how the possibly several views are arranged in space. This file can be opened in the BigDataViewer, and this is the one you open when you launch a new MaMuT annotation
* <tt style="font-size: 1.25em;">MaMuT_Parhyale_demo-mamut.xml</tt>: this is the MaMuT file. It contains as we said the the annotation and pointers to the image file.
...
To reload a MaMuT file, go to the {{bc | Plugins | MaMuT | Load MaMuT file}} Fiji menu item, and browse to the MaMuT XML file. This will load the annotation and the image data. Careful, do not mix the MaMuT XML file with the BigDataViewer XML file when opening the later.
...
picture of a mammoth. A BigDataViewer window should appear. 

[[Image:bdv-MaMuT_ViewerExample.png|500px]]
...
* {{key press|Right Drag}} or {{key press|Middle Drag}} Move in the XY-plane of the view.
...
* {{key press|up}} / {{key press|down}} Zoom in / out. With {{key press|Shift}} fast zoom. With {{key press|Control}} slow zoom.
...
* {{key press|up}} / {{key press|down}} Rotate clockwise / counter-clockwise around the chosen rotation axis.
...
* {{key press|Shift|Y}} or {{key press|Shift|C}} Rotate to the XZ-plane of the current source (look along the y-axis of the current source).
* {{key press|Shift|Z}} Rotate to the XY-plane of the current source (look along the z-axis of the current source).


'''To navigate in time:'''

* {{key press|N}} Move to the previous time-point.
* {{key press|M}} Move to the next time-point.
* {{key press|[}} Jump to the previous time step (explained below).
* {{key press|]}} Jump to the next time step.
...
[[Image:bdv-MaMuT_ViewerOverlays2.png|700px]]
...
* {{key press|A}} to add a spot at the mouse location.
* {{key press|D}} to delete the spot under the mouse location.
* {{key press|Q}} / {{key press|E}} to decrease / increase the spot radius. Use {{key press|Shift}} / {{key press|Control}} to change the radius by a greater / smaller amount.
* {{key press|Space}} is used to move a spot in the XY view plane: put the mouse pointer inside the spot you want to move, then press and hold space while moving the mouse. The spot will follow your mouse until you release the {{key press|Space}} key.
...
* Move to another time-point ({{key press|N}}, {{key press|M}}, {{key press|[}}, {{key press|]}}, the time slider).
...
* {{key press|L}} toggle a link between the two spots of the selection. This does nothing if there is not exactly two spots in the selection. 
...
[[Image:bdv-MaMuT_DesiredTracking_2.PNG|500px]]
...
* {{key press|[}} Jump to the previous time step.
* {{key press|]}} Jump to the next time step.
...
[[Image:bdv-MaMuT_ConfigureTrackDisplay.png]]
...
* There are <tt style="font-size: 1.25em;">track features</tt>, defined for whole tracks. For instance: the number of spots in a track, the displacement from the start to the end of track, etc. If you pick a track feature, all the links of a track will share the same color.
* There are <tt style="font-size: 1.25em;">edge features</tt>, defined for a single link. These are features that are simply defined by a link between two spots, for instance, the instantaneous velocity for the link. Then each link may be of a different color.
...
[[Image:bdv-MaMuT_TrackSchemeStart.png |500px]]
...
* {{key press|Mousewheel}} scrolls up and down.
* {{key press|Shift|Mousewheel}} scrolls left and right.
* {{key press|Space|Mousedrag}} pans the view, à la ImageJ. If you pull the mouse out of the TrackScheme window, it will scroll in the direction of the mouse cursor.
* {{key press|Space|Mousewheel}} is used for zooming.
...
* {{key press|+}} zoom in. 
* {{key press|-}} zoom out.
* {{key press|equals}} restores the zoom to its default level.
...
Finally, there is two '''Styles''' for the spot schemes. The <tt style="font-size: 1.25em;">simple</tt> style sonly displays them as round spots. The <tt style="font-size: 1.25em;">full</tt> style displays them as rounded boxes, with each spot name apparent. In the <tt style="font-size: 1.25em;">full</tt> style, small thumbnails can be captured and displayed in TrackScheme for all spots. Just next to the menu, there is a thumbnail button. If activated, thumbnails are collected from all spots, using the image source they were created on. Thumbnails are captured around the spot location, using their radius plus a tolerance factor. Interestingly, the '''Spot display radius ratio''' is used to define the size of the thumbnail. For instance, with a display factor of 2, you can obtain the layout below. Notice that the spot boxes can be resized manually to better display thumbnails.

[[Image:bdv-MaMuT_TrackSchemeThumbnails.png|500px]]
...
[[Image:bdv-MaMuT_TrackSchemeExportButtons.png|500px]]
...
* {{key press|Left-Click}} outside a spot to clear the selection. 
* {{key press|Shift|Left-Click}} on a spot or link to add or remove this spot or link to the selection.
* {{key press|Mousedrag}} to select multiple spots and links in a selection box. Hold {{key press|Shift}} to add them to the current selection.
...
* <tt style="font-size: 1.25em;">Select whole track</tt> will include all the spots and the links of the tracks the selection belongs to.
* <tt style="font-size: 1.25em;">Select track downwards</tt> walks from the spots and links in the selection, and add the spots and links that connect from them, forward in time (downward in TrackScheme).
* <tt style="font-size: 1.25em;">Select track upwards</tt> does the same, but backward in time.
...
[[Image:bdv-MaMuT_TrackSchemeSelectionOnly.png|700px]]
...
To generate the plot, click the '''Plot features''' button. 
...
[[Image:bdv-MaMuT_TrackSchemeSideBar.png|800px]]
...
[[Image:bdv-MaMuT_TrackSchemeUnlaidSpots.png|700px]]
...
Here is a way to do it. In TrackScheme using {{key press|Mousedrag}} select all the spots in the unlaid column. {{key press|Right-click}} somewhere in TrackScheme to make the pop-up menu appear. One of the menu item should be something like <tt style="font-size: 1.25em;">Link 10 spots</tt>. Choose this one. Each spots is then linked to the next one, frame by frame, and the links should appear in TrackScheme and in the MaMuT viewer. You just created a new track.
...
[[Image:bdv-MaMuT_TrackSchemeDragAndDropLink.png|500px]]



=== Removing spots and links. ===

The last you link you added may have strongly perturbed our annotation, particularly if you did what was on the screenshot above. Correct it by removing the last link. Simply select it press {{key press|Delete}}. The same key will remove everything in the selection.
...
Spots also have a name, that you can see either in the MaMuT viewer by checking the '''Display spot names''', either in TrackScheme by using the <tt style="font-size: 1.25em;">full</tt> display style. They are all called <tt style="font-size: 1.25em;">ID##</tt> by default, which is not very informative.
...
You can also set the name of several spots at once. For instance, select the the whole second track (now named 'A') and {{key press|Right click}} (outside of a spot) to bring the popup menu. There is an item called <tt style="font-size: 1.25em;">Edit 10 spot names</tt>. The closest spot is changed to an edit box. When you validate the new name, all the selected spots get this new name.

Apart from their use to mark some biological meaning to the annotations, spot names have several uses. There is a search box in TrackScheme toolbar that centers the view on spots with name matching the text you enter there. Press {{key press|Enter}} to loop over all the matching spots.
...
[[Image:bdv-MaMuT_SemiAutoTracking_2.png]]
...
Then, amongst all the candidate cells that remain, the one with the largest quality value is accepted as a successor and linked to the initial cell. The process is then reiterated using the newly found cell as initial cell. The parameter <tt style="font-size: 1.25em;">Max nFrames</tt> allows to define a maximal number of times (and as such, of time-points) we iterate.

In our case, the error made at the time-point 7 was because there was a very bright cell that appeared in the neighborhood. Even if it was farther than the right candidate, it was within the <tt style="font-size: 1.25em;">Distance tolerance</tt> bound and was deemed as a successor. On this demo dataset, the temporal resolution is excellent, and the cells do not move much from one frame to another. A way to prevent the error we had would be to restrict the <tt style="font-size: 1.25em;">Distance tolerance</tt> to for instance 1.2 radius.
...
[[Image:bdv-MaMuTRecordMaxProjection.png]]
...
The dialog that pops let you choose first the source that will be resliced, in the <tt style="font-size: 1.25em;">Target source</tt> menu. Here we picked the first one.  There is an important gotcha with the source selection and viewer orientation. The capture always uses the source intrinsic orientation. It will always capture Z-planes of the chosen source, irrespective of the view orientation in the MaMuT viewer. So if you capture the same track with two different sources, you might have very different orientation. The <tt style="font-size: 1.25em;">Image size</tt> field determines the size of the data exported, in units of the radius of the first select spot. Finally, the <tt style="font-size: 1.25em;">Dimensionality</tt> radio button lets you chose between exporting a 3D volume or just a single slice centered on the spot. In the example above, we generated the 3D volume following the track 'A', and on the left image, generated its maximal intensity projection.
...
| style="padding: 5px;" | <tt style="font-size: 1.25em;">Quality</tt>
| style="padding: 5px;" | The quality of the spot detection. This feature is normally created by spot detectors (such as the one used in the semi-automatic tracking), and relates to how likely the spot is to be relevant. By convention it is a positive number, and high values indicate good detection. Spot created manually have all a quality value equal to -1. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">X</tt>, <tt style="font-size: 1.25em;">Y</tt> & <tt style="font-size: 1.25em;">Z</tt>
| style="padding: 5px;" | The spot location in the global common coordinate system. Units are whatever physical units used when creating the BigDataViewer file.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">T</tt>
| style="padding: 5px;" | The spot time in physical unit. Units are whatever time units used when creating the BigDataViewer file.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Frame</tt>
| style="padding: 5px;" | The time-point in which the spot appears.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Radius</tt>
| style="padding: 5px;" | The spot radius, in physical units.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Source ID</tt>
| style="padding: 5px;" | The index of the source view in which the spot was created. For instance, if you created the spot with the first source active, then its source ID is 0.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Cell division time</tt>
| style="padding: 5px;" | This is a special feature made for cell lineaging. It is expressed in time units how long a the cell tracked by this spot takes to divide. The time is measured from the mother division to the daughter division, so the cell division time value will be the same for all the spots of a branch in a track. The branches that do not start with a cell division or do not finish by a cell division are excluded from calculation, and the value of spots that belong to such branches defaults to <tt style="font-size: 1.25em;">NaN</tt>. This is why if you use this feature in with our demo dataset to color spots, they will all be dark gray, as none of our tracks follow a full cell cycle.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Manual spot color</tt>
| style="padding: 5px;" | This is a special feature, that is not automatically calculated. You can assign manually a color to a set of spots. To do so, select some spots and in TrackScheme, right-click to show the pop-up menu. The menu item <tt style="font-size: 1.25em;">Manual color for X spots</tt> will let you set the selected spot color. This color will then be used in the views if you select the <tt style="font-size: 1.25em;">Manual color</tt> item in the '''Set color by''' menu in the spot panel of the main GUI window. Manual colors are saved to and retrieved from a MaMuT file.
|-
...
| style="padding: 5px;" | The cost associated to the link. This value is normally set by particle-linking algorithms, that can create links between spots following minimization of global cost. Links created manually get a default value of -1. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Velocity</tt>
| style="padding: 5px;" | The velocity calculated between the source and target spots, in physical units. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Displacement</tt>
| style="padding: 5px;" | The displacement calculated between the source and target spots, in physical units. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Manual edge color</tt>
| style="padding: 5px;" | Life for manual spot color, but for links.
|-
...
{| border="1" style="border-collapse:collapse;"
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Track index</tt> & <tt style="font-size: 1.25em;">Track ID</tt>
| style="padding: 5px;" | The track index and ID. Track index runs from 0 to the number of tracks. Track IDs are unique numbers generated for every new track.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Duration of track</tt>
| style="padding: 5px;" | The track duration in time units. Track duration is calculated from the earliest to the latest spot in time in the track. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Track start</tt> & <tt style="font-size: 1.25em;">Track stop</tt>
| style="padding: 5px;" | The track start and stop times. They are measured at the earliest and latest spots in the track. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Track displacement</tt>
| style="padding: 5px;" | The track displacement in physical units, calculated from the earliest to the latest spot in time in the track. Careful, this is not the maximal displacement in the track, just the distance between the first and last spot. 
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Number of spots in track</tt>
| style="padding: 5px;" | The number of spots in the track.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Number of gaps</tt>
| style="padding: 5px;" | The number of gap events in the track. A gap happens when a link is made between two spots separated by more than one frame.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Longest gap</tt>
| style="padding: 5px;" | The size of the largest gap in the track, in number of frames.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Number of split events</tt>
| style="padding: 5px;" | The number of split events in the track. A split event happens when a spot is the source to more than one link (but the target of at most one link). For instance, a cell division gives rise to a split event with two outgoing links.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Number of merge events</tt>
| style="padding: 5px;" | The number of merge events in the track. A merge event happens when a spot is the target to more than one link (but the source to at most one link). When several cell merges into one gives rise to a merge event.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Complex points</tt>
| style="padding: 5px;" | The number of complex points in the track. A complex point is a spot which the source to more than one link and the target of more than one link.
|-
| style="padding: 5px;" |  <tt style="font-size: 1.25em;">Mean cell division time</tt> & <tt style="font-size: 1.25em;">Std cell division time</tt>
| style="padding: 5px;" | The mean and standard deviation values of the <tt style="font-size: 1.25em;">Cell division time</tt> spot feature described above, averaged across track branches, and excluding undefined branch values.
|-
...
=== The <tt style="font-size: 1.25em;">mamut.properties</tt> file. === 
...
It follows the syntax <tt style="font-size: 1.25em;">key=command</tt>, one per line. Modifier keys such as {{key press|Control}} and {{key press|Shift}} are specified by prepending the key with their name, separated by a space escaped with a backslash '\'. Spaces in commands do not need to be escaped. The dash # character at the beginning of a line is used to insert comments. 
...
{| border="1" style="border-collapse:collapse;"
| style="padding: 5px;" | <tt style="font-size: 1.25em;">add spot</tt>
...
| style="padding: 5px;" | Delete the spot at mouse location.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">increase spot radius</tt>
| style="padding: 5px;" | Increase the spot radius at mouse location by 10%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">increase spot radius a lot</tt>
| style="padding: 5px;" | Increase the spot radius at mouse location by 100%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">increase spot radius a bit</tt>
| style="padding: 5px;" | Increase the spot radius at mouse location by 1%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">decrease spot radius</tt>
| style="padding: 5px;" | Decrease the spot radius at mouse location by 9.10%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">decrease spot radius a lot</tt>
| style="padding: 5px;" | Decrease the spot radius at mouse location by 50%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">decrease spot radius a bit</tt>
| style="padding: 5px;" | Decrease the spot radius at mouse location by 1%.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle link</tt>
| style="padding: 5px;" | Add / Remove a link between two spots in the selection.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle linking mode</tt>
| style="padding: 5px;" | Toggle the <tt style="font-size: 1.25em;">auto-linking</tt> mode.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">semi-auto tracking</tt>
| style="padding: 5px;" | Launch semi-automatic tracking from the cells currently selected.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">help</tt>
| style="padding: 5px;" | Display the help window.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">brightness settings</tt>
| style="padding: 5px;" | Toggle the brightness settings dialog.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">visibility and grouping</tt>
| style="padding: 5px;" | Toggle the visibility and grouping settings dialog.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle interpolation</tt>
| style="padding: 5px;" | Toggle interpolation method for pixel drawing between <tt style="font-size: 1.25em;">Nearest neighbor</tt> and <tt style="font-size: 1.25em;">Tri-linear interpolation</tt>.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle fused mode</tt>
| style="padding: 5px;" | Toggle fused or single-source display mode.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle grouping</tt>
| style="padding: 5px;" | Toggle grouping mode.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">toggle source visibility X</tt>
| style="padding: 5px;" | Replace X by a digit from 0 to 9. Toggle the visibility of the source X.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">align XY plane</tt>
| style="padding: 5px;" | Change the current view to align it with the XY planes of the source currently active.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">align XZ plane</tt>
| style="padding: 5px;" | Change the current view to align it with the XZ planes of the source currently active.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">align ZY plane</tt>
| style="padding: 5px;" | Change the current view to align it with the ZY planes of the source currently active.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">next timepoint</tt>
| style="padding: 5px;" | Move to the next time-point.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">previous timepoint</tt>
| style="padding: 5px;" | Move to the previous time-point.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">step time forward</tt>
| style="padding: 5px;" | Jump to the next time step.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">step time backward</tt>
| style="padding: 5px;" | Jump to the previous time step.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">set bookmark</tt>
| style="padding: 5px;" | Store a bookmark for the current view.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">go to bookmark</tt>
| style="padding: 5px;" | Move the view to the specified bookmark.
|-
| style="padding: 5px;" | <tt style="font-size: 1.25em;">got to bookmark orientation</tt>
| style="padding: 5px;" | Orient the view to the specified bookmark orientation.
...
[[Image:bdv-MaMuTKeyboardLayout.png]]
...
{| border="1" style="border-collapse:collapse;"
| style="padding: 5px;" |  {{key press|F2}} 
| style="padding: 5px;" | Edit current spot name.
|-
| style="padding: 5px;" |  {{key press|Delete}} 
| style="padding: 5px;" | Delete the current selection. 
|-
| style="padding: 5px;" |  {{key press|Home}}
| style="padding: 5px;" | Center view on the first spot in current selection. 
|-
| style="padding: 5px;" |  {{key press|End}}
...
|-
| style="padding: 5px;" |  {{key press|-}} 
| style="padding: 5px;" | Zoom out. 
|-
| style="padding: 5px;" |  {{key press|Shift|equals}} 
| style="padding: 5px;" | Reset zoom. 
|-
| style="padding: 5px;" |  {{key press|1}} {{key press|2}} ... {{key press|9}} on the keypad 
| style="padding: 5px;" | Pan the view. 
|-
| style="padding: 5px;" |  {{key press|Control|A}}
| style="padding: 5px;" | Select all. 
|-
| style="padding: 5px;" |  {{key press|Control|Shift|A}}
...
|-
| style="padding: 5px;" |  {{key press|Page up}} / {{key press|Page down}} 
| style="padding: 5px;" | Jump to the the previous / next track. 
|-
| style="padding: 5px;" |  {{key press|Shift|Mousedrag}} 
| style="padding: 5px;" | Pan the view. 
|-
| style="padding: 5px;" |  {{key press|Shift|Mousewheel}} 
| style="padding: 5px;" | Zoom in / out. 
|-
