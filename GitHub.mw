{{DevelopMenu | tools}}In the [[ImageJ]] and related [[SciJava]] projects, we make extensive use of [https://github.com/ GitHub], a web site that hosts [[open source]] projects for free.
...
After pointing the web browser to a specific repository, typing the {{Key|T}} key will let you type out parts of the file name and hit {{Key|Enter}} when the file in question is at the top of the list (you can also navigate the list using the cursor keys).

== Linking to specific lines ==

Hitting the {{Key|L}} key lets you jump to a specific line (and modifies the URL which you can then send around to refer to that line).

After one line is selected already, you can select a line ''range'' by {{Key|Shift}}-clicking on the line number of the other end of the line range.
...
* On the command-line, Git [https://github.com/git/git/blob/v2.1.3/Documentation/diff-options.txt#L466-L470 understands the <code>-w</code> flag] to ignore whitespace changes.
* GitHub provides the same functionality, too: just append <code>?w=1</code> to the URL (or <code>&w=1</code> if there are already GET parameters).

= Editing files =

If you want to modify some file's contents and you are certain that the changes do not need to be tested locally, you can hit the <code>Edit</code> button on the upper right corner after navigating to the file in question (in case you don't have ''Push'' permission on the repository in question, this will ''fork'' the project at the same time). This will let you edit the file online and commit the changes after providing a commit message (you should still try to write a [[Coding_style#SCM_history|meaningful commit message]], of course).
