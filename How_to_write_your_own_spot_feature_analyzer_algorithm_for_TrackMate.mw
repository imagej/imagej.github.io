...
You must first create a {{GitHub|org=fiji|repo=TrackMate|source=fiji/plugin/trackmate/features/spot/SpotAnalyzerFactory.java|label=SpotAnalyzerFactory}}. This factory will be in charge of the TrackMate integration. The interface extends both the {{GitHub|org=fiji|repo=TrackMate|source=fiji/plugin/trackmate/TrackMateModule.java|label=TrackMateModule}} and the {{GitHub|org=fiji|repo=TrackMate|source=fiji/plugin/trackmate/features/FeatureAnalyzer.java|label=FeatureAnalyzer}} interfaces. It is the class you will need to annotate with a [[SciJava]] annotation for automatic discovery.

But it is also in charge of instantiating {{GitHub|org=fiji|repo=TrackMate|source=fiji/plugin/trackmate/features/spot/SpotAnalyzer.java|label=SpotAnalyzer}}s. As you can see, this interface just extends ImgLib2 {{GitHub|repo=imglib|path=algorithms/core/src/main/java/net/imglib2/algorithm/Algorithm.java|label=Algorithm}}, so all parameters will have to be passed in the constructor, which can be what you want thanks to the factory. We do not need a return value method, because results are stored directly inside the spot objects. But we will see this later.
...
Right now, let's focus on the factory class itself. There is not much to say: its content resembles all the feature analyzers we saw so far. So I am going to skip over the details and point you to the full source code {{GitHub|org=fiji|repo=TrackMate-examples|source=plugin/trackmate/examples/spotanalyzer/RelativeIntensitySpotAnalyzerFactory.java|label=here}}.

The one interesting part is the factory method in charge of instantiating the <code>SpotAnalyzer</code>:
...
The code for the whole class is {{GitHub|org=fiji|repo=TrackMate-examples|source=plugin/trackmate/examples/spotanalyzer/RelativeIntensitySpotAnalyzer.java|label=here}}.
...
As stated above, our new analyzer depends on some other features to compute. Therefore, the analyzers that calculate these other features need to run ''before'' our analyzer. Or else you will bet <code>NullPointerException</code>s randomly. 
...
For instance, if you check the annotation part of the spot analyzer factory, you can see that there is an extra parameter, <code>priority</code>:

<source lang="java">
@Plugin( type = SpotAnalyzerFactory.class, priority = 1d )
</source>

This priority parameter accepts a <code>double</code> as value and this value determines the order of execution. Careful, the rule is the opposite of what would make sense for a priority: 

{{ambox | text =Feature analyzers are executed in order according to '''increasing priority'''. This means that analyzers with the greatest priority are executed last. }}

By convention, if your feature analyzer depends on the features calculated by N other analyzers, you take the larger priority of these analyzers, and add 1. In our case, we depend on the {{GitHub|org=fiji|repo=TrackMate|source=fiji/plugin/trackmate/features/spot/SpotIntensityAnalyzerFactory.java|label=SpotIntensityAnalyzerFactory}}, which as a priority of 0 (the default if the parameter is unspecified). So quite logically, we set the priority of our analyzer to be 1. This ensures the proper execution order.
...
[[File:TrackMate CustomSpotAnalyzer 01.png|600px]]



{{Person|JeanYvesTinevez}} ([[User talk:JeanYvesTinevez|talk]]) 07:32, 11 March 2014 (CDT)
