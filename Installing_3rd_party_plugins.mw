...
: [[Image:Install-Plugin-DragNDrop.png|750px]]

: This will ask you for the <u>destination</u> of the plugin, just hitting ''Save'' is safe.

* Alternatively, use {{bc | Plugins | Install Plugin...}}:

: [[Image:Install-Plugin.png|550px]]

: It will ask you for the file name of the plugin, and install the given file.

* You can also copy the ''.jar'', ''.class'' or ''.java'' files (or a script in any language Fiji supports) to ''Fiji.app/plugins/'', followed by calling {{bc | Help | Refresh Menus}}.

: '''Note:''' On MacOSX, you need to {{key|Ctrl}}+{{key|click}} on the ''Fiji'' icon in the Finder and select ''Show package contents'' to see the ''plugins/'' folder.
...
where {{bc | File}} is the menu you want to add the item to -- at the moment, the
only valid choices are {{bc | Plugins}}, {{bc | Plugins | <submenu>}}, {{bc | File | Import}},
{{bc | File | Save}}, {{bc | Analyze | Tools}}, {{bc | Help | About}} and {{bc | Edit | Options}}.
