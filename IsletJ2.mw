...
IsletJ2 is a tool implemented as an ImageJ plugin that segments and reports properties of the Langerhans islets. The plugin was available already in 2018 under name [https://imagej.net/IsletJ IsletJ]. However, many changes have been implemented since then. The original version can be found [https://imagej.net/IsletJ here].
...
# a path to a directory containing images to analyze (or choose to analyze the currently active image),
...
Pretrained segmentation models can be downloaded [http://ptak.felk.cvut.cz/Medical/microscopy/IKEM/IsletJ/public/ here]. Note that future versions of IsletJ2 need not support older segmentation models. However, we will try to maintain a compatible model for each version. 
