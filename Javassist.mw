...
While Javassist allows to edit bytecode directly, its most powerful feature is that it allows you to modify methods using snippets of Java code. There are two slightly different methods to do so: the [http://www.csg.ci.i.u-tokyo.ac.jp/~chiba/javassist/html/index.html CodeConverter] and the [http://www.csg.ci.i.u-tokyo.ac.jp/~chiba/javassist/html/javassist/expr/ExprEditor.html ExprEditor] (it depends which one to use, ''CodeConverter'' is easier to use but ''ExprEditor'' is more powerful.
...
Note that the special <tt>$_ = $proceed($$)</tt> is a Javassist-specific syntax referring to the original code. See [http://www.csg.ci.i.u-tokyo.ac.jp/~chiba/javassist/tutorial/tutorial2.html#before the Javassist tutorial] for details.
...
If you want to avoid being misled by the error message (the method mentioned in the ''VerifyError'' is most likely to be <u>not</u> the offending one), and moreover want to know where in the bytecode the error happened, probably the easiest way to go forward is to use [http://commons.apache.org/bcel/ BCEL] (you can get it directly from the [http://maven.imagej.net/index.html#nexus-search;gav%7E%7Ebcel%7E%7E%7E Maven repository]), Apache's ''Byte Code Engineering Library''. For that, you have to write out .class files first:
...
More on this issue can be found [http://elliotth.blogspot.com/2008/03/generating-jvm-bytecode.html here].
...
If you are using Fiji's {{GitHub|org=fiji|repo=fiji-compat|source=fiji/JavassistHelper.java|label=JavassistHelper class}}, you can use the ''verify()'' method which does nothing else than to hand off to the ASM component's verifier. Example:
