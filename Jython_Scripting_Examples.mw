{{Notice | This page is a copy of [[Jython Scripting]]. The original wiki page is in a rewrite progress. When rewriting [[Jython Scripting]] is finished, this wiki page will be shortened to only contain Jython code examples.}}

{{Learn|scripting}}[http://www.jython.org/ Jython] is an implementation of the [https://www.python.org/ Python programming language] designed to run on the Java platform.
...
How ImageJ does it, internally, has to do with the [https://imagej.net/developer/api/ij/process/ImageStatistics.html ImageStatisics] class:
...
To calculate other parameters, specify them by bitwise-or composition (see flags in [https://imagej.net/developer/api/ij/measure/Measurements.html Measurements]):
...
To display the histogram window ourselves, we may use the [https://imagej.net/developer/api/ij/gui/HistogramWindow.html HistogramWindow] class:
...
The [https://fiji.sc/javadoc/ij/CompositeImage.html CompositeImage] is a stack whose slices are interpreted as belonging to specific color channels, Z slices and time frames. To find out which slice corresponds to what, use the <i>getStackIndex</i> method of the [https://fiji.sc/javadoc/ij/ImagePlus.html ImagePlus], which translates between color channels, z slices and time frames to the slice index in the underlying [https://fiji.sc/javadoc/ij/ImageStack.html ImageStack].
...
Will work with regular stacks and with any kind of complex stack like a composite image or a 4d volume. Keep in mind that all stack types in ImageJ consists of a sequence of 2d images, each editable with an [https://imagej.net/developer/api/ij/process/ImageProcessor.html ImageProcessor] obtained from the [https://imagej.net/developer/api/ij/ImageStack.html ImageStack] that one can get from the [https://imagej.net/developer/api/ij/ImagePlus.html ImagePlus]. (The [https://imagej.net/developer/api/ij/ImagePlus.html ImagePlus] being what the opener or the [https://imagej.net/developer/api/ij/WindowManager.html WindowManager] provides.)
...
Then open the [[Fiji Updater]], push the "Manage update sites" at the bottom left of the dialog, and install the [https://fiji.sc/~schindelin/ffmpeg-plugins/ FFMPEG] plugin by {{Person|Schindelin}} by ticking its checkbox, as [[How_to_follow_a_3rd_party_update_site|explained in more detail here]].
...
To populate an update site, the updater let the possibility to directly upload jython scripts as .py (or $py.class for the one that resides in the <code>Jars/Lib</code> folder).  
...
First make sure that you have installed Maven and a java jdk and add the path to those 2 in your environment variables (see [https://maven.apache.org/install.html Maven installation]). 

Then download a zip of this [https://github.com/m-entrup/imagej-jython-package Github repository]. And follow the guidelines : 
* place your script in a subfolder of <code>Ressources</code>
* eventually modify the pom file according to your dependencies (see [https://imagej.net/Building_a_POM Building a POM] 
* then open a command line in the folder and type <code>mvn package</code>.    
Eventually you can also type <code>mvn -Dimagej.app.directory=/path/to/your/Fiji.app -Dimagej.deleteOtherVersions=older</code> to have the dependencies automatically coped to an ImageJ installation : handy to test your package on a fresh installation.   

NB : This latter command with the dependencies was [http://forum.imagej.net/t/use-opencv-within-jython-macro/10320/18?u=lthomas reported not to function with windows Powershell], although the normal packaging works. Use a different command line tool in this case.
