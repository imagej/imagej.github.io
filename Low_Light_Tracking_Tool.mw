...
| name                   = Blob Tracking
| software               = Fiji
| author                 = {{Person|Alex-krull}},  {{Person|Damienrj}}
| maintainer             = {{Person|Alex-krull}},  {{Person|Damienrj}}
...
"A Divide and Conquer Strategy for the Maximum Likelihood localization of Low Intensity Objects" by Alexander Krull et al, 2013 Optics Express, Vol. 22, Issue 1, pp. 210-228 (2014).
...
To begin use Fiji to load the data to be used for tracking. Use whichever method you prefer in Fiji. The LLTT-plugin will work with 8-bit, 16-bit and 32-bit-float stacks. Moreover you can use 3D and multichannel hyperstacks.
If you want to try the LLTT-plugin but do not have the right data at hand, you can download a file with sample data here:
[[File:LLTT_BeadsExampleData.zip]].
...
The '''EMCCD gain''' and  '''Electrons per A/D count''' are required for tracking with the [[Low_Light_Tracking_Tool#EMCCD-GaussianML | EMCCD-GaussianML method]]. The [[Low_Light_Tracking_Tool#GaussianML | GaussianML method]] uses these values to correct the measured flux of the object and background.

=The User Interface=
==The different windows==
When the plugin starts on a 2D image stack you have three windows and the control panel. The main data window lets you move through the slices like normal. On the sides you see the two kymographs, which makes it easier to see movement over time. 
...
If you close the control panel, this will terminate the program. Don't forget to [[Low_Light_Tracking_Tool#Saving_and_Loading | '''Save''']] your results before.
...
There are three ways of navigating through your stack or hyper stack:
...
To start tracking select at least one trace and click on the '''Start tracking''' button or hit the hot-key '''G'''. 
The software will start at the frame you are currently looking at and continue with the following frames until the end of the movie  or until you hit '''G''' again or click on '''Stop tracking'''

Objects that are very close to each other should generally be tracked jointly by selecting them both before starting to track.
...
When you start the plugin again with the same data file your results will be automatically loaded. 
...
The '''ΔZ ratio''' is the slice thickness divided by the pixel width. This value is only used for 3D data.
...
The algorithm builds a [[wikipedia:Pyramid %28image processing%29|pyramid]] by repetitively smothing the image with a Gaussian kernel with the standard deviation given in the '''Smoothing σ''' field and than down-sampling it by a factor  set in the '''Downscaling''' field. The pyramid's hight (i.e. the number of repetitions) can be set in the '''iterations''' field.
...
== Colt library ==
We redistribute packages from the [http://acs.lbl.gov/software/colt/ colt] library. They have the following licenses: 
