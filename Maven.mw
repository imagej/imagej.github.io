...
* [https://maven.apache.org/what-is-maven.html What is Maven?]
* [https://maven.apache.org/guides/getting-started/maven-in-five-minutes.html Maven in 5 Minutes]}}
{{DevelopMenu | tools}}{{Minibox | logo=Maven-icon.png | size=51px | blurb=<br>'''Apache Maven''' is a [[wikipedia:Convention over configuration|convention over configuration]] build automation tool.}}[[ImageJ]], [[Fiji]] and other [[SciJava]] projects use [https://maven.apache.org/ Maven] for their project infrastructure.

Maven artifacts are published to the [[SciJava Maven repository]].
...
Out of the box, Maven will look in the so-called [https://search.maven.org/ Maven Central repository]. Some ImageJ and SciJava components are deployed there, including the [[Architecture#Maven_component_structure|pom-scijava parent POM]] which declares important metadata, such as the [[Bill of Materials]]: current artifact versions intended to work together.
...
As a rule of thumb: components [[Versioning|versioned at 0.x]] are deployed to the SciJava Maven repository, while those at 1.x or later are deployed to Maven Central.
...
In the context of [[SciJava]], you will most likely never write a ''pom.xml'' from scratch. You will rather more likely edit an existing one, possibly after having copied it. We recommend using the [https://github.com/imagej/tutorials/tree/master/maven-projects/load-and-display-dataset ImageJ "Load and Display a Dataset" tutorial] as a starting point.

= How to find a dependency's groupId/artifactId/version (GAV)? =

Most popular open source libraries upon which you might want to depend are stored in the [https://search.maven.org/ Maven Central repository]. However, the ImageJ and Fiji JARs are not yet stored there, but in the [[Architecture#Maven_repositories|SciJava Maven repository]]. Fortunately, you can search both at once, by visiting:

  https://maven.scijava.org/

For example, let's suppose you want to depend on the [http://snakeyaml.org snakeyaml] library. Typing "snakeyaml" into the search box at [https://maven.scijava.org maven.scijava.org] tells us to use a <code>groupId</code> of <code>org.yaml</code>, <code>artifactId</code> of <code>snakeyaml</code>, with available versions ranging from <code>1.4</code> to <code>1.10</code>. In the case of many results, you can click the "Drill down" link to view more details of that specific GAV combination. You can also click an entry to get a formatted <code>dependency</code> block for direct copy-pasting into your POM.
...
* If the dependency is narrower in scope, you could [[Mailing Lists|contact the ImageJ & Fiji maintainers]] to get your needed dependency added to the SciJava Maven repository. Note that you will then be responsible for distributing the dependency with your code—so ensure it is [[Licensing|licensed appropriately]].

Finally, for local testing you can [https://maven.apache.org/guides/mini/guide-3rd-party-jars-local.html install the dependency into your local Maven repository cache yourself]. The command is <code>mvn install:install-file</code>. For example, if you have a library <code>foo.jar</code> to install, you could run:
...
When in doubt, [[Help|contact the community]] with your questions.

= Further reading =

* Our very own [[Maven - Frequently Asked Questions|Maven FAQ]]
* [https://maven.apache.org/guides/getting-started/ Maven's Getting Started]
