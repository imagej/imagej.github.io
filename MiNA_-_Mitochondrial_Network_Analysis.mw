...
|-
| High contrast
| <b>Only required when using ridge detection!</b> This defines the High Contrast value for the ridge detection plugin. 
|-
| Low contrast
| <b>Only required when using ridge detection!</b> This defines the Low Contrast value for the ridge detection plugin. 
|-
| Line width
| <b>Only required when using ridge detection!</b> The expected width of the line like mitochondrial features. 
|-
| Line length
| <b>Only required when using ridge detection!</b> The minimum line length to be included in the analysis. Setting this above 0 can aid in removing spurious small lines.
|-
| User comment
...
| The mean of the sum of the lengths of branches for each independent structure (as represented by the morphological/topological skeleton). This is the sum of all branch lengths divided by the number of independent skeletons.
|-
| summed branch lengths median
| The median of the sum of the lengths of branches for each independent structure (as represented by the morphological/topological skeleton).
|-
| summed branch lengths stdevp
| The standard deviation of the sum of the lengths of branches for each independent structure (as represented by the morphological/topological skeleton).
|-
...
<ol>
<li>Open an image. MiNA expects an 8-bit or 16-bit single channel image that can be 2D or 3D (with limited options).</li>
<li>Select a single cell using the rectangular ROI tool. You may make a copy of the cropped region if you wish.</li>
<li>Run the script by navigating to Plugins -> StuartLab -> MiNA Scripts -> MiNA Analyze Morphology. A user dialog will pop up. Fill in the parameters as desired and select OK to run the analysis.</li>
<li>An overlay as shown in an above figure will be generated for you to visually inspect the faithfulness of the analysis. The magenta region is the binarized signal used for calculating the area or volume. The green lines are the morphological skeleton.</li>
<li>To save a copy of the image with overlays, save the image as a PNG or flatten the image and save it in whatever format you wish.</li>
</ol>

<h1>Reporting Bugs and Suggestions</h1>
The source code for MiNA is available on [https://github.com/StuartLab/MiNA Github]. Software bugs can be reported using the [https://github.com/StuartLab/MiNA/issues issue tracker]. If you implement your own feature, fix a bug, or extend the tool and want to contribute it back to the project you can always make a [https://github.com/ScienceToolkit/MiNA/pulls pull request].

When reporting bugs, please provide the steps required to reproduce the issue. Before reporting, try running the updater (Help &rarr; Update...) and, if that fails, reinstalling Fiji and adding the update sites as described in the installation section. Please only report software bugs with with the [https://github.com/StuartLab/MiNA/issues issue tracker]. Also, be sure to check the FAQ (frequently asked questions) in the [https://github.com/StuartLab/MiNA/blob/master/README.md README.md] first. 
