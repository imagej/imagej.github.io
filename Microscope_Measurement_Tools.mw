...
| filename               = [https://github.com/demisjohn/Microscope-Measurement-Tools/releases Download from GitHub/Releases]
| source                 = {{GitHub|org=demisjohn|repo=Microscope-Measurement-Tools}}
| released               = Sept. 24<sup>th</sup>, 2015
| latest version         = 2.3
| status                 = beta
...
4. Select the {{bc | Analyze | Set Scale}} menu item.
...
: This settings file is an actual python script, so you can edit the lists there using normal Python syntax. eg. functions like <code>myList.append( [ ] )  </code >   or   <code >  myList = ['cm'] * 5</code > etc. will work.
...
11. Run {{bc | Analyze | Microscope Measurement Tools | Choose Microscope Calibration}} - the pop-up window shows the new names and calibration values you set in your user-settings file.
...
2. Select {{bc | Analyze | Microscope Measurement Tools | Draw Annotation - Line}}.
...
== Author(s) ==
This simple python (jython) plugin was created by {{Person|Demis}}, 2015.
