...
* open the [[Media:NucleiDAPIconfocal.png|sample image of touching DAPI stained cell nuclei from a confocal laser scanning microscope.]]

[[Image:NucleiDAPIconfocal.png | 250px]]

* Run a [http://imagejdocu.tudor.lu/doku.php?id=gui:process:filters Gaussian Blur filter] on the image to blur out the "speckle", actually Poisson distributed, statistical "photon [[wikipedia:Shot noise|shot noise]]", and also to smooth out the inhomogeneity of the nuclear staining. We will use a large sigma value of 3 for this task.  A value of sigma too small will mean that the segmentation will be disturbed by the noise and staining pattern. Too high a sigma value, and the objects will be too blurred, making it harder to find their edges precisely and separate them later.  Run menu command: Process - Filters - Gaussian Blur, with a sigma value of 3 pixels. You can preview other values to see how they look also. You should get an image that looks like this:

[[Image:NucleiDAPIconfocalGauss3pxSigma.png | 250px]]

=== Pixel Intensity Threshold  - find the foreground areas ===

* Next we need to separate the objects from the background using pixel intensity thresholding. Fiji has a number of built in [[Auto_Threshold|Automatic Thresholding]] methods that try to distinguish the background from the foreground. In this case the default method works pretty well,  but you can see there is a long list of methods, which give slightly different threshold results for this image.  You can play with different methods if you like. You might get a different answer in the end!
...
[http://imagejdocu.tudor.lu/doku.php?id=gui:analyze:analyze_particles documentation for analyze particles.]
...
[[Image:NulceiDAPIconfocalSegmentationResults.png | 700px]]
