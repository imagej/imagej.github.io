{{ComponentStats:sc.fiji:PIV_analyser}}This plugin calculates the optic flow for each pair of images made with the given stack. 

== Principle ==

The plugin works using the PIV method, which is the most basic technique
for optic flow. This technique, mainly used in
acoustics or in fluids mechanics, enables the measurements of a velocity
field in one plane, using imaging and image analysis<ref>
{{Citation
  | last = Raffael
  | first = M.
  | last2 = Willert
...
direction and in what amount a
...
and imaged at two very close instants. The cross-correlation between parts of
...
situations and after photo-ablation<ref> {{citation
  | last = Supatto
  | first = W.
  | last2 =  Débarre
  | first2 = D.
...
<li>quantifying blood flow during cardiogenesis in zebrafish embryo<ref>{{citation
 | last = Hove
 | first = J.R.
 | last2 Köster
 | first2 = R.W.
...
educational purpose. It has a very pedestrian approach,
...
approximately 1 second. I recommend downsampling the images, this would
...
The image to the right is made of 2000 small gaussian blobs (sigma=1), spawned randomly 
(you can get the original file [https://fiji.sc/tinevez/testpiv3.tif here]). The 
second frame was obtained by adding a shift to each blob, given by:

* &Delta;Y = &alpha; &times; (X-Xm)
* &Delta;X = &alpha; &times; (Y-Ym)

with &alpha;=25 and Xm and Ym half of the image width. This effectively generate a sort of 
circular twist to the image. 

Below is what the PIV analysis found, with a window of 64x64 (the max displacement the 
algorithm can find is given by half of the window size, so we had to go for at least 50x50), 
no masking and no interpolation. The color encode flow direction and magnitude, using the 
reference that can be seen in the color wheel.

<gallery>
Image:Testpiv3.gif|The original 2 frame sequence
Image:Testpiv3-fd.png|The resulting flow encoded in color
Image:Color coded orientation.png|Color coded orientation and magnitude
</gallery>

Here are two other examples, taken from the [http://www.math.uio.no/~jks/matpiv/ MatPIV page] of J. Kristian Sveen. The first one depict a surface wave in water:

<gallery>
Image:Surface wave.gif|Consecutive images of small particles in water during wave propagation
Image:Surface wave-fd.png|Result from analysis
</gallery>

The second one depict the internal wave. Notice the shearing in the middle of the frame. 

<gallery>
Image:Internal wave.gif|Consecutive images of small particles in water during wave propagation
Image:Internal wave-fd.png|Result from analysis
</gallery>
...
The plugin is part of the Fiji distribution. You can find it in the {{bc | Plugins | Optic Flow}} menu.

== Usage ==

[[Image:PIV_analyser_window.png]]
...
* '''Do masking''': is set, results will be masked using the peak height value. For peak height values lower than '''Masking level''' &times; <code>max(pkh)</code>, the flow will be set to 0.
