...
In Fiji, you can adjust the dynamic range of an image by calling {{bc | Image | Adjust | Brightness & Contrast}}.  However, this only lets you choose a linear mapping between pixel intensity and lookup table.  This project aims to provide non-linear controls, such as piecewise linear functions, gamma curves, splines, etc
...
'''Language:''' Java.<br />
'''Mentor:''' {{Person|Iarganda}}, {{Person|Albertcardona}}<br />
...
So far, we have [https://fiji.sc/cgi-bin/gitweb.cgi?p=fiji.git;a=shortlog;h=refs/heads/jmathlib a branch which adds rudimentary JMathLib bindings to Fiji's scripting interface] and [https://fiji.sc/JMathLib/.git a Git-SVN mirror of the JMathLib source code repository] with a special ''fiji'' branch. The idea is to work on this branch to adjust JMathLib in certain ways to support this project, and once that is done, contribute the changes back to the JMathLib project.

The proof-of-concept version of [https://fiji.sc/cgi-bin/gitweb.cgi?p=fiji.git;a=blob;f=src-plugins/JMathLib_Interpreter/Refresh_JMathLib_Scripts.java;hb=refs/heads/jmathlib the Refresh_JMathLib_Scripts class] illustrates how the [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/interpreter/Interpreter.java;hb=refs/heads/fiji JMathLib Interpreter] needs to be instantiated and called.

The following issues need to be tackled in the JMathLib source code:

* In embedded (non-standalone) mode, the [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/functions/FunctionManager.java;hb=refs/heads/fiji#l45 FunctionManager installs] a [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/functions/WebFunctionLoader.java;hb=refs/heads/fiji WebFunctionLoader] which disallows new functions to be added. Either the WebFunctionLoader needs to be taught to allow (maybe optionally) new functions to be added, or there needs to be another mode in which JMathLib is embedded, but still allows new functions to be defined.

* In embedded mode, JMathLib <u>must not</u> [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/toolbox/jmathlib/system/quit.java;hb=refs/heads/fiji call ''System.exit()'']. 

* [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/functions/FunctionManager.java;hb=refs/heads/fiji#l45 Only in standalone mode] are the available functions discovered at runtime rather than [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/functions/WebFunctionLoader.java;hb=refs/heads/fiji#l40 read from an embedded file]. This file should not be necessary. JMathLib should detect whether it is bundled in a .jar or not, and use [http://download-llnw.oracle.com/javase/1.5.0/docs/api/java/util/jar/JarInputStream.html a JarInputStream] or traverse the directory hierarchy otherwise. Probably the best place to do this is to teach the [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/core/functions/FileFunctionLoader.java;hb=refs/heads/fiji FileFunctionLoader] to accept a [http://download-llnw.oracle.com/javase/1.5.0/docs/api/java/net/URL.html URL] instead of a [http://download-llnw.oracle.com/javase/1.5.0/docs/api/java/io/File.html File], too.

* JMathLib supports Java via a [https://fiji.sc/cgi-bin/gitweb.cgi?p=JMathLib/.git;a=blob;f=src/jmathlib/plugins/dynjava/JavaPlugin.java;hb=refs/heads/fiji non-standard mechanism] based on [http://old.koalateam.com/djava/ DynamicJava]. This is incompatible with [[MATLAB]], so there needs to be native support using [http://download-llnw.oracle.com/javase/1.5.0/docs/api/java/lang/reflect/package-summary.html reflection] to support the method to instantiate Java objects mentioned above.
...
== Add a [[wikipedia:Read-eval-print loop|"REPL" (Read-Eval-Print-Loop)]] to the [[Script Editor]] ==
...
ImageFlow has its own {{GitHub|org=Dahie|repo=imageflow|label=Git repository}} ([https://fiji.sc/cgi-bin/gitweb.cgi?p=imageflow.git;a=summary our mirror]).
...
For now, {{bc | File | Save As}} always saves the result as a ''.tiff'' file, even if the user specified a file name ending in, say, ''.png''.
...
Note: this is more or less implemented in the Auto_Threshold and Auto_Local_Threshold plugins.--{{Person|Landini}} 14:47, 29 November 2009 (CET)
