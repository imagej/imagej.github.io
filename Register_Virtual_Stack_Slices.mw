...
|style="vertical-align:top" |[[Image:Rvs_scheme.png|380px|Register Virtual Stack scheme - All images are transformed to match the reference]]
|}
...
[[Image:Rvs_main_window.png|400px|thumb|Register Virtual Stack main window]]
...
*'''Save transforms'''. Option to save the resulting transforms into files, that can be loaded using [[Transform Virtual Stack Slices]].
</ul>   
</p>
<p><b>Step 3</b>: choose the '''reference''' image (only if the "Shrinkage constraint" checkbox was not marked):<br />
<ul>
...
=== Shrinkage constrain ===
[[Image:RVS_shrink_option.png|thumb|400px|Shrinkage constrain checkbox]]
Since the 09/13/2009 version and thanks to [http://hebb.mit.edu/ Daniel Berger], Register Virtual Stack Slices has the option of constraining the registration to avoid section shrinking for large sets of images.
...
[[Image:RVS_regularization_params.png|thumb|200px| Shrinkage regularization parameters ]]
* the [[wikipedia:Shear mapping|shear]],
* the [[wikipedia:Scaling (geometry)|scaling]],
* and the [[wikipedia:Isotropy|isotropy]] or aspect ratio.
...
In the case of the "elastic" model, only a rigid registration will be performed. If you need to regularize the elastic transformation between the sections, you better do it through the bUnwarpJ paramters (see [[Register_Virtual_Stack_Slices#Notes | Notes]] below).
...
=== Save Transforms ===
Since the 09/24/2009 version, the plugin has the option to store the resulting transforms into files. This way, the results can be reproduce later on the same images or on a difference sequence by using the plugin [[Transform Virtual Stack Slices]].
...
The transforms are saved as '''.XML''' files, following [[TrakEM2]] format.
...
<tr><td>[[Image:RVS_Tem_example.gif|input data]]</td><td>[[Image:RVS_Registered_tem.gif|output data]]</td></tr>
</table>
...
== See also ==
* [[Transform Virtual Stack Slices]], to see how to load the saved transforms.
* [https://fiji.sc/cgi-bin/gitweb.cgi?p=mpicbg.git;a=blob;f=mpicbg/imagefeatures/FloatArray2DSIFT.java;hb=HEAD FloatArray2DSIFT.java] which contains the subclass Param which is the 'sift' member of the Register_Virtual_Stack_Slices.Param subclass.
* {{GitHub|org=fiji|repo=register_virtual_stack_slices|source=register_virtual_stack/Register_Virtual_Stack_MT.java|label=Register_Virtual_Stack_MT.java}}, including the Param subclass.
* [[Elastic_Alignment_and_Montage|Elastic Alignment and Montage plugin]], for elastic registration/montage of large datasets using spring meshes (local deformations).
