{{Warning | The SLIM Curve plugin for ImageJ has been discontinued in favor of [[FLIMJ]].}}
...
| status                 = discontinued in favor of [[FLIMJ]]
| website                = https://slim-curve.github.io/
...
The SLIM Curve plugin is available from the "SLIM Curve" [[update site]].

Once you have installed the SLIM Curve plugin it becomes available on the menu under {{bc | Analyze | Lifetime | SLIM Curve}}.
...
[[Image:Excitation.png|thumb|760px|Controlling the default excitation]]
...
All the operations in SLIM Plugin operation are completely compatible with the popular [https://imagej.net/developer/macro/macros.html ImageJ macro language]. Each of the button, selection, choice of algorithm, binning, noise model, default excitation selection with custom start-end time is completely macro record-able. Below is a typical macro recording for a typical usage where the user sets the algorithm, noise model, changes transient time, loads default excitation, sets the chi2 target, fixes A value for fitting and then starts fitting. 
[[Image:Screenshot slim final.png|thumb|500px|Example of macro recording SLIM Curve]]
...
If you have questions, please ask on the [https://forum.image.sc/ Image.sc Forum] with the [https://forum.image.sc/tags/slim-curve <code>slim-curve</code> tag].
