...
A list of possible data types and the corresponding widgets is provided below.<br>  
The optional style argument defines how the widget are rendered in the input window.<br>
...
Integer and flaot can have the optional argument ''min'', ''max'' and ''stepSize'' value (default 1) as well as a default value indicated by ''value''.<br>
...
[[File:ScriptParameters-IntegerStyles.JPG|450px]]
...
Any parameter can be turned into a multiple-choice selector by adding a <code>choices={...}</code> property. <br>
...
The single <code>File</code> parameter support the styles "''file''", "''directory''", "''open''", "''save''". 
...
The <code>File[]</code> parameter supports the styles "''files''", "''directories''", "''both''".
