...
* Model ([https://fiji.sc/javadoc/fiji/plugin/trackmate/Model.html <code>fiji.plugin.trackmate.Model</code>]) is the class in charge of <u>storing the data</u>. It cannot do anything to create it. It can help you follow manual modifications you would made in the manual editing mode, interrogate it, ... but it is conceptually just a data recipient. 

* Settings ([https://fiji.sc/javadoc/fiji/plugin/trackmate/Settings.html <code>fiji.plugin.trackmate.Settings</code>]) is the class storing the fields that will configure TrackMate and pilot how the data is created. This is where you specify what is the source image, what are the detector and tracking algorithms to use, what are the filters to use, etc...
...
There are some gotchas: some feature analyzers require other numerical features to be already calculated. If something does not work, it is a good idea to directly check the preamble in the source code of the analyzers ([https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/ TrackMate feature logic]).
...
Contributed by {{Person|Eglinger}} during a NEUBIAS course. Quoting from Jan:
...
{{GitHubEmbed|org=fiji|repo=TrackMate|source=../../../scripts/Run_TrackMate_Headless.groovy}}
...
<source lang="python"># @ImagePlus imp
...
[[File:TrackMateScriptBeforeCapture.png|600px]]
