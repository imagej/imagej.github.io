...
When starting the plugin from the {{bc|Plugins|Segmentation|Simple Neurite Tracer}} menu option, you should be presented with a dialog like this (outdated snapshot):
[[Image:Snt-initial-dialog.png|center]]
...
[[Image:Snt-tracer-starting.png|600px|center]]

The top of the left hand dialog box should always give you an idea about the next steps in basic tracing of paths.  However, the first step when you load a new image should probably be to enable the option for using Hessian-based analysis of the image - this is done via a checkbox a little further down the dialog box. It will take some time to calculate the Gaussian convolution of the image once you select this option, but this only needs to be done once per image, so it's best to do it at the start. There is more on the use of this option in the [[#Miscellaneous_Tips|Tips]] section below.

''At this point, you may wish to switch to some shorter [[Simple Neurite Tracer: Step-By-Step Instructions|step-by-step]] instructions - some people have suggested that the description below is a bit verbose.  Alternatively, carry on below...''

Now, as the instructions suggest, your next step should be to click on the point in the image stack where you want to start the path. To navigate in the stack you would adjust the slices scroll bar or used the {{key|<}} and {{key|>}} keys as usual. The red cross-hairs indicate the corresponding position in the three different views of the stack, but the slice positions will not be synchronized between the three windows unless you hold down {{key|Shift}} while moving the mouse or activate the ''Enable cursor snapping'' option (window synchronization is particularly useful if you're creating a [[#Branching and Joining Paths|branching branch]] at the beginning or end of a path).

[[Image:Snt-temporary-path.png|frame|right|A temporary path, yet to be confirmed.]]
[[Image:Snt-searching-for-path.png|frame|right|The progress of the search for a temporary path.]] [[Image:Snt-confirmed-path.png|frame|right|The confirmed part of the current path.]]
...
While the plugin is searching you can still move about in the image stack to see the progress through the depth of the stack as well. If it seems to be taking a very long time to find a path between the points you should probably cancel the search (by pressing {{key|Esc}}) and try a point that's nearer, since the path searching slows down as it has explored more of the image.

Once you have a dark blue temporary path like that in the left hand image the instructions will prompt you to either keep that path segment (by pressing {{key|Y}}) or cancel it ({{key|N}}) to try picking another point (probably one nearer to the original point). If you confirm that path then the path will turn red, to indicate that this is a confirmed part of the current path. If you click further along the structure again then the dark blue colour will again show the temporary part of a path as opposed to the red confirmed part.

Once you have picked out a few more points along the path, you can complete the path by clicking "Complete Path" {{key|F}} at the top of the dialog. By default - Paths can be colorized at any later point using the drop-down menu in the ''Paths'' window - completed paths are shown in magenta and appear in the path list in the tracer's dialog box. If you select one or many paths in this list, the selected paths are shown in green instead. (You need to have particular paths selected for filling out neurons or branching and joining paths.)
[[Image:Snt-multiple-paths.png|frame|center|Several completed paths, with one selected (in green)]]

You've probably noticed that the default presentation of the paths in an image is to show the a projection of the path through the entire stack, rather than just those points of the path that are in the current slice. This can become confusing with many paths in a single image, in which case you should change the ''View Paths 2D'' option from ''Projected through all slices'' to ''Parts in nearby slices'' (shortcut: {{key|5}}).

====Branching and Joining Paths====

In this plugin each path can only be a linear succession of points in the image. However, you can indicate more complex structures by creating a path that starts on another path (branches off it) or ends on another path (joins the other path). To do this you have to select the existing path you want to branch off or join to in the path list (check that the right path is green in the image) and then hold down {{key|Ctrl}} (on Windows or Linux) or {{key|Alt}} (on MacOS) while you select the branch or join point. You almost always will want to hold down {{key|Shift}} as well as {{key|Ctrl}} / {{key|Alt}} while you're finding this image so that the crosshairs show you the exact point that you're joining to. The detailed procedure is described [[Simple_Neurite_Tracer:_Step-By-Step_Instructions#Branching:_Start_a_path_on_an_existing_path|here]].
...
[[Image:Snt-initial-filling.png|frame|center|A few seconds after selecting "Fill Out Path(s)" with 3 branched paths selected]]
...
[[Image:Snt-filling-statistics.png|frame|right|The filling-related part of the dialog (outdated screenshot).]]
...
[[Image:Snt-zoomed-filling.png|frame|center|After having zoomed in on part of the fill]]
...
[[Image:Snt-transparent-filling.png|frame|center|Having made the fill transparent]]
...
====Key Shortcuts====
Key Shortcuts and keyboard accelerators are described in [[Simple_Neurite_Tracer:_Key_Shortcuts|Key Shortcuts]].

====Miscellaneous Tips====

* The simplest way to follow a path through a stack is to select that path and hold down {{key|Ctrl}} / {{key|Alt}} and {{key|Shift}} while moving the mouse along it.
