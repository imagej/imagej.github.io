...
To process a single image, load your image ("test.lsm", say) into Fiji and select {{bc | Plugins | Process | Frangi Vesselness}}.  (There is more information about this plugin [[Frangi|on its page]].)  By way of example, let's say that you select 4 scales from half the x voxel separation to twice that value.
