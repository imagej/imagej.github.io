...
* Projects must be tracked and managed in Git, and published on [https://fiji.sc/cgi-bin/gitweb.cgi] (we will provide you with an account).
...
Whenever releasing Fiji, or after major changes, we would like to be reasonably safe that we did not break things that used to work before.  We already have a few [https://fiji.sc/cgi-bin/gitweb.cgi?p=fiji.git;a=tree;f=tests tests], but hardly enough.
...
'''Goal:''' Build a minimal text editor with a good font for coding, syntax highlighting for java and javascript (and optionally jruby, clojure and beanshell), and the ability to compile java code and parse stack traces to highlight lines that failed to compile. Building in [http://jvi.sourceforge.net jvi] a big bonus, but must enable <i>non-modal</i> editing capabilities with implicit minimal emacs keybindings (like in any default MacOSX text area: {{key|Ctrl}}+{{key|A}}, {{key|Ctrl}}+{{key|E}}, {{key|Ctrl}}+{{key|W}}, etc.)<br />
...
At the moment, there is a rudimentary {{GitHub|repo=fiji|path=src-plugins/Fiji_Updater/UpdateFiji.java|label=plugin updater}} which should be a good starting point for this project.  The list of current plugins used by that plugin can be found [http://update.fiji.sc/current.txt here].
...
* integrating JMathLib as a script language into Fiji (using the infrastructure shared by Jython, JRuby, Clojure, Javascript and BeanShell) -- I suggest having a look at {{GitHub|repo=fiji|path=src-plugins/JRuby/JRuby_Interpreter.java|label=the JRuby Interpreter}} for an example,
...
= Other links =
 * [https://imagej.net ImageJ's homepage]
