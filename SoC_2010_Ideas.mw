...
* integrating JMathLib as a script language into Fiji (using the infrastructure shared by Jython, JRuby, Clojure, Javascript and BeanShell) -- I suggest having a look at {{GitHub|repo=fiji|path=src-plugins/JRuby_Interpreter/src/main/java/JRuby/JRuby_Interpreter.java|label=the JRuby Interpreter}} for an example,
...
Also, the Updater cannot upgrade the Java Runtime as of now. There is little reason why it should not offer to download the installer directly from the [http://java.sun.com/javase/downloads/index.jsp Sun Developer Network], run the installer, and install Java&nbsp;3D on top.

'''Goal:''' Improve the Fiji Updater.<br />
'''Language:''' Java.<br />
'''Mentor:''' Johannes Schindelin (johannes.schindelin AT gmx.de)<br />
...
However, it would be nice to have an installer (similar to [http://izpack.org/ IzPack] or other Java-based installers), which just copies Fiji from where it happens to be at the moment to a user specified location, and then adds menu entries, file type associations, desktop icons, QuickLaunch shortcuts, maybe adjusts the PATH environment variable.

In short, basically provide all the niceties one expects from an installer.

'''Goal:''' Provide a proper installer for Fiji.<br />
'''Language:''' Java.<br />
'''Mentor:''' Johannes Schindelin (johannes.schindelin AT gmx.de)<br />
...
[1] Piotr Dollar, Zhuowen Tu, and Serge Belongie, [http://citeseerx.ist.psu.edu/viewdoc/download?doi=10.1.1.91.4383&rep=rep1&type=pdf ''Supervised Learning of Edges and Object Boundaries''], CVPR, 2006.<br/>
[2] Michael Maire, Pablo Arbelaez, Charless Fowlkes and Jitendra Malik, [http://www.eecs.berkeley.edu/Research/Projects/CS/vision/grouping/papers/mafm-cvpr08.pdf ''Using Contours to Detect and Localize Junctions in Natural Images.''],CVPR 2008
...
[http://www.micro-manager.org/ µManager] is a partly platform-dependent ImageJ project that adds support for controlling microscopes (focus, stages, filter banks, objective wheels, etc).
...
This project is motivated by a specific biological questions associated with large-scale image set describing patterns of gene expression in [http://www.fruitfly.org/cgi-bin/ex/insitu.pl <i>Drosophila</i> embryogenesis]. It is inspired by the software developed under the [http://www.pnas.org/content/106/6/1826.long Cell Profiler project].
...
This approach has been successfully used to classify embryo images yet a usable interactive implementation [http://bioinformatics.oxfordjournals.org/cgi/content/full/24/17/1881 is missing]. Interestingly, the images are described by a control vocabulary annotation which presents an unique way to independently validate the results of the machine learning and could also be incorporated in the image selection process (i.e. starting from a more homogenous group).
...
*Feature extraction approaches implemented [[Feature_Extraction|in Fiji]]
*Machine learning implemented in Fiji ([http://www.cs.waikato.ac.nz/ml/weka/ Weka])
*[http://bioinformatics.oxfordjournals.org/cgi/content/full/24/17/1881 Paper] describing the principles of unsupervised machine learning approach on this dataset using combination of number of invariant features 
...
Develop storage backend that enables fast random access through limited bandwidth channels to large scale image datasets, e.g.
a Java client that can arbitrarily slice into very large volumes hosted
by a server without making the server render the slices and without the
client loading the full volume. 
...
<b>Goal</b> Implementation and detailed specification of hierarchical compressable cell-based storage format<br>
<b>Language</b> Java<br>
<b>Mentors</b> Stephan Saalfeld (saalfeld at mpi-cbg.de)<br>
...
* [http://www.cse.cuhk.edu.hk/~leojia/projects/motion_deblurring/index.html Webpage of Siggraph Paper]

<b>Goal</b> Implementation of motion deblurring using algorithms avaible in imglib<br>
<b>Language</b> Java<br>
<b>Mentors</b> Stephan Preibisch (preibisch at mpi-cbg.de)<br>
...
[[Image:NucleiDAPIconfocal.png|400px|center]]
...
[[Image:Tem-clahe-50-256-2.5.jpg|500px|center]]

There are [http://ai.stanford.edu/~ruzon/tex_seg/ a number] [http://people.csail.mit.edu/sarasu/pub/texture05/ of] [http://www-sop.inria.fr/ariana/DEMOS/wold/node2.html texture] [http://www.cs.iupui.edu/~tuceryan/research/ComputerVision/moment-paper.pdf segmentation] [http://citeseerx.ist.psu.edu/viewdoc/summary?doi=10.1.1.49.6534 algorithms] that appear to solve the problem to various extents.

This proposal is about adding at least one such segmentation algorithm to Fiji.

'''Goal:''' Add a segmentation algorithm to Fiji which uses texture features to discern different objects from background (and from each other).<br>
'''Language:''' Java<br>
'''Mentor:''' Johannes Schindelin (johannes.schindelin@gmx.de)
...
'''Goal:''' Implement a plugin that trains a region merging model on user-specified images.<br>
'''Language:''' Java<br>
'''Mentor:''' Johannes Schindelin (johannes.schindelin@gmx.de)
