...
You should generate the pre-aligned stack with [[TrakEM2]] and export it at the resolution that you want to finally use (e.g. 10% for TEM series where the pixel size is 10&times; smaller than the section thickness) as RGB with green background (rgb 0,255,0).  In the Test plugin you would then set '''scale''' to 1.0 and use accordingly scaled parameters for all fields that take their input in pixels.  In the actual application, you then need to scale those parameters back (e.g. &times;10 when you had used a scale of 10% for export).
