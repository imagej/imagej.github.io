{{Learn}}ImageJ is easy to use, but sometimes you wish for some function that is actually implemented, yet you do not know how to trigger.  This page lists a few of those tricks.
...
Hit {{key|Ctrl|L}} to start the [[Command Finder]].  Type (part of) the name of the entry, then click on ''Show full information''.
...
Double-click on the pipette, or hit {{key|Ctrl|Shift|K}} (on Macs, it is {{key|Cmd|Shift|K}}), or select the menu item ''{{bc | Image | Color | Color Picker...}}
...
Simply activate the image with the desired ROI, then the image you want to put that ROI into, and hit {{key|Ctrl|Shift|E}}. This triggers the {{bc | Edit | Selection | Restore Selection}} which "restores" the selection.
