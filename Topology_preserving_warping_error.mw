[[Image:Warping-error-comparison.png|thumb|400px|Application of the topology-preserving warping error. Example A and B have almost the same amount of pixel error with respect to the ground truth, however, example B has no topological error.]]
...
== Pixel error ==
[[Image:Pixel-error-description.png|thumb|360px|Pixel error between two different segmentations labels (A and B) with respect to the original labels (*, ground truth).]]
The simplest way of evaluating a segmentation is by measuring the pixel error between the original labels and the segmented ones. Let <math>l_i</math> denote the value of the boundary labeling ''L'' at image location ''i''. The pixel error of ''L'' with respect to another binary labeling ''L''<sup>*</sup> is the number of pixel locations at which the two labelings disagree. This can also be written as the squared [[wikipedia:Euclidean distance|Euclidean distance]] <math>\parallel L - L^*\parallel^2</math>, which is equivalent to the [[wikipedia:Hamming distance|Hamming distance]] since the labels are binary-valued. 
...
Jain ''et al.'' <ref name="Viren2010" /> introduced the '''warping error''', another metric for comparing boundary labelings based on concepts from the field of [[wikipedia:Digital topology|digital topology]]. 
...
Now consider the pixel error of ''T'' relative to warpings of ''L''<sup>*</sup>. The ''warping error'' between some candidate labeling ''T'' and a reference labeling ''L''<sup>*</sup> is the [[wikipedia:Hamming distance|Hamming distance]] (or equivalently squared [[wikipedia:Euclidean distance|Euclidean distance]]) between ''L'' and the "best warping" of ''L''<sup>*</sup> onto ''T'':
...
[[Image:Simple-points-example.png|thumb|300px|Label relaxation is only allowed at simple points. The black pixels in the top circle and the white pixels in the bottom circle are not simple points, because flipping them would cause a merger or a split. Figure courtesy of [http://www.hhmi.org/research/fellows/jain_bio.html Viren Jain].]]
To impose topological constraints on the warping, we use concepts from [[wikipedia:Digital topology|digital topology]], a field that extends the concepts of continuous-space topology to digital images. One of the most fundamental principles of this field is that
complementary definitions of adjacency must be used for foreground (“1”) and background (“0”) pixels, so that a digital analog of the [[wikipedia:Jordan Curve Theorem|Jordan Curve Theorem]] holds. We use the 4-adjacency for foreground and the 8-adjacency for background, and calculate [[wikipedia:Connected-component labeling|connected components]] based on these adjacencies.
...
called a [[wikipedia:Homotopy|homotopic deformation]]). The converse has also been proven: two images that are topologically equivalent
...
would cause by being flipped. The possible topological changes are 
* splitting,
* merging, 
...
There are many ways to define the '''mask''', depending on the exact nature of the desired geometric constraints. For example, in the original paper implementation, they choose ''M'' to be the set of all pixels within [[wikipedia:Euclidean distance|Euclidean distance]] 5 from the background of ''L''. This allows the foreground of ''L'' to expand arbitrarily, as long as topology is preserved. But the foreground can shrink by only a limited amount. Note that basing ''M'' on ''L'' makes warping an asymmetric relation.
...
As described by the authors, there is not an efficient algorithm for finding the global minimum in Eq. (1), and indeed this is likely to be an [[wikipedia:NP-hard|NP-hard]] problem. However, there is a very simple descent algorithm for finding local minima. During warping, we are allowed to flip simple points of ''L'' that lie inside the mask ''M'', i.e., points in the set <math>simple(L) \cap M</math>. Flipping any such pixel ''j'' of ''L'' satisfying <math> \mid t_j - l_j \mid > 0.5 </math> produces a new warping with smaller error. The descent algorithm greedily picks the pixel for which this error reduction is the largest, breaking ties randomly.
...
Since <math>\parallel T-L \parallel ^2</math> is decreasing, the algorithm is guaranteed to converge to a local minimum of the warping error. How problematic is the lack of an efficient algorithm for finding a global minimum? It does not seem a problem in practice. The warpings found by this descent algorithm look reasonable to the eye. In spite of the random choice of flipped pixels, the results are highly reproducible in practice.

===Warping error versus Rand error===
At first glance, it may seem that the '''warping error''' measures only boundary detection performance. But it is also a good measure of segmentation performance. This is because digital topology tells us how any single pixel affects the global topology of an image.
...
The warping error metric is implemented for 2D images in the [[Trainable Weka Segmentation]] library. Here is an example of how to use it in a [[Beanshell_Scripting|Beanshell script]]:
