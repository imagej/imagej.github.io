...
So if your image as <code>microns</code> as physical units for the pixel size, and <code>seconds</code> as units of the frame interval, any velocity reported by TrackMate computed on this image will be in units of <code>microns/s</code>. 

=== Track branching analyzer. ===

[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackBranchingAnalyzer.java code on github]
...
The size of the longest gap. That is: the number of frames that are skipped over by the longest gap in the track.
For instance, a gap between a spot in frame 2 and a spot in frame 4 would generate a gap of size 1 (the frame 3).
...
[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackDurationAnalyzer.java code on github]
...
[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackIndexAnalyzer.java code on github]
...
[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackLocationAnalyzer.java code on github]
...
[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackSpeedStatisticsAnalyzer.java code on github]

Feature values related to velocity. 
...
Same thing, but taking the max, min, median and standard deviation respectively. 
All these feature values take the link velocities of a track as a distribution they summarize. 


=== Track spot quality. ===

[https://github.com/fiji/TrackMate/blob/master/src/main/java/fiji/plugin/trackmate/features/track/TrackSpotQualityFeatureAnalyzer.java code on github]
...
[https://github.com/tinevez/TrackMate-TrackAnalysis/blob/master/src/main/java/fiji/plugin/trackmate/features/track/LinearTrackDescriptor.java code on github]
...
It was written to add track descriptor that would help characterize the track motility type. 
The features calculated there will help you determine whether a tracked object moves 'efficiently' or has an apparently random motion, etc. I simply took some of the feature described in the following paper:

''Methods for cell and particle tracking.'', Meijering E, Dzyubachyk O, Smal I., Methods Enzymol. '''2012''';504:183-200. [https://www.sciencedirect.com/science/article/pii/B9780123918574000094 link to paper].
...
[[File:TrackMate track features.png]]


==== Total distance traveled. ====

This feature value reports the full distance the particle traveled throughout the track. 
It is computed by taking the sum of the link distance (distance from the source spot to the target spot of the link, for all links). 
...
This feature reports the distance to the furthest point of the track, with respect to the first spot in time of the track. 
To get this value we simply computes the straight-line distance between each spot of the track to the first spot of the track, and returns the largest distance.
...
The confinement ratio tells how "efficient" was a track displacement in getting far away from its starting point.
It is defined as the net-displacement divided by the total-distance. The net-displacement is given by the '''Track displacement''' feature, and the total distance is given by the '''Total distance traveled''' feature.
...
Sometimes this value is called differently in other papers. For instance in [http://jcb.rupress.org/content/jcb/217/9/3031.full.pdf De Pascalis ''et al.'', 2018], is called '''persistence'''.
