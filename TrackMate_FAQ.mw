...
This list of frequently asked questions is compiled from several questions we have seen several times on the [http://forum.imagej.net/ ImageJ forum]. It will be updated as questions appear there.
...
There is a ROI exporter action is in a separate package called <tt>TrackMate-extras</tt>. To install it, you have to download its ''jar'' file and drop it in the ''plugins'' folder of Fiji. Link and instructions are on the [[TrackMate#Downloadable_jars]] page.
...
''We would like to know if there is any tool to copy the spots/track created in one stack (fluorophore 1, file 1) and paste them in another stack (fluorophore 2, file 2). We explored the option <tt>Copy overlay to</tt> but it did not work for more than a single frame of the stack.''
...
* Change the <tt>filename</tt> tag to point to the other TIF file on which you want to copy the overlay.
...
=== Various <tt>java.lang.NoClassDefFoundError</tt>s upon execution. ===
...
<pre>Exception in thread "TrackMate plot spot features thread" java.lang.NoClassDefFoundError: 
...
* [[Updater|Update]] Fiji.
* Check if you have the [[2015-12-22_-_The_road_to_Java_8|Jave8]] update site in the Fiji [[Updater]]
* Finally, if nothing works, rather than digging in your Fiji installation in details, just install Java 8 and [[Downloads|re-download]] a fresh Fiji application.
