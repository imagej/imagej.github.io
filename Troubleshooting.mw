...
<ul>
<li>[[File:Tux.png|x20px]] On Linux 64-bit (from a console):
<source lang="bash">DEBUG=1 $HOME/ImageJ.app/ImageJ-linux64</source>
</li>
<li>[[File:Osx.png|x20px]] On OS X (from Terminal):
<source lang="bash">DEBUG=1 /Applications/ImageJ.app/Contents/MacOS/ImageJ-macosx</source>
</li>
<li>[[File:Win.png|x20px]] On Windows 64-bit:
<ul>
...
You can control the log level more precisely by setting the <code>scijava.log.level</code> system property. E.g., on Linux:
...
[[File:Win.png|x20px]] On some 32-bit Windows systems, ImageJ may initially request more memory than Windows can handle. If you launch ImageJ in debug mode (see above), and receive a message like:
<source>
...
<ul><li>Note that by default, Windows hides file extensions; you may need to [http://windows.microsoft.com/en-us/windows/show-hide-file-name-extensions show file extensions] before you can successfully name the file <code>ImageJ.cfg</code> as required.</li></ul>
</li>
<li>Try running <code>ImageJ-win32.exe</code> again.</li>
</ul>
You can replace the <code>512m</code> with however many megabytes of memory you wish to give to ImageJ.
...
You can fix this by clicking on {{bc | Image | Adjust | Brightness/Contrast...}} and hitting the ''Auto'' button.

You can verify whether the actual data is there by moving the mouse over the image, and looking at the pixel probe output in the [[Getting Started#The_status_bar|status bar area of the main ImageJ window]].
...
You can override the autoscaling using the [https://imagej.net/docs/guide/146-28.html#sub:Adjust Brightness/Contrast] dialog.

It is important to understand that [[Principles#What_are_pixel_values.3F|your image is a collection of samples, each of which has a numerical intensity value]]. The unit of these values is rather arbitrary and unspecified, depending on the type and calibration of your detector. Your file is stored with a certain [https://imagej.net/docs/guide/146-7.html#toc-Section-7 bit depth], meaning these intensities can range from 0 (no light detected) to a particular maximum value (the most light the detector is capable of detecting). For example, 8-bit images have a maximum value of 255, whereas 16-bit images have a maximum of 65535. In practice though, especially with higher bit depths, your detector will not typically record sample intensities across that entire range of values (and if it does record a significant number of values at the maximum, you probably oversaturated your detector, which will skew your analysis!).

Because the full range of values is typically much less than the maximum—e.g., in the case of a 12-bit detector the actual maximum range is 0-4095, and often even smaller in practice—ImageJ performs '''autoscaling''' to show you a meaningful or "pretty good" image by default, which is not just a black square (see previous question). That is: it maps the darkest actual intensity in your data to black, and the brightest actual intensity in your data to white. You can override this mapping using the [https://imagej.net/docs/guide/146-28.html#sub:Adjust Brightness/Contrast] dialog under the {{bc | Image | Adjust}} menu (shortcut: {{Key | shift}}+{{Key | C}}).
...
Are you using a [https://imagej.net/docs/guide/146-7.html#sub:Native-Formats compressed format] such as JPEG, PNG or ZIP? The file size on disk is smaller than the size of the pixels in memory. ImageJ reports this true (uncompressed) size of the image in the subtitle bar of the image window. For example: an uncompressed image of 16000 pixels x 16000 pixels x 32 bit (RGBA) will occupy 976 MB in memory.
...
** If the two versions of ImageJ match but produce different numerical results, it is a bug—please [[Report a Bug|report it]]!
...
The best way to monitor ImageJ's actual memory usage is to run the [https://imagej.net/docs/guide/146-31.html#toc-Subsubsection-31.3.5 Monitor Memory...] command. You can also click on the ImageJ [[status bar]] to trigger a garbage collection operation, which will typically decrease the memory use.
