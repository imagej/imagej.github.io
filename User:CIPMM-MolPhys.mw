...
== Installation ==
You can install LRoi (and all future plugins) by simply adding the following update site to the ImageJ updater: http://sites.imagej.net/CIPMM-MolPhys/[http://sites.imagej.net/CIPMM-MolPhys/].
...
[[File:LineTool.png]] [[File:SeedingLine.png]]


Open the LRoi plugin and specify the number of ROIs you want to create, the ROI width and height.

[[File:LRoi_example.png]] [[File:LRoi-length.png]]


The ROI height is used to adjust the length of your seed line w.r.t. its center point. This can be used as an drawing aid, since it might sometimes be difficult to draw a line with a speciffic length by hand.
LRoi ensures, that your seeding line will have the exact length you specified.
...
These advanced ROI creation schemes can be activated by simply clicking the respective checkbox in the LRoi window.
Regular ROI grids are defined by the number of pixels they should include (If you define a grid size of 8, each grid cell will be 8 by 8 pixels large).
