...
* [http://www.maestrodev.com/better-builds-with-maven/0-about-this-guide Better Builds with Maven]


== Install m2eclipse ==

'''Note:''' if you use Eclipse 3.7 Indigo or newer, you most likely have m2e already. If your {{bc | File | Import}} offers you a ''Maven'' section, you have it, and can skip the rest of this section.

* From the Eclipse menu, choose {{bc | Help | Install New Software...}}
...
Open the file "C:\Users\you\Programs\eclipse\eclipse.ini" in a text editor, and follow [http://wiki.eclipse.org/Eclipse.ini#Specifying_the_JVM these instructions] to specify the JVM using the "-vm" flag. Be careful to follow the instructions exactly.
...
I ([[JeanYvesTinevez|Jean-Yves]] speaking) noticed a very annoying problem. At work, I live behind a firewall, and whatever I did, the version of maven shipped with Eclipse (Indigo or Juno) never managed to reach out the outside world (even tweaking proxy configuration). This resulted in the impossibility to import any maven project.

One solution that worked for me was to configure Eclipse no to use the maven version it shipped. Instead, I used the one shipped with [http://pdb.finkproject.org/pdb/package.php/maven fink] accessible from the command line, and with a system proxy configured correctly.
