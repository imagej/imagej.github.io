There is a [[Downloads|new version of Fiji]]!
...
If you already have a Fiji installation, please just use the [[Fiji Updater]] to update (make sure that you update <u>only</u> the Updater first if you have not updated in quite a while) -- do not forget to switch to the ''Advanced mode'' and hit ''Update Java''.
...
It is now possible to turn a binary image of points into a point selection, by starting {{bc | Edit | Selection | Points from Mask}}.

There is now a plugin that saves the current image in EPS (Encapsulated PostScript) format. Thanks to Wilhelm Burger.

Use {{bc | Plugins | Transform | Panorama equirectangular view}} to look at 360° panorama photographs and pan around. Thanks to Stephan Saalfeld.
...
Any script or macro can now live in any menu, by putting it into Fiji.app/plugins/Scripts/. For example, if you want to have a macro in {{bc | File | Import}}, you might save it to Fiji.app/plugins/Scripts/File/Import/.

As always, Fiji was synchronized frequently with ImageJ, and a few bug fixes were contributed to ImageJ through Fiji. Thanks to {{Person|Rasband}}.
...
Add the {{bc | Plugins | Examples | Extended Profile Plot}} script to get profile plots of closed polygons.

Add ''Scale to DPI'' function to {{bc | Image | Adjust}} menu, thanks to Curtis Rueden.
...
Fiji's handling of {{bc | Help | Refresh Menus}} has seen a number of bug fixes, as well as Fiji's headless mode (which will stop being a clever hack with ImageJ2, but instead be a proper solution).
...
When using the Recent Commands (available via the {{key|9}} key), the Recorder properly records which action was chosen.
...
The Script Editor is now usable even if the user updated ImageJ to the daily version. Thanks to {{Person|Rasband}}.

The Script Editor offers to {{bc | File | Open Recent}}. Thanks to Albert Cardona.

The startup time of the Script Editor was reduced dramatically.

In the Script Editor, {{bc | File | Open}} will no longer offer to open .class or .jar files.
...
To generate tool icons from 16x16 pictures, use {{bc | Plugins | Examples | Image To Tool Icon}}.
...
For Fiji Developer, adding documentation to the Fiji Wiki has been made even more convenient with enhancements to {{bc | Plugins | Utilities | Fiji | New Fiji Tutorial}} and New Fiji Wiki Screenshot in the same menu.
