...
== 3D Segmentation ==
This plugin implements various algorithms to segment 3D images, as part of the [[3D ImageJ Suite]].
...
* 3D '''hysteresis thresholding''' with two thresholds (see [https://imagejdocu.list.lu/plugin/segmentation/hysteresis_thresholding/start 2D hysteresis] for explanation).
* 3D '''simple segmentation''' with thresholding to label 3D objects (similar to [https://imagejdocu.list.lu/plugin/analysis/3d_object_counter/start 3D object counter]).
...
* '''Edges''' : the threshold where the objects has maximal edges (see [https://imagejdocu.list.lu/plugin/filter/3d_edge_and_symmetry_filter/start edge plugin]) 
...
[[File:IterativeTouching.png|600]]
...
If you find this plugin useful for your work, please cite this paper and refer to  [[3D ImageJ Suite]] and this article : [http://www.biomedcentral.com/1471-2105/15/9  A generic classification-based method for segmentation of nuclei in 3D images of early embryos]

== 3D Spot Segmentation ==
The plugin works with two images, one containing the '''seeds''' of the objects, that can be obtained from local maxima (see [[3D Filters]]), the other image containing signal data. The program computes a '''local threshold''' around each seeds and cluster voxels with values higher than the local threshold computed. Spots-like objects can be enhanced using a topHat (see [[3D Filters]]) or a [[Edge and symmetry filter ]]. A plugin [https://imagejdocu.list.lu/tutorial/plugins/3d_maxima_finder 3D maxima Finder] is also available to compute these local maxima, similarly to the find maxima in ImageJ/Fiji. 
...
A tutorial is also available :  [[Media:3d seg spot tutorial.pdf]]

[[File:Heck-orig.png]] [[File:Heck-watershed.png]][[File:Heck-seg.png]]
...
Two plugins '''3D splitting''' and '''3D Voronoi''' are also available, more details in this brief [https://imagejdocu.list.lu/tutorial/general/watershed_3d tutorial].
...
== License ==
GPL distribution (see [http://www.cecill.info/index.en.html license]). Sources  are available freely.
