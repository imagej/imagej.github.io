...
You should have a sub-menu {{bc | Plugins | BigDataViewer}}.
...
[https://fly.mpi-cbg.de/~pietzsch/bdv-example/ here], comprising two views and three time-points.
...
To start BigDataViewer, select {{bc | Plugins | BigDataViewer | Open XML/HDF5}} from the Fiji menu.
...
For example, open the sample image {{bc | File | Open Samples | Mitosis (26MB, 5D stack)}} from the Fiji menu.
Subsequently select {{bc | Plugins | BigDataViewer | Open Current Image}} which will launch the BigDataViewer with the sample image, for navigation by arbitrary re-slicing.

==== Imaris files ====
[http://www.bitplane.com/imaris/imaris Imaris (Bitplane)] uses a hierarchical data format (similar to BigDataViewer's XML/HDF5 format).
In order to open Imaris <tt style="font-size: 1.25em;">.ims</tt> files select {{bc | Plugins | BigDataViewer | Open Imaris (experimental)}} from the Fiji menu.
...
The axis-rotation commands (e.g., {{key press|Shift|X}}) rotate around the current mouse location.
That is, if you press {{key press|Shift|X}}, the view will pivot such that you see a ZY-slice through the dataset (you look along the X-axis).
The point under the mouse will stay fixed, i.e., the view will be a ZY-slice through that point.
...
Select {{bc | Settings | Visibility & Grouping}} from the BigDataViewer menu to bring up a dialog to control source visibility.
...
(menu {{bc | Settings | Visibility & Grouping}} or shortcut {{key press|F6}}).
...
To change the brightness, contrast, or color of particular sources select {{bc | Settings | Brightness & Color}} or press the shortcut {{key press|S}}.
...
BigDataViewer allows to bookmark the current view.
...
Each bookmark has an assigned shortcut key, i.e., you can have bookmarks "a", "A", "b", ..., "1", "2", etc.
To set a bookmark for the current view, press {{key press|Shift|B}} and then the shortcut you want to use for the bookmark.
To recall bookmark, press {{key press|B}} and then the shortcut of the bookmark.

BigDataViewer provides visual feedback for setting and recalling bookmarks.
When you press {{key press|Shift|B}}, the message "'''set bookmark:'''" appears in the lower right corner of the main window, prompting to press the bookmark shortcut next.

[[Image:bdv-set-bookmark.png|500px]]

Now press the key you want to use as a shortcut, for example {{key press|A}}.
The prompt message will change to "'''set bookmark: a'''" indicating that you have set a bookmark with shortcut {{key press|A}}.
Instead of pressing a shortcut key you can abort using {{key press|esc}}.

Similarly, when you press {{key press|B}} to recall a bookmark, the prompt message "'''go to bookmark:'''" appears.
Now press the shortcut of the bookmark you want to recall, for example {{key press|A}}.
The prompt message will change to "'''go to bookmark: a'''" and the view will move to the bookmarked location.
Instead of pressing a shortcut key you can abort using {{key press|esc}}.

Note, that bookmark shortcuts are case-sensitive, i.e., {{key press|A}} and {{key press|Shift|A}} refer to distinct bookmarks "a" and "A" respectively.

The bookmarking mechanism can also be used to bookmark and recall orientations.
Press {{key press|O}} and then a bookmark shortcut to recall only the orientation of that bookmark.
This rotates the view into the rotation of the bookmarked view (but does not zoom or translate to the bookmarked location).
The rotation is around the current mouse location (i.e., the point under the mouse stays fixed).
...
Select {{bc | File | Save settings}} from the menu to store settings to an XML file, and {{bc | File | Load settings}} to load them from an XML file.
...
(If you select {{bc | File | Save settings}}, this filename is already suggested in the Save File dialog.)
...
You can open individual images from a dataset as ImageJ stacks using {{bc | File | Import | BigDataViewer...}} from the Fiji menu.
...
If you enable the ''"open as virtual stack"'' checkbox the image will open as an ImageJ ''virtual stack''.
This means that the opened image is backed by BigDataViewer's cache and slices are loaded on demand.
Without ''"open as virtual stack"'', the full image will be loaded into memory.
...
This file format is build on open standards [http://www.w3.org/XML/ XML] and [http://www.hdfgroup.org/HDF5/ HDF5].
...
The format is explained in detail in our paper [http://arxiv.org/abs/1412.0488 BigDataViewer: Interactive Visualization and Image Processing for Terabyte Data Sets].

=== About the BigDataViewer data format ===
Each BigDataViewer dataset contains a set of 3D grayscale image volumes organized by ''timepoints'' and ''setups''.
In the context of lightsheet microscopy, each channel or acquisition angle or combination of both is a ''setup''.
E.g., for a multi-view recording with 3 angles and 2 channels there are 6 ''setups''.
Each ''setup'' represents a visualisation data source in the viewer that provides one image volume per ''timepoint''.
We refer to each combination of ''setup'' and ''timepoint'' as a ''view''.
Each ''view'' has one corresponding grayscale image volume.
...
Each ''view'' has one corresponding image volume which is stored in the HDF5 file.
...
In this situation, BigDataViewer can rapidly load and render low-resolution data, filling in high resolution detail later as it becomes available.
This multi-resolution pyramid scheme is illustrated in the following figure.

[[Image:bdvTikz-pyramidblocks.png]]
...
Each level of the multi-resolution pyramid is stored as a ''chunked multi-dimensional array''.
Multi-dimensional arrays are the standard way of storing image data in HDF5.
The layout of multi-dimensional arrays on disk can be configured.  We use a ''chunked'' layout which means that the 3D image volume is split into several chunks (smaller 3D blocks).
...
During interactive navigation, subsequent slices typically intersect with a similar set of chunks because their pose has changed only moderately, i.e.. cached data are re-used.
Only chunks that are not currently in the cache need to be loaded from disk, as illustrated in the following Figure.

[[Image:bdvTikz-cacheblocks.png]]
...
In particular, when exporting images to the BigDataViewer format, the following parameters are adjustable:
...
You can export any dataset to BigDataViewer format by opening it as a stack in Fiji and then selecting {{bc | Plugins | BigDataViewer | Export Current Image as XML/HDF5}} from the Fiji menu.
If the image has multiple channels, each channel will become one ''setup'' in the exported dataset.
If the image has multiple frames, each frame will become one ''timepoint'' in the exported dataset.
Of course, you may export from virtual stacks if your data is too big to fit into memory.

To get started, let's open one of the ImageJ sample images by {{bc | File | Open Samples | T1 Head (2.4M, 16-bits)}}.
Selecting {{bc | Plugins | BigDataViewer | Export Current Image as XML/HDF5}} brings up the following dialog.

[[Image:bdv-export-stack.png|500px]]
...
If the ''"use deflate compression"'' checkbox is enabled, the image data will be compressed using HDF5 built-in DEFLATE compression.
...
In part (B) of the dialog the ''value range'' of the image must be specified.
BigDataViewer always stores images with 16-bit precision currently, while the image you want to export is not necessarily 16-bit.
...
I.e., the minimum of the value range will be mapped to the minimum of the unsigned 16-bit range (0).
...
After you have specified the value range and selected and export path, press ''OK'' to export the dataset.
Messages about the progress of the operation are displayed in the ImageJ Log window.

[[Image:bdv-export-stack-log.png|500px]]
...
If the checkbox ''"manual mipmap setup"'' is enabled, you can customize the multi-resolution mipmap pyramid which stores your image stacks.
...
The ''"Subsampling factors"'' field specifies a comma-separated list of resolution levels, formatted as <tt>{level, ..., level}</tt>.
Each <tt style="font-size: 1.25em;">level</tt> is a list of subsampling factors in ''X, Y, Z'', formatted as <tt>{x-scale, y-scale, z-scale}</tt>.
For example consider the specification <tt>{{1,1,1}, {2,2,2}, {4,4,2}}</tt>.
...
The third level has half the resolution of the second in all axes, i.e., it is down-scaled by factors ''4, 4, 2'' in ''X, Y, Z'' respectively.
...
The ''"Hdf5 chunk sizes"'' specifies the chunk sizes into which data on each resolution level is sub-divided.
This is again formatted as <tt>{level, ..., level}</tt>, with the same number of levels as supplied for the ''Subsampling factors''.
Each <tt style="font-size: 1.25em;">level</tt> is a list of sizes in ''X, Y, Z'', formatted as <tt>{x-size, y-size, z-size}</tt>.
For example consider the specification <tt>{{16,16,16}, {16,16,16}, {16,16,16}}</tt>.
This will sub-divide each resolution level into chunks of ''16&times;16&times;16'' pixels.
...
With ''"manual mipmap setup"'' turned off, reasonable values will be determined automatically depending on the resolution and anisotropy of your dataset.
...
If the checkbox ''"split hdf5"'' is enabled the dataset will be split into multiple HDF5 ''partition'' files.
The dataset can be split along the ''timepoint'' and ''setup'' dimensions.
Specify the number of ''timepoints per partition'' and ''setups per partition'' in the respective input fields.

For example, assume your dataset has 4 setups and 10 timepoints.
Setting ''timepoints per partition = 5'' and ''setups per partition = 2'' will result in 4 HDF5 partitions:
...
BigDataViewer seamlessly integrates with the "Multiview Reconstruction" plugins that Fiji offers for registration and reconstruction of lightsheet microscopy data.
Recent versions of these tools build on the same XML format as BigDataViewer itself.
...
In principle, BigDataViewer is able to display a TIFF dataset as is.
However, for quick navigation this is not the ideal format:
When navigating to a new timepoint, BigDataViewer needs to load all TIFF files of that timepoint into memory, suffering a delay of tens of seconcds.
Therefore, it is beneficial to convert the TIFF dataset to HDF5.
Note, that this can be done at any point of the processing pipeline (i.e., before registration, after registration, after multiview fusion or deconvolution, etc)
because the "Multiview reconstruction" plugins can operate on HDF5 datasets as well.

We assume that the user has already created an XML/TIFF dataset, as explained in [[Multiview-Reconstruction]].

To convert the dataset to HDF5, select {{bc | Plugins | Multiview Reconstruction | Resave | As HDF5}} form the Fiji menu.
This brings up the following dialog.

[[Image:bdv-mvr-export1b.png|500px]]
...
Once you have determined what you want to convert press ''OK''.

This brings up the next dialog, in which you need to specify the export path and options.

[[Image:bdv-mvr-export2.png|500px]]
...
[[SPIM_Registration_on_cluster|'''this wiki page''']]. Specifically, the sections
[[SPIM_Registration_on_cluster#Define_XML|'''Define XML''']] and
[[SPIM_Registration_on_cluster#Re-save_as_HDF5|'''Re-save as HDF5''']] deal with the conversion process.
...
Subsequently, we launch one HDF5 re-save job per time point which generates as many <tt style="font-size: 1.25em;">.h5</tt> files as there are time points.
An additional master <tt style="font-size: 1.25em;">.h5</tt> file and the updated XML allow seamless navigation of such dataset with BigDataViewer.
Since the dataset will typically reside on a cluster filesystem without a graphical user interface, it is advisable to register it with the [[BigDataServer|'''BigDataServer''']] and examine it remotely.
