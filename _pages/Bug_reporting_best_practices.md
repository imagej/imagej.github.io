{{HelpMenu}}A bug report is a reproducible set of steps describing a problem. They are a common communication medium between users and developers. Users willing to take the time to write helpful bug reports drive the development of ImageJ, making it a better product for everyone.

= TL;DR Summary =

* Report the issue using the [[Report a Bug]] plugin (in the Help menu).
* Provide a [http://stackoverflow.com/help/mcve minimal, complete, verifiable example] (MCVE).
...
Here are some quick tips for [http://en.wikiquote.org/wiki/Blaise_Pascal writing a shorter letter]:
* Use bullet points to summarize.
* Do not inline lengthy logs; use [https://gist.github.com/ Gist] or [http://pastebin.com/ Pastebin] instead.
* Use formatting such as '''emphasis''' and <code>code snippets</code> to make text easier to read.
* Use [https://help.github.com/articles/github-flavored-markdown link syntax] instead of relying on URL autolinking.
* Use footnotes [1] for lengthy asides.
* Most importantly, provide a [http://stackoverflow.com/help/mcve Minimal, Complete and Verifiable example] (MCVE), ideally as a standalone project.
...
The easiest and safest way to provide sample data is via the {{bc | Help | [[Upload Sample Image]]}} command. This sends the image to a private space accessible only by trusted developers. This allows proprietary data to be tested without fear of public distribution.
...
The {{ListOfUpdateSites|default update sites}} are intended to be fairly stable, but if you have additional update sites enabled there can be a risk of skewed or out of date dependencies (due to changes in core libraries), and some update sites are intentionally experimental.
...
# If necessary, identify the jar containing the problematic class(es), e.g. by using {{bc | Plugins | Utilities | Find Jar for Class}} in Fiji.
# Start the updater with <code>{{bc | Help | Update...}}</code>
# Switch to Advanced Mode
# Search for the problematic components. Their associated update site will be listed here.
# Select <code>Manage Update Sites</code> and disable the update site identified in 4.
# Repeat 1-5 until your problem is resolved.

'''NOTICE:''' During this process it is critical to keep in mind that update sites take precedence in the {{ListOfUpdateSites|order they are declared}} - update sites lower on the list will overwrite components in sites higher on the list.

=== Binary search through your external plugins ===

If it is not clear which update sites or external plugins are causing problems in your installation, a simple technique to help identify the cause of your problem is a [[wikipedia:Binary search algorithm|binary search]]. The general procedure is such:
...
Note: if the "erroneous behavior" is [[#Catastrophic_failure|catastrophic]] to the point you can not start ImageJ, you can instead start from a clean build and re-introduce update sites and local plugins as detailed above until the problem is identified.
