...
[ImageJ](ImageJ ) and [Fiji](Fiji ) are developed according to the [SciJava philosophy](Philosophy ), thus these applications are used throughout this tutorial to illustrate the development lifecycle.
...
{% include box float='right ' title='When to use a topic branch? ' text='[Core SciJava components](Architecture ) employ a "release ready master branch" approach:
...
-   **Collaborating developer.** If you have permission to push directly to the project's remote repository, then you can simply use [Git](Git ) and [GitHub](GitHub ) to [clone the repository](https://help.github.com/articles/cloning-a-repository/) and make your changes. For non-trivial changes, you will typically [create a topic branch](Git_topic_branches ) to develop and test the changes. This also provides a forum for discussion and review with your fellow developers.
...
-   **Collaborating developer.** Minimal changes can be pushed back directly to `master` on the remote repository. If your work is on a [topic branch](Git_topic_branches ) then you should use a [pull request](https://help.github.com/articles/using-pull-requests/) (PR) so that the topic branch can be reviewed before being merged to `master`.

<!-- -->

-   **External developer.** First push your changes back to a branch of your forked repository (it doesn't necessarily have to be `master`). Then you can file a [pull request](https://help.github.com/articles/using-pull-requests/) (PR) on [GitHub](GitHub ) to merge your branch back to the official repository. This invites code review from other interested developers. Reviewers might ask for changes to the code to address any issues. After any needed revisions have been made, a [project maintainer](Governance ) will accept your changes and then merge to the official `master` branch.

Phase 3: Released
-----------------

Once the `master` branch of a component has your desired new functionality, the next step is to cut a *release* version of the component. Normally, the Maven version (in the [pom.xml](https://maven.apache.org/pom.html#Introduction)) on master is a [*SNAPSHOT* version](http://stackoverflow.com/q/5901378), meaning it is [unstable](Reproducible_builds ) and not yet released. However, a [stable](Reproducible_builds ) *release* artifact can be deployed to the appropriate remote Maven repository.
...
-   [Travis CI](Travis_CI ) performs the actual release for you, using credentials which are encrypted in the repository itself.
...
The script will verify that your master branch is ready to go, then create and push a [tag](http://git-scm.com/book/en/v2/Git-Basics-Tagging) for the release. [Travis CI](Travis_CI ) will then notice the tag and complete the release for you. You should receive an email from Travis after the release is complete indicating whether the build was successful.
...
-   Actually, for the moment, both of the above statements are untrue, but they represent the direction we are heading. Right now, core components of both ImageJ and Fiji are distributed manually via the Java-8 update site. See the [Java 8](Java_8 ) page for details.
...
If you do manage [your own update site](How_to_set_up_and_populate_an_update_site ), you can upload your release yourself.

See the [documentation on update sites](Update_Sites ) for further instructions.
