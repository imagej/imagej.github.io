...
[[File:FLIMJ_usage_open_dataset.png|300px|center]]
...
[[File:FLIMJ_first_launch.png|600px|center]]
...
FLIMJ plugin currently only supports the selection of IRF from a single pixel in an [[#Startup|acceptable dataset]] that is taken during a standard IRF measurement procedure (such as {{GitHub|org=flimlib|repo=flimj-ui|path=test_files/urea.sdt|label=this one using urea crystals}}). The steps are as follows:

# Click on the drop-down menu, select ''From file''; select the dataset file that contains the IRF.
# In '''Preview''' panel, select ''IRF Intensity'' for the "Show" option (you may also select ''Grayscale'' for the "as" option to deactivate pseudocoloring):[[File:FLIMJ_irf_show_irf.png|200px|thumb|center|Choose ''IRF Intensity'' to enter IRF picking mode.]]
# Click on the image to select the candidate pixel and adjust the ''Start'' and ''End'' cursors in '''Plot''' pannel to crop out the section of interest (''IRF is plotted in green''):[[File:FLIMJ_irf_crop_irf.png|350px|thumb|center|Drag the cursors to crop IRF.]]
# Exit the IRF picking mode by selecting other options for the "Show" in '''Preview''' panel and adjust the ''Start'' cursor to get a better fit.
...
[[File:FLIMJ_fit_dataset_preview_results_show.png|220px|thumb|center|Viewing the <math>\tau</math> (lifetime) parameter of the fitted dataset.]]

You may also change the "as" option to specify how the results are rendered. Currently supported options are:

[[File:FLIMJ_fit_dataset_preview_results_as.png|600px|thumb|right|Left to right: The <math>\tau</math> image rendered with '''Grayscale''', '''Color''' and '''Composite Color'''.]]
...
* '''Color''': Linearly maps the middle 80 percentile of the result image's values to colors from <code><span style="color:hsl(20,100%,50%)">hsl(20, 100%, 50%)</span></code> to <code><span style="color:hsl(220,100%,50%)">hsl(220, 100%, 50%)</span></code> (see color bar below). Out-of-range values follow the same rule as in '''Grayscale'''.
...
[[File:FLIMJ_fit_dataset_preview_results_lut.png|200px|thumb|center|Hover on image to see the color bar, the pixel value, and its place in the range.]]

== Image export ==

[[File:FLIMJ_image_export.png|200px|thumb|right|Options on the '''Export''' pannel.]]
...
{{Notice | You may follow the following steps to reproduce the composite color image after export:
# Export "Intensity" with desired fitted images;
# Convert all images to RGB format by searching and executing the "RGB Color" command in FIJI;
# Search and execute the "compose rgb-stacks" script, make the intensity image the "source" and the fitted image the "target", and choose "Multiply" as "compose method". Hit "OK".
}}
