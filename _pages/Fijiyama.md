...
| released               = February 10<sup>th</sup>, 2020
...
[[File:Reg_present_1.png|700px]]

==Dataset with DOI for testing purpose==
A comprehensive dataset can be found at [https://zenodo.org/record/3695736#.Xl6JaEpCdhE this link] on dryad.
...
{{#widget:YouTube|id=scm6UPlfgzU|width=480}}

==Preparing your data==
In order to register your data, Fijiyama needs properly formatted dataset:
* For two images pairwise registration : '''Two 3D stacks/images''' of the same object. Use your own data or the provided example datasets here: [https://imagej.net/_images/f/fd/Test_dataset_01_vine_crops.zip case study 1, two-images registration]
* For series registration : '''N 3D stacks/images''' of the same object. Use your own data or the provided example datasets here: [https://imagej.net/_images/c/c8/Test_dataset_04_time_series.zip case study 4, times series registration].
...
* '''Step 1 : ''' Prepare your data or download the example set: [https://imagej.net/_images/f/fd/Test_dataset_01_vine_crops.zip case study 1, two-images registration]
...
{{#widget:YouTube|id=DVr3LBH5ayY|width=480}}

* '''Step 1 : ''' Prepare your data or download the example data set : [https://imagej.net/_images/c/c8/Test_dataset_04_time_series.zip case study 4, times series registration]
...
Major updates are symbolized with a change in the plugin version name.
For minor updates, only the release time is modified.

[[File:Versioning Fijiyama.jpeg|700px|thumb|center]]
...
| <b>First release</b>
| <b>Last release</b>
| <b>Integration of new features</b>
| <b>Modifications causing reproducibility issues</b>
|-
| <b>Elastic Eucalyptus</b>
| 2020/02/10
| 2020/02/20
| Integration of the multimodal series registration feature. Redefinition of default settings in the settings editor.
| Default settings changes in the settings editor
|-
| <b>Felicity Ficus</b>
| 2020/02/20
| 2020/03/02
| Integration of the time-lapse multimodal series feature. Versioning of serialized files
| 1th march: versioning of serialized files starts. Previously serialized files are then open with default values for level min, level max and block matching iterations.
|-
| <b>Gorgeous grapevine</b>
| 2020/03/20
| 2020/03/20
| Hyperimage support (registration of 4D / 5D multi-channels or multi-time images
|-
| <b>Handsome Honeysuckle (Future release) </b>
| Mid-june 2020
| ...
| New transformation models : Rigid 2D, Similarity 2D, Dense 2D. Support of extensive grandeurs (images which intensity have to raise when it is shrinked). Support of hyperimage with varying number of channels
| ...
|}

==Possible issues and recommendations==
* '''Dealing with large datasets : ''' biological images are sometimes very large. In the field of time-lapse microscopy, [[BigStitcher]] plugin handles such data, using BigDataViewer, as its registration algorithm, perfect for monomodal data of lightsheet microscopy, is well fitted to this task. 
...
* Rigid transformations (translation + rotation), estimated using [https://imagej.net/VIB-lib VIB-lib]
* Similarities (rigid + isotropic homothetic factor) estimated using [https://imagej.net/VIB-lib VIB-lib]
...
== Limitations ==
For large datasets (15 and more timepoints) or images (1GB+), specific tools should be used. 
Here is a non-exhaustive list of plugins for [[ImageJ]] / [[Fiji]], available for 3D registration of large datasets: [[BigStitcher]], [[Atlas_Toolkit]], [[TrakEM2]] or [[Elastix]] wrapping plugin. 
Outside from the ImageJ world, for people who are not afraid to script, [http://elastix.isi.uu.nl/ Elastix] is a proper tool for series, and [https://www.slicer.org/ Slicer] can also do a great job. 
Limited to two-images registration, [https://med.inria.fr/ MedInria] is a very powerful and popular tool for medical images, which shares a lot of algorithmic basis with Fijiyama.
...
* Johannes Schindelin et al for [https://imagej.net/Fiji Fiji] (Schindelin et al., 2012)
* Johannes Schindelin et al. for the [https://imagej.net/VIB-lib VIB-lib]
* Benjamin Schmid et al for the [https://imagej.net/3D_Viewer 3D Viewer] (Schmid et al., 2010)
* The [https://itk.org/about/ ITK] and [http://www.simpleitk.org/ SimpleITK] teams (Yoo et al., 2002; Lowekamp et al., 2013)
