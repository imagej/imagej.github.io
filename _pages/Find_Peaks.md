...
| author          = {{Person|Tiago}}
| filename        = {{GitHub|org=tferr|repo=Scripts|path=BAR/src/main/resources/scripts/BAR/Data_Analysis/Find_Peaks.bsh}}
| source         = {{GitHub|org=tferr|repo=Scripts|path=README.md#data-analysis}}
| released        = February 2014
| category        = [[:Category:Analysis|Analysis]], [[:Category:Scripting|Scripting]], [[:Category:Plugins|Plugins]]
}}

A [[BAR]] script (written in [[Beanshell_Scripting|BeanShell]]) that retrieves local maxima and minima from an ImageJ plot. The easiest way to install ''Find Peaks'' is by [[BAR#Installation|subscribing]] to the BAR {{ListOfUpdateSites|update site}}.
...
* Use ''Scientific notation'' and ''Decimal places'' in {{bc|Analyze|Set Measurements...}} to improve the representation of values that are too big or too small to be displayed in the decimal form
...
== Installation ==
The easiest way to install ''Find Peaks'' is by [[BAR#Installation|subscribing to the BAR update site]]. 
