...
# Many ImageJ 1.x plugins assume the data is stored in an ImageJ 1.x data structure, which is not what BDV uses. So [[ImageJ Legacy|"mixing and matching" IJ1 and IJ2 functionality]] is tricky here. But we are working to lift these restrictions as time goes on.
...
Install Java 8, and delete or rename the <code>ImageJ.app\java</code> and/or <code>ImageJ.app\jre</code> folders, if they exist. If this does not result in ImageJ using the expected Java version, check the Environment Variables ({{bc | Control Panel | System and Security | System | Advanced Settings | Advanced | Environment Variables}}) for the variable "JAVA_HOME".  Update or create this variable as needed; its value should be the desired JDK or JRE that you would like to use for ImageJ (for instance: "C:\Program Files\Java\jdk1.8.0_172"). See also [http://stackoverflow.com/questions/1672281/environment-variables-for-java-installation Java environment variable setup].
...
Note: the <code>--java-home</code> flag does not support Apple Java installations. Specifying Apple Javas with this flag will give an error message about being unable to find <code>lib/server/libjvm.dylib</code>. However, ImageJ will fall back to Apple Java if no other Java installations are available.

In particular, this means that to run Java 6 on the latest macOS versions, you must do '''''ONE''''' of the following:
* '''''EITHER:''''' Remove all installations of Java 7 and Java 8 ([https://www.java.com/en/download/help/mac_uninstall_java.xml remove the JRE] and [http://docs.oracle.com/javase/7/docs/webnotes/install/mac/mac-jdk.html#uninstall remove all JDKs]).
...
If you flag any changes to be made, press the "Apply changes" to update your ImageJ. And after restarting ImageJ, you might want to run {{bc | Help | Update...}} again to make sure everything looks the way you expect!

== How do I install additional plugins? ==

If the plugin is published on an [[Update Sites|ImageJ update site]], you can run {{bc | Help | Update}} then click the ''Manage update sites'' button to enable it. Not only does this install the plugins for you automatically, but you will also be notified of any updates whenever they are released.

Otherwise, you can drag 'n drop the .jar files onto the ImageJ window, or use {{bc | Plugins | Install Plugin...}} with .jar, .class and .java files, or copy the plugins to ''ImageJ.app/plugins/'' and restart ImageJ. See the [[Installing 3rd party plugins|walk-through with screenshots]].
...
* You can change the version of [[ImageJ 1.x]] used via the {{bc | Help | Update ImageJ...}} menu item. This ''only'' changes the version of ImageJ 1.x—not all ImageJ components, [[plugins]], etc.
* You can [[#How do I launch ImageJ with a different version of Java?|switch to a different version of Java]].
* You can [[Downloads#Fiji|download a Fiji Life-Line version]].
...
The safest way to ensure these conventions is to use the {{bc | File | Export as .jar file}} menu item.

== I tried to update ImageJ via {{bc | Help | Update}}, but it throws an exception instead? ==

Please download a fresh copy from [[Downloads|here]].

== I updated ImageJ via {{bc | Help | Update}}, and now it does not start anymore! ==

See [[Troubleshooting#If_ImageJ_does_not_start_up|If ImageJ does not start up]] on the Troubleshooting page.

== The Updater always says ''Connection refused''. Maybe it does not use the proxy? ==

Indeed, an earlier version of the Updater does not use your system-wide network proxy settings. You can [[Downloads|download a new ImageJ]] to receive the fix. Or you can update the Updater manually like this:
...
[[ImageJ1|ImageJ1]] is an extensible platform for scientific image analysis. [[ImageJ|ImageJ2]] is redesigned from the ground up to meet the same needs, yet engineered with future-proof flexibility to adapt to researchers' growing requirements (new data types, higher dimensionality, huge images, etc...).
...
Yes, see the [https://github.com/imagej/tutorials/tree/master/maven-projects/call-modern-from-legacy call-modern-from-legacy] tutorial example.
...
Fiji is a ''distribution'' of ImageJ: it bundles the core ImageJ application with a curated selection of plugins pre-installed. Specifically, those plugins found on the {{ListOfUpdateSites|Fiji update site}}.
...
Fiji is just ImageJ. If you are running [[ImageJ2]], simply run {{bc | Help | Update}}, click ''Manage update sites'', and enable the ''Fiji'' update site.
...
Use the ImageJ-MATLAB [[update site]]. See the [[MATLAB]] page for details.
...
# '''Using [http://search.imagej.net search.imagej.net]
...
#* This will open the source from the [https://imagej.net/source/ ImageJ web site online source].
...
#* Run {{bc | Plugins | Scripting | Open Source for Menu Item}}.
...
#** E.g., {{bc | Process | Binary | Make Binary}}.
#** Again, select it from the menu directly, not using the Command Finder.
#* This will open the source using the [https://fiji.sc/git fiji.sc gitweb interface].
...
If your software is written in Java, we recommend using [[Maven]] to structure your project. You can then add dependencies to portions of ImageJ that are of interest to you. See the {{GitHub|org=imagej|repo=tutorials|label=ImageJ tutorials}} for examples.
...
See also:
* [[2011-10-05 - The relation between ImageJA and ImageJ|an ImageJ2 blog post clarifying specifically the relationship between ImageJ and the version used in Fiji]]
* [[2012-02-01 - The Fiji Updater has been integrated into the ImageJ2 code base|an ImageJ2 blog post illustrating that we move code from Fiji to ImageJ2 as appropriate]]
...
If you have an issue that is not mentioned here, you might want to [[Help|ask on one of the mailing lists, or via IRC]], or [https://fiji.sc/bugzilla/ enter a bug report].

== How to report issues? ==

Use the {{bc | Help | [[Report a Bug]]}} command, which adds a ticket report to the [https://fiji.sc/bugzilla/ Fiji bug tracker]. While private mail might seem more desirable sometimes, but it is almost always inferior to the open process we established in ImageJ. For starters, bug reporters are unlikely to know who would be the best person to address the issue.
...
There are not many professional developers whose formal jobs include responding to and addressing bug reports. These developers are not paid ''solely'' to fix bugs—rather, they are working to solve scientific problems and to advance the software's capabilities. And their attention is spread thin across many projects: [https://github.com/imagej ImageJ], [https://github.com/fiji Fiji], [https://github.com/scifio SCIFIO], [https://github.com/scijava SciJava], [https://github.com/uw-loci LOCI], [https://github.com/openmicroscopy OME], [https://github.com/imglib ImgLib2], [https://github.com/openspim OpenSPIM], [https://github.com/slim-curve SLIM Curve], [https://github.com/maven-nar NAR plugin for Maven], and others.

The organizations above total over 300 source code repositories, many of which have dozens of open issues in their issue trackers. There are also several hundred tickets in the [http://trac.imagej.net/ old ImageJ issue tracker] that are pending migration to GitHub Issues, as well as over a hundred [https://fiji.sc/bugzilla/buglist.cgi?resolution=--- open issues in the Fiji Bugzilla].
...
A shorter term pragmatic solution would be an ImageJ web client that connects to a remote server running ImageJ in headless mode, which does the heavy lifting. See also the [http://openmicroscopy.org/info/omero OMERO project].
