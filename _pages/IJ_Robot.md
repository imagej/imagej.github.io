{{ComponentStats:sc.fiji:IJ_Robot}}This plugin calls the Robot Java class. The purpose of the plugin is to allow the macro language to control other programs via clicking and key presses.
...
'''CaptureScreen''': this is similar to the IJ function {{bc | Plugins | Utilities | Capture Screen}}.
...
It may be also necessary to slow down the macro calls to this plugin between orders by using the macro command: wait(time_in_milliseconds). For instance if you are grabbing an image with an external programme, the grab function may not be available while the snapshot is being taken.
...
== Menu Path ==
{{bc | Plugins | Utilities (2nd on the list!) | IJ Robot}}
