{{Learn | techniques}}{{ComponentStats:sc.fiji:Stitching_}}{{TOC|small}}
== Citation ==
...
* '''Average''': In the overlapping area the average intensity between all images is computed ({{GitHub|org=fiji|repo=Stitching|source=mpicbg/stitching/fusion/AveragePixelFusion.java|label=example source code}}).
...
[[File:GridStitching1013.png|400px|thumb|Shows the main dialog for the grid stitching.]]
...
The ''fusion methods'' are almost the same as the ones for the [[Image Stitching#Pairwise Stitching | Pairwise Stitching]], please check them out above.
...
[[File:DownsampleDialog.png|100px|thumb|Shows the secondary dialog for downsampling during stitching.]]
...
Please see the [[Grid/Collection Stitching Plugin]] page for a complete set of instruction on how to use Grid/Collection stitching in Fiji.
...
For any type of comment, questions or input please write to preibischs@janelia.hhmi.org or visit my [http://fly.mpi-cbg.de/preibisch homepage].

== Download for ImageJ & Acknowledgements ==

Although the Stitching depends on quite some Fiji-specific libraries, there is a download for ImageJ available on my webpage in the [http://fly.mpi-cbg.de/~preibisch/software.html software section].
...
* ''edu_mines_jtk.jar'': Efficient 1-dimensional FFT implementation by [http://inside.mines.edu/~dhale/jtk/ Dave Hale].
* ''mpicbg.jar'': Transformation package by {{Person|Saalfeld}}
* ''[[imglib]]'': N-dimensional image processing library for Java by {{Person|Pietzsch}}, {{Person|StephanP}} & {{Person|Saalfeld}}
* ''fiji-lib.jar & Fiji_Plugins.jar'': Fiji libraries by {{Person|Schindelin}}
* ''loci_tools.jar'': [[Bio-Formats]] Java library for reading and writing life sciences image file formats. I want to especially thank {{Person|Rueden}} and {{Person|Linkert}}
...
Danielle Bower, {{Person|Albertcardona}}, Nicolas Gompel, Wieland Huttner, {{Person|Jefferis}}, Arnim Jenett, {{Person|tomka}}, David Koos, Jan Peychl, {{Person|Tomancak}}, James Truman, Nicholas Weiler and {{Person|White}}
...
* [http://www.xuvtools.org/doku.php XuvTools] similar stitching software (but you cannot access XuvTools' source code freely) from the University of Freiburg and the [http://dx.doi.org/10.1111/j.1365-2818.2008.03094.x abstract] of the accompying publication
