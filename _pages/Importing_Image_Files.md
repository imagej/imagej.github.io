...
Most of the commands listed, particularly Fiji extensions, are also available (via ImageJ's [https://imagej.net/plugins/file-handler.html HandleExtraFileTypes] feature) simply by using the File / Open... menu item.

== Built-in formats ==

Fiji primarily uses TIFF as the image file format. The menu command {{bc | File | Save}} will save in TIFF format. The menu command {{bc | File | Open}} will open TIFF files and import a number of other common file formats (e.g. JPEG, GIF, BMP, PGM, PNG) as well as those detailed below. Files can be also dragged and dropped to the toolbar to open them.

== Bio-Formats ==

Many more file formats can be imported via Fiji plugins. One plugin is called [[Bio-Formats]]. This will import and export a wide number of file formats along with their important metadata. They can be accessed directly via the menu command {{bc | File | Import | Bio-Formats}} or indirectly via the {{bc | File | Open}} menu command which will use the Bio-Formats plugin if needed. See the [http://openmicroscopy.org/site/support/bio-formats5/supported-formats.html list of supported file formats] on the Bio-Formats website for the complete list (135 as of this writing).

If Bio-Formats is used, you will be prompted with a big dialog and your preferences remembered.

Drag and dropping an image file to the Fiji toolbar will effectively open the file the same as the {{bc | File | Open}} command.
...
The LSM panel plugin duplicates the Zeiss LSM file import function of the LOCI Bio-Format Importer, but will also extensively catalogue the LSM metadata. The ''[[LSM Toolbox]]'' is activated by the menu command {{bc | File | Import | LSM.}} Once you select LSM the panel will appear.

Images are opened as 8-bit color images with the “no-palette” pseudo color from the LSM acquisition software. Each channel is imported as a separate image/stack. Lambda stacks are therefore imported as multiple images, not a single stack. They can be converted to a stack with the menu command: {{bc | Image | Stacks | Images to stack.}}
...
Non-images will also be opened and may appear as blank images and need deleting: {{bc | Image | Stacks | Delete slice}}. The stack will "interleave” the multiple channels you recorded, and can be de-interleaved via {{bc | Plugins | Stacks - Shuffling | DeInterleave}}. 

== AVI and MOV files ==

There are two plugins which can open uncompressed AVIs and some types of MOV file. AVIs can be opened via {{bc | File | Import | AVI}}
...
* '''Analyze...'''
* '''[[LSM Toolbox | Show LSM Toolbox]]''': Open the LSM Toolbox window, from which LSM files can be opened, viewed in multi dimensions, montaged,  their metadata directly inspected, time and z stamped and more. 
