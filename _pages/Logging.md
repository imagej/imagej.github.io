...
The `LogService` is accessible from a SciJava plugin in the same way as any other [service](Services): as a field annotated with `@Parameter`. See the [Writing plugins](Writing_plugins) guide for details.
