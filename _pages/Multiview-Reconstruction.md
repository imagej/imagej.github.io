...
To enable GPU hardware accelerated processing, you might want to download the **native CUDA code** for:
  ...
  - Non-Separable Convolution: Used for the [MultiView Deconvolution](Multi-View_Deconvolution "wikilink"), available on [GitHub](https://github.com/StephanPreibisch/FourierConvolutionCUDALib)
...
Interactive viewing and annotation of the data is provided by integration with Tobias Pietzsch's [BigDataViewer](BigDataViewer "wikilink"). Both projects share a common XML data model to describe multiview datasets.

### History

This software package is the successor to the [SPIM Registration](SPIM_Registration "wikilink") package. While the SPIM Registration will continue to live within Fiji for the time being, we will mostly offer support only for this new software package. It has all the functionality the SPIM Registration offered, but is much more flexible and supports many more types of registration, fusion and data handling.
...
<!-- end list -->
  ...
  - **[Resave the dataset as HDF5/TIFF](MVR-ResaveDataset "wikilink")**
      - Once the dataset is defined, you might want to resave all the image data as HDF5 (to be able to view it using the [BigDataViewer](BigDataViewer "wikilink")) or simply as TIFF to enable fast loading of the image data. Also note that those two formats are the only ones that allow to extend the dataset/XML with newly fused data.
...
<!-- end list -->
  ...
  - **[Interest Point Registration](MVR-InterestPointRegistration "wikilink")**
      - Once interest points are detected, they can be used to register/align multiple views over time. To find corresponding detection the registration module supports the following modules:
          ...
          - *Translation-invariant matching* using any kind of detections (an approximate knowledge of the rotation is required, e.g. 45 degrees around the x-axis. Check the [Tools-Section](Multiview-Reconstruction#Tools "wikilink") for how to provide approximate transformations.)
...
<!-- end list -->
  ...
  - **[Fusion/Deconvolution](MVR-FusionDeconvolution "wikilink")**
      ...
      - Alternatively, there is no need to fuse the data and you can interactively look at the data using the [BigDataViewer](BigDataViewer "wikilink") if you resaved it as HDF5. You can still do that now after the registration is complete.
...
<!-- end list -->
  ...
  - **[Duplicate Transformations](MVR-DuplicateTransformations "wikilink")**
