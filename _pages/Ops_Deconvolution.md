...
[[wikipedia:Richardson%E2%80%93Lucy deconvolution|Richardson Lucy]] is an iterative deconvolution algorithm that can be used to reconstruct a blurred image.   

<math>\mathbf{o}_{k+1}(s)=\left\{\left[\frac{i(s)}{(o_{k}*h)(s)}\right]*h(-s)\right\}{o}_{k}(s)</math>
...
To prevent noise amplification the Richardson Lucy algorithm should be regularized.  [https://en.wikipedia.org/wiki/Regularization_%28mathematics%29     Regularization] "refers to a process of introducing additional information in order to solve an ill-posed problem or to prevent overfitting".

[http://www.ncbi.nlm.nih.gov/pubmed/16586486 Richardson Lucy with Total Variation Regularization]

<math>\mathbf{o}_{k+1}(s) = \left\{\left[\frac{i(s)}{(o_{k}*h)(s)}\right]*h(-s)\right\}\frac{{o}_{k}(s)}{1-\lambda_{TV}div\left({\frac{\nabla_{ok}(s)}{|\nabla_{ok}(s)|}}\right)}</math>
...
[http://www.ncbi.nlm.nih.gov/pubmed/18250863 Acceleration of Iterative Algorithms]

[http://ceur-ws.org/Vol-446/p400.pdf Richardson Lucy with TV Regularization and Vector Acceleration]
...
[http://bigwww.epfl.ch/deconvolution/challenge2013/index.html?p=doc_math_rl Non-circulant Richardson Lucy]

==Scripts==

A collection of scripts that use ops-deconvolution is being developed and can be found [https://github.com/imagej/imagej-scripting/tree/master/src/main/resources/script_templates/Deconvolution here on github].  The scripts can also be accessed via the Templates->Deconvolution menu in the Fiji script editor.  Note that as new scripts are added it may take a couple of weeks for them to be available in Fiji.  Thus check github for the most up to date collection of scripts. 

==Preliminary Results==

Preliminary results have been generated using the [http://bigwww.epfl.ch/deconvolution/bio/ C. Elegans Embryo] images and PSFs provided by the Bio-Imaging Group of EPFL.  Note that this result is not yet optimal, as the image has spherical aberration (asymmetrical 'flare' in the xz view) and the provided PSF did not have SA.  In the future better ops to produce a theoretical PSF with spherical aberration will be developed. 

We encourage users of deconvolution to try deconvolving this image with various commercial and open source packages and discuss results on the [http://forum.imagej.net/ ImageJ Forum].

===Original===
[[File:Composite_orig_xy.jpg|300px]][[File:Composite_orig_zx.jpg|140px]]

===Deconvolved===
[[File:Composite_decon_xy.jpg|300px]][[File:Composite_decon_zx.jpg|140px]]
