...
= Installation =
The ParticleSizer script combines a number of different plugins. The most easy way (and at the moment the only way) is to [https://imagej.net/How_to_follow_a_3rd_party_update_site follow] Biomedgroup & ParticleSizer [https://imagej.net/Update_Sites update site]. 
...
The ParticleSizer supports the [https://labs.inf.fh-dortmund.de/NanoDefiner/?anchor=version#version NanoDefiner e-tool]. After the analysis of a dataset is finished, you can select the results table and export the cummulative mass function of the feret min diamater by "Results > Export CMF". The exported file can be uploaded to the [https://labs.inf.fh-dortmund.de/NanoDefiner/?anchor=version#version NanoDefiner e-tool].
