...
| source                 = [https://imagej.net/plugins/download/Rolling_Ball_Background.java Rolling_Ball_Background.java]
| released               = 22 November 2007
| status                 = first version
| category               = [[:Category:Filtering|Filtering]]
| website                = [https://imagej.net/plugins/rolling-ball.html]
}}
...
This plugin implements (differently) the same algorithm as the one built-in ImageJ in the {{bc | Process | Subtract background}} menu, but adds a useful ''Preview'' capability. Also, to display the background subtracted in a separate (new) window, hold the ALT key when pressing "OK" (Preview must be off).
