...
=SNT Commands=
SNT registers three commands in Fiji's menu structure in the {{bc|Plugins|NeuroAnatomy|&nbsp;}} sub-menu:
;'''[[#Startup_Prompt|SNT]]...''':The main interface with ''all'' available commands with access to all viewers and canvases. Tracing and path editing operations are done from this interface.
;'''[[SNT: Reconstruction Viewer|Rec. Viewer]]''':A fast, streamlined interface for analysis of existing neuroanatomical data.
;'''[[#RecPlotter|Reconstruction Plotter]]''':A utility command for quickly render reconstructions in vector format without having to load other interfaces.
...
* {{bc|Export As| SWC|&nbsp;}} Allows export of all traced paths in the [[SNT:_FAQ#swc|SWC]] file format, the most common format for representing neuronal reconstructions. Note that you can also export subsets of paths using the [[#Path Manager|Path Manager]].
* {{bc|Save Measurements...}} Allows export of measurements generated by the ''Measure...'' and ''Quick Measurements'' commands in the [https://en.wikipedia.org/wiki/Comma-separated_values CSV] file format.
* {{bc|Reset SNT and Restart...}} Resets all preferences and restarts SNT.
<div align="left">
<ul> 
<li style="display:inline-block;"> [[Image:SNT-Path-CSV-Properties-Example-Spreadsheet.png|none|thumb|1000px|Exported measurements for OP_1 (truncated)]] </li>
</ul>
</div>
{{clear}}

===Analysis===
* {{bc|Path Order Analysis}}   Produces a table and plot from morphometric statistics on the group of paths associated with each Branch Order. See [https://imagej.net/SNT:_Analysis#Path_Order_Analysis] for details.
* {{bc|Sholl Analysis...}}  Brings up the [https://imagej.net/Sholl_Analysis Sholl Analysis] dialog with a pre-defined set of focal points the user can choose from. See [https://imagej.net/SNT:_Analysis#Sholl_Analysis] for details.
* {{bc|Sholl Analysis (by Focal Point)...}}  Allows precise positioning of the focal point used by Sholl Analysis. See [https://imagej.net/SNT:_Analysis#Sholl_Analysis_.28by_Focal_Point.29] for details.
* {{bc|Strahler Analysis}}  Conducts [https://imagej.net/Strahler_Analysis Strahler Analysis] on existing paths, producing a table of analysis results and a Strahler plot. See [https://imagej.net/SNT:_Analysis#Strahler_Analysis] for details.
* {{bc|Export CSV Properties...}} Allows export of information regarding individual Paths, including (but not limited to) morphometrics, neurite compartments, linkage relationships to other Paths and start and end coordinates.
* {{bc|Measure...}}   Provides a comprehensive selection of measurements to apply to one connected component (i.e., rooted tree structure) from the Path Manager. See [https://imagej.net/SNT:_Analysis#Measurements] for details.
* {{bc|Quick Measurements|}}  Shows a table with summary statistics for all existing paths. To measure only selected paths, use {{bc|Analyze|Measure}} in the Path Manager. See [https://imagej.net/SNT:_Analysis#Measurements] for details.

===Utilities===
* <span id="RecPlotter"></span>{{bc|Reconstruction Plotter...}} Plots a dynamic 2D projection of all existing paths. A control panel is included which allows adjustment of the spatial orientation of the tracing. If paths are assigned color tags, use {{bc|Actions|Render final (colorized) plot}} from the control panel to render the (static) plot with color-mapped paths, with the final orientation matching that of the dynamic plot.
<div align="left">
<ul> 
...
* {{bc|Batch Scripts|&nbsp;}} A collection of batch processing scripts related to image processing, reconstruction analysis and data curation. See [https://imagej.net/SNT:_Scripting#Batch Script Templates: Batch] for details.
{{clear}}

===Scripts===
See [https://imagej.net/SNT:_Scripting SNT: Scripting].

===View===
Contains commands for organizing tracing views on screen, plus commands for displaying cached data used for auto-tracing.

* {{bc|Arrange Views}}  Resets the view pane on-screen positions to their default.
* {{bc|Hide Tracing Canvas|&nbsp;}} Allows toggling of the visibility of the three orthogonal view panes as well as the Legacy 3D View window.
* {{bc|Show Cached Filtered Image}}  Displays the cached filtered image in a separate window. Note a filtered image must be loaded in order to use this feature. The process of loading a filtered image is detailed in [[SNT:_Step-By-Step_Instructions#Generating_Filtered_Images|Generating Filtered Images]].
* {{bc|Show Cached Hessian Tubeness Image}}  Displays the cached ''Hessian (Tubeness) image'' in a separate window. Note there are two ways to enable this feature. Either 1) enable ''Hessian-based analysis'' then run the ''Cache All Hessian Computations'' command from the "Auto-tracing" gear drop-down menu or 2) use the ''Load Precomputed "Tubeness" Image'' option in the same gear menu. These features are detailed in the "Auto-tracing" section of the [[SNT:_Overview#Main_Dialog|Main Dialog]] overview guide.
...
<div align="left">
<ul> 
<li style="display:inline-block;"> [[Image:Snt-Estimate-Radii-Image-Result.png|none|thumb|200px]] </li>
...
{{Tip| id = coloring-paths|tip = The [[#Path Manager|Path Manager]] offers several ways to colorize Paths: 1) Using {{bc | Tag | Color}} swatches (custom colors can be temporarily assigned to empty swatches, by right-clicking on them) or 2) Using {{bc|Analyze|Color Coding...}}, providing morphometric-based [[SNT:_Overview#Analyze|color mapping]]. Note that Path Manager commands are applied to all paths if no paths are selected.
<div align="center">
<ul> 
<li style="display:inline-block;"> [[Image:Path-manager-color-tag.png|none|thumb|272px|Color tagging]] </li>
<li style="display:inline-block;"> [[Image:Color-tag-result-2.png|none|thumb|150px|Arbitrary path coloring]] </li>
<li style="display:inline-block;"> [[Image:Color-tag-result-image.png|none|thumb|216px|Result]] </li>
</ul>
</div>
}}
...
* '''Overlay MIP(s) at X% opacity''' Overlays the [https://en.wikipedia.org/wiki/Maximum_intensity_projection Maximum Intensity Projection] of the image "over" the image canvas at the specified opacity. The overlaid projection is only used as a visualization aid and is ignored by the auto-tracing algorithms. It is rendered using the LUT of the channel currently being traced. To reload the overlay (e.g., in case the image being traced changes during a tracing session) toggle the checkbox twice.
{{clear}}
<div align="center">
<ul> 
...
** The max number of parallel threads to be used by SNT, as specified in ImageJ's {{bc|Edit|Options|Memory &amp; Threads...}}
...
The [https://imagej.net/SNT:_Reconstruction_Viewer Reconstruction Viewer] is an advanced, fully scriptable OpenGL viewer enabling 3D visualization and interaction with reconstructions, OBJ meshes (including multiple model organism neuropil annotations and reference brains), computed surfaces (convex hulls) and other shape annotations. For performance reasons, some Path Manager changes may need to be synchronized manually from the RV controls. To open Reconstruction Viewer with the current contents of the Path Manager, press ''Open Reconstruction Viewer''. To instead open the viewer as a standalone application, go to {{bc|Plugins|NeuroAnatomy|Reconstruction Viewer...}} in the main Fiji dialog.
===SciView=== 
[[SciView|SciView]] is IJ2's modern replacement for the Legacy 3D Viewer, providing sophisticated 3D visualization and virtual reality capabilities for arbitrarily large image volumes and meshes. Since it is fully integrated with SNT, SciView can enhance tracing, analysis and visualization routines by providing a deeper, more contextualized view of the imagery at hand. Additionally, native integration with [https://github.com/morphonets/cx3d Cx3D] enables simulation of neurodevelopmental processes, including neuronal growth and formation of cortical circuits. See [https://imagej.net/SNT:_Modeling SNT: Modeling] for details.
...
==Menu Commands==
===Edit===
[[File:SNT-Path-Manager-Edit.png|right|thumb|250px|"Edit" menu]]
* {{bc|Delete...}} Removes selected Path(s) from the Path Manager. If no Paths are selected, all Paths are deleted.
* {{bc|Rename...}} Renames the selected Path. Only one Path may be renamed at a time.
* {{bc|Make Primary}} Makes the selected Path the primary Path, moving it to the top of the tree hierarchy. Note that this will alter parent-child relationships between Paths and, by consequence, individual nodes.
* {{bc|Disconnect...}}  Disconnects the selected Path from all of its connected Path(s) (Undoable operation).
* {{bc|Merge...}} Merges the selected Paths (at least two) into one. Note the starting node of Path ''i'' is merged to the endpoint of Path ''i+1''.
* {{bc|Specify Radius...}} Assigns a constant radius to all the nodes of selected Path(s). This setting only applies to unfitted Paths and overrides any existing values. 
* {{bc|Ramer-Douglas-Peuker Downsampling...}} Given an inputted maximum permitted distance between adjacent nodes, performs [https://en.wikipedia.org/wiki/Ramer%E2%80%93Douglas%E2%80%93Peucker_algorithm Ramer-Douglas-Peucker] downsampling on the selected Path(s).

===Tag===
[[File:SNT-Path-Manager-Tag.png|right|thumb|"Tag" Menu]]
Assigns tags to Paths. Tags are organized in the following categories:
* {{bc|Type|&nbsp;}} Type of neurite compartment (''Axon'', ''(Basal) Dendrite'', ''Soma'', etc.), as used by the SWC file format. It is also possible to pair each type with a color tag through the {{bc|Tag|Options..}} dialog<br>These tags are considered to be essential annotations and all paths are assigned the ''Undefined''-type tag when created. For this reason they are not eliminated by the ''Remove All Tags'' command.
* {{bc|Color|&nbsp;}} A preset swatch color, or a custom one chosen from the color chooser (right-click on a blank swatch). Note it is also possible to assign a metric-based color mapping using the {{bc|Analyze|Color Coding...}} command.
* {{bc|Image Metadata|&nbsp;}} Information on hyperstack position details  (e.g., channel, frame or slice labels on which a path was traced).
* {{bc|Morphometry|&nbsp;}} Morphometric properties, such as ''Path length'', ''Path mean radius'' or ''[[SNT:_Analysis#Path_Order_Analysis|Path order]]''.
* {{bc|Custom...}} Ad-hoc comments.
...
* {{bc|Fit Path(s).../Un-fit Path(s)/Apply Existing Fit}}  This option will change depending on which Path(s) are currently selected. You can use it to 1) Fit selected Path(s), 2) un-fit Path(s) that have already been fitted, or 3) apply a generated preview of the fit or an existing fit.
...
* {{bc|Discard Fit(s)...}} Deletes the existing fit(s) for the selected Path(s), or all fits if no Paths are selected.
...
<div align="center">
<ul> 
<li style="display:inline-block;"> [[Image:Fit-parameter-prompt.png|none|thumb|286px|Fitting parameters]] </li>
<li style="display:inline-block;"> [[Image:Before-fitting.png|none|thumb|300px|Before fitting]] </li>
<li style="display:inline-block;"> [[Image:After-fitting.png|none|thumb|300px|Fitted path]] </li>
</ul>
</div>

===Fill=== 
[[File:SNT-Path-Manager-Fill.png|right|thumb|"Fill" menu]]
* {{bc|Fill Out...}} Begins the filling process for selected paths. For detailed instructions see [[SNT:_Step-By-Step_Instructions#Filling|Filling: Step-By-Step Instructions]].
...
* {{bc|Color Coding...}} Assigns color codes to paths based on the chosen metric. 
...
{{clear}}
* {{bc|Distribution Analysis...}} Plots a histogram of the selected metric.
...
{{clear}}
* {{bc|Measure.../Quick Measurements}} Shows a table of summary statistics for selected paths. The former command allows for specific selections of metrics. If measuring multiple tracings, the table can be sorted by column.
[[File:SNT-Path-Manager-Measurements.png|center|thumb|1000px|Measurements table for OP_1]]
{{clear}}
* {{bc|Convert to ROIs...}} Allows conversion of Path(s) to ImageJ [https://imagej.nih.gov/ij/docs/guide/146-10.html#sec:Selections-Intro ROIs] (Regions of Interest). Creates an instance of the [https://imagej.nih.gov/ij/docs/guide/146-30.html#fig:The-ROI-Manager ROI Manager]
...
{{clear}}
* {{bc|Plot Profile}} Uses the pixel intensity values under point annotations to generate a pixel intensity vs. distance (from the start node of a Path) plot for selected Path(s).
<center>
{|
|-
...
{{clear}}
* {{bc|Save Subset as SWC...}} Exports the selected subset of Path(s) as an [http://www.neuronland.org/NLMorphologyConverter/MorphologyFormats/SWC/Spec.html SWC] file. Note the paths to be exported must include a primary path (i.e., one at the top level in the Path Manager hierarchy).
{{clear}}

==Filter Toolbar==
[[Image:SNT-Path-Manager-Text-Filter.png|right|thumb|]]
The filter toolbar allows paths to be searched and filtered quickly using tags (colors, annotations, SWC-type, etc.) or morphometric properties. The text field is used for text-based searches (recent searches can be recovered through its drop-down menu). The {{key|down}} and {{key|up}} arrow keys find the next/previous occurrence of the entered phrase, while the [[Image:SNT-Text-Filter-Balloon-Button.png]] button highlights all occurrences of the entered phrase. Settings for advance text-based filtering can be accessed through the [[Image:SNT-Text-Filter-Menu-Button.png]] button, including wildcard support, case sensitive matching, and replace-by-pattern. In addition, the [[Image:SNT-Text-Filter-Advanced-Button.png]] button restricts filtering to the selected subset of Path(s). Other means of filtering Paths include:
[[File:SNT-Path-Manager-Text-Filter-Color-Filters.png|right|thumb|"Color Filters" menu]]
* '''Color Filters''' [[Image:SNT-Text-Filter-Color-Button.png]] Allows filtering of Paths by color tags. Custom colors may be selected by right-clicking an empty swatch, which will bring up the CMYK palette. The chosen color is temporarily saved in that swatch.
* '''Morphology Filters''' [[Image:SNT-Text-Filter-Morphology-Button.png]] Allows filtering of Paths by selected morphological properties (including cell identity). Note that these filters do not require Paths to be labeled using {{bc|Tag|Morphology|&nbsp;}}.
** ''Path Order...'' Filters for Paths of [[SNT:_Analysis#Path_Order_Analysis|Path order]] in the inputted range. Example queries: <tt>1-2</tt>: selects all primary and secondary branches; <tt>max-max</tt>: selects all terminal branches. 
