...
| software               = Fiji
| author                 = Jan Kožusznik, Petr Bainar, Jana Klímová, Michal Krumnikl, Pavel Moravec, Pavel Tomancak
...
| source               = [https://github.com/fiji-hpc/hpc-workflow-manager/ on github]
}}
...
= Installation =
After you install and launch [https://imagej.net/Fiji/Downloads Fiji], go to {{bc | Help | Update... | Manage update sites}}, tick ''P2E-IT4Innovations'' and close the window. Then click ''Apply changes'' and restart Fiji.
...
Following a successfully finished pipeline, you can interactively examine the processed SPIM image data using the [https://imagej.net/BigDataServer BigDataServer] as well as download resultant data and a summary file containing key information about the performed job. Importantly, you can edit the corresponding local configuration file in a common text editor, and restart an interrupted, finished, or failed job.
...
Using the developed plugin, we executed the pipeline on the Salomon supercomputer. As the test data set we used 90 time-point SPIM acquisition of a ''Drosophila melanogaster'' embryo expressing FlyFos fluorescent GFP fusion reporter for the ''nrv2'' gene.
