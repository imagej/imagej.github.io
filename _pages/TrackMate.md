[[Image:TrackMate-Logo85x50-color-300p.png|right|160px]]
{{ComponentStats:sc.fiji:TrackMate_}}
== Citation == 
...
[[File:TrackMate Celegans devel.gif]]

The first 2 hours of a ''C.elegans'' embryo development, followed in 3D over time using TrackMate (strain: [http://www.wormbase.org/db/gene/strain?name=AZ212;class=Strain AZ212])


[[Image:TrackMate Celegans lineage detail.png|500px]]

A detail of the corresponding unannotated lineage visualized in TrackScheme. The full lineage of this one hour of imaging is [[:Image:TrackMate Celegans lineage.pdf|there]].


[[Image:TrackMate Split Merge Gapclosing.gif]]

TrackMate can be set to detect and deal with gap-closing events, splitting events and merging events.


[[Image:TrackMate Celegans CellDiameter.png|TrackMate Celegans CellDiameter.png]]
...
[[Image:TrackMate CelegansCellFollowed.gif]]
...
The [http://www.sciencedirect.com/science/article/pii/S1046202316303346  TrackMate paper] contains a polished, pdf version of the documentation below. But we host it here:

[[File:TrackMate-manual.pdf]] 14 MB
...
* [[TrackMate Performance]] reports various measures of the performance of its components. Performance reports include execution time and memory usage.
* [[TrackMate Accuracy]] reports various measures of the accuracy of its components. Accuracy reports how well TrackMate can generate scientifically correct results, and compare its output to known ground truth.
* Various very useful tips and tricks contributed by users:
** Dave Mason on using pivot tables to get track statistics based on spot features: [https://forum.image.sc/t/mean-intensity-of-tracks-using-trackmate/11848/5]
...
TrackMate LoG detector will crash if you have the {{ListOfUpdateSites|Mosaic suite update site}} activated. Apparently they ship something that interferes with the FFT code used by the LoG detector. The bug report can be read [https://fiji.sc/bugzilla/show_bug.cgi?id=806 on BugZilla]. 
...
|-
| TrackMate-extras
| Various small additions to TrackMate:
...
| [http://maven.imagej.net/service/local/repositories/releases/content/org/scijava/TrackMate_extras/0.0.4/TrackMate_extras-0.0.4.jar on ImageJ maven Nexus]
| [https://github.com/tinevez/TrackMate-extras on Github]
|-
| [[Find maxima (Trackmate module)]]
| This plugin implements the find maxima detection algorithm for TrackMateas in the <i>Process -> Find Maxima...</i> command. The results are almost the same. Subpixel accuracy is activated by default.
| Thorsten Wagner
| [https://github.com/thorstenwagner/ij-trackmate-findmaxima/releases/latest on Github]
| [https://github.com/thorstenwagner/ij-trackmate-findmaxima on Github]
|-
| Track analysis
...
| Jean-Yves Tinevez
| [http://maven.imagej.net/content/repositories/snapshots/sc/fiji/TrackMate_trackanalysis/0.0.1-SNAPSHOT/TrackMate_trackanalysis-0.0.1-20170614.171211-1.jar on ImageJ maven Nexus]
| [https://github.com/tinevez/TrackMate-TrackAnalysis on Github]
|-
| TrackMate -> Spot-On connector
| This extension adds an action allowing to automatically transfer a tracking analysis performed in TrackMate to [https://spoton.berkeley.edu Spot-On], without having to export the tracks and reimport them. Spot-On is a web-interface designed for the analysis of single-molecule tracking experiments.
| Maxime Woringer
| [https://gitlab.com/tjian-darzacq-lab/Spot-On-TrackMate/tags on Gitlab]
| [https://gitlab.com/tjian-darzacq-lab/Spot-On-TrackMate on Gitlab]
|}

=== Extra source code ===


[https://github.com/chicoronny/RonnyTrackMate Ronny Sczech TrackMate repository] contains the source code to various TrackMate enhancements, in Java and macros:
...
* [[Find_maxima_(Trackmate_module)|Find maxima detector]] from Thorsten Wagner.

== Who uses TrackMate ==

It turns out that TrackMate has a decent user base, as exemplified by a crude search on [https://scholar.google.fr/scholar?q=trackmate+fiji Google Scholar]. These citations accumulated before the TrackMate paper was out.
...
* [[ImgLib2]] is used everywhere we need dealing with pixels. Relying on imglib made it trivial to have a plugin that deals indifferently with 2D or 3D images. In particular, we use code from Stephan Preibisch, {{Person|Saalfeld}}, Larry Lindsey and Lee Kamentsky.
...
* Internally, the tracks are represented by a mathematical [[wikipedia:Graph %28mathematics%29|graph]]. To manipulate it, we take advantage of the excellent [http://www.jgrapht.org/ JGraphT] library.
...
* For the icons, as almost every ImageJ plugin with a GUI, we used the [http://www.famfamfam.com/lab/icons/silk/ silk icon set], by Mark James. But we are also very lucky to have icons and logos designed specifically for TrackMate by [http://www.illuscientia.com/ IlluScienta].
