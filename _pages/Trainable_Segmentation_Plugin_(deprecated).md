...
| software               = Fiji
| author                 = Verena Kaynig, {{Person|Iarganda}}, Albert Cardona
| maintainer             = [http://www.kaynig.de Verena Kaynig] and {{Person|Iarganda}}
| source                 = {{GitHub|org=fiji|repo=Trainable_Segmentation}}
...
The plugin can be found in the Fiji menu under {{bc | Plugins | Segmentation | deprecated | Trainable Segmentation}}.
...
Now open the plugin ({{bc | Plugins | Segmentation | deprecated | Trainable Segmentation}}). It opens a training window that contains the training image.
...
*''' I don't find the plugin in my Fiji menus!''': Don't worry, depending on your Fiji release, the updater may not offer you this plugin automatically. Just go to {{bc | Help | Update Fiji}}, then click on "Advanced mode", and on the "View Options" box, select "View uninstalled plugins only". Then the Trainable Segmentation plugin should appear and you should be able to install it.
