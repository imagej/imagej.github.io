...
===[[wikipedia:Gaussian blur|Gaussian Blur]]===
...
The RF algorithm uses numerous (default 200) [[wikipedia:Decision tree learning|classification trees]] (CTs) to 'vote' for which class a pixel, with its corresponding set of features, belongs to. The algorithm builds multiple trees from the same dataset by [[wikipedia:Bootstrapping %28statistics%29|bootstrapping]] (sampling with replacement).
