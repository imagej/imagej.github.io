...
The `LogService` is accessible from a SciJava plugin in the same way as any other [service](/SciJava_Common#Services): as a field annotated with `@Parameter`. See the [Writing plugins](/develop/plugins) guide for details.
