...
ImageJ 2.0.0-beta3 is a "beta"-quality release, meaning the code is not finished. The design is more stable than before but still subject to change until the final 2.0.0 release. It is recommended that people continue to use [ImageJ v1.x](/software/imagej-1.x) for critical work.
...
{% include img align="right" src="/media/news/mouse-wheel-zoom.gif" caption="The Zoom tool allows precise zooming at a specific pixel using the mouse wheel." width="456px" %}
...
-   {% include bc path='Edit | Selection | Specify'%} plugin (non-interactive for now)
...
{% include img align="right" src="/media/news/spectral-composite.gif" caption="The Spectral Composite plugin generates channel LUTs based on actual wavelengths of light." width="410px" %}
...
-   {% include bc path='FFT | Swap Quadrants'%} now working
-   {% include bc path='Hyperstack | Reduce Dimensionality'%} now working
-   {% include bc path='File | Open Recent'%} now working correctly
