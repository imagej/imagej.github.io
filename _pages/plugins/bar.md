...
{% capture source%}
{% include github org='tferr' repo='Scripts' %}
{% endcapture %}
...
In more detail: Pass the active image - an [ImagePlus](http://javadoc.imagej.net/ImageJ1https://imagej.nih.gov/ij/ImagePlus.htm) object - to `myRoutines()`. Retrieve a random {% include wikipedia title='Universally unique identifier' text='UUID'%} (e.g., `f7dfd6a9-f745-42c2-8874-0af67380c3f5`), convert it to a string, then use that string to rename the image using the [setTitle()](http://javadoc.imagej.net/ImageJ1https://imagej.nih.gov/ij/ImagePlus.html#setTitle(java.lang.String)) method in `ij.ImagePlus`.
