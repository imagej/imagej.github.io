...
{% include thumbnail src='/media/FindPeaksSnapshot.png' title='Analysis of synthetic data plotted from a .csv file. CSV files can be imported into ImageJ by drag and drop, or by using {% include bc path="File | Import | Results" %}. Once [options](#options) are specified, coordinates of retrieved peaks are logged to the table of a new plot window'%}
