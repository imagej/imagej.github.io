...
Many more file formats can be imported via Fiji plugins. One plugin is called [Bio-Formats](/formats/bio-formats). This will import and export a wide number of file formats along with their important metadata. They can be accessed directly via the menu command {% include bc content='File | Import | Bio-Formats'%} or indirectly via the {% include bc content='File | Open'%} menu command which will use the Bio-Formats plugin if needed. See the [list of supported file formats](http://openmicroscopy.org/site/support/bio-formats5/supported-formats.html) on the Bio-Formats website for the complete list (135 as of this writing).
