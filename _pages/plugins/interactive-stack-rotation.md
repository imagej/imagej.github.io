---
mediawiki: Interactive_Stack_Rotation
title: Interactive Stack Rotation
categories: [Transform,Interactive]
---


{% capture author%}
{% include person id='axtimwalde' %} ([1](mailto:saalfeld@mpi-cbg.de))
{% endcapture %}

{% capture maintainer%}
{% include person id='axtimwalde' %}
{% endcapture %}

{% capture source%}
{% include github org='axtimwalde' repo='mpicbg' branch='master' path='mpicbg_/src/main/java/Stack_Rotate.java' %}
{% endcapture %}
{% include info-box name='Interactive Stack Rotation' software='Fiji' author=author maintainer=maintainer source=source released='September 29<sup>th</sup>, 2009' latest-version='February 26<sup>th</sup>, 2010' status='stable, active' category='Plugins, Transform' %}

Here is a screencast how to use the Interactive Stack Rotation plugin:

{% include video platform='youtube' id='7so6zkwvNeQ'%}

For a quick overview of keyboard shortcuts, please press the {% include key key='F1' %} key to be shown this usage:

![](/media/plugins/interactivestackrotation.jpg)

  
