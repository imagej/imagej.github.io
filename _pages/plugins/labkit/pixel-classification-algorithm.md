...
* Each sigma value in the list is used to blur the input images with an according Gaussian blur filter. This produces $$N$$ differently blurred versions of the input image which we will denote by $$G_i$$:
  $$\begin{aligned}G_i := \mathrm{gauss}_{\sigma_i} I \quad \mathrm{for} \quad i=1,...,N\end{aligned}$$
...
* We donate the first order partial derivatives of these images by $$\partial_x G_i$$ and $$\partial_y G_i$$.
  ...
  $$\begin{aligned}partial_x G_i = {1 \over w_x} [-0.5 \ 0 \  0.5] \ast G_i\end{aligned}$$
...
* Similarely we denote the second oder partial by $$\partial^2_x G_i$$
  ...
  $$\begin{aligned}partial^2_x G_i = {1 \over w_x^2}[1 \ -2 \ 1] \ast G_i\end{aligned}$$
* The mixed second order derivative are denoted $$\partial_x \partial_y G_i$$ and they are calculated by two consequetive convolutions of the input image, with a kernel $$[-0.5 \ 0 \ 0.5]$$
  $$\begin{aligned}partial_x \partial_y G_i := {1 \over w_y}[-0.5 \ 0 \ 0.5]_y \ast \left( {1 \over w_x}[-0.5 \ 0 \ 0.5]_x \ast G_i \right)\end{aligned}$$

## Feature Filters

### Original Image
This filter is the simplest of them all, its output image $$F^\mathrm{ID}$$ is simple the input image:
$$\begin{aligned} F^\mathrm{ID} := I\end{aligned}$$

### Gaussian Blur
This filter outputs $$N$$ images which are the gaussian blurred versions of the input image:
$$\begin{aligned} F^\mathrm{GAUSS}_i := G_i \quad \mathrm{for} \quad i = 1,...,N\end{aligned}$$
...
$$\begin{aligned} F^\mathrm{DOG}_{i,j} := G_i - G_j \quad \mathrm{for} \quad i,j=1,...,N \quad \mathrm{with} \quad i<j\end{aligned}$$
...
$$\begin{aligned} F^\mathrm{GGM}_i := \sqrt{(\partial_x G_i)^2 + (\partial_y G_i)^2 + (\partial_z G_i)^2}
\qquad \left(=|\ \mathrm{grad} \ G_i \ | \ \right)\end{aligned}$$
...
$$\begin{aligned} H^{xx}:=\partial^2_x G \quad H^{yy}:=\partial^2_x G \quad H^{xy} := H^{yx} :=\partial _x \partial_y G\end{aligned}$$
For each pixel $$p$$ in the image G, those 4 partial derivatives can be sample into 2x2 matrix.
$$\begin{aligned} H_p := \begin{bmatrix}
H^{xx}(p) & H^{xy}(p) \\
H^{yx}(p) & H^{yy}(p)
...
$$\begin{aligned}F^\mathrm{HE}_1(p) := \lambda_{1,p} \quad F^\mathrm{HE}_2(p) := \lambda_{1,p}\end{aligned}$$
...
$$\begin{aligned}F^\mathrm{STEV}_1(p) := \lambda_{1,p} \quad F^\mathrm{STEV}_2(p) := \lambda_{1,p}\end{aligned}$$
