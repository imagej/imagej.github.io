...
#### Save Tracings...
Saves all paths to a TRACES file. Shortcut: {% include key key='ctlcmd|S' %}

#### Save Tracings &amp; Open Next Image
When an image is loaded the program also checks if there are other images in the same directory. This command saves current tracings alongside the image file, then loads the next image (alphabetically) in the same folder. This facilitates tracing multiple images. Note that only file extensions in _Next/Prev image: suffix filter_ in the [Additional Options...](#misc) prompt are considered (default is ".tif", ".tiff"). Shortcut: {% include key key='ctlcmd|Shift|O' %}

#### Save Tracings &amp; Open Previous Image
Similar to [Save Tracings &amp; Open Next Image](#save-tracings--open-next-image): Saves the tracing in the same directory, then loads the previous image in the same folder. Shortcut: {% include key key='ctlcmd|Alt|O' %}

#### Save Tracings As...
Saves all traced paths to a new TRACES file.

#### Export As SWC...
Exports all traced paths in the SWC format. Note that it is also possible to export subsets of paths using the [Path Manager](#path-manager).
...
#### Backup Tracings
Saves a timestamped copy of the current tracings to the `backups/` subfolder of the active workspace.
Unlike {% include bc path='File|Save Tracings|Save' %} ({% include key key='ctlcmd|S' %}), which overwrites the working file, this command always creates a new, uniquely named file. Use it at meaningful milestones, e.g., before making structural edits you may want to undo. Shortcut: {% include key key='ctlcmd|Shift|S' %}

#### Revert to Backup...

Presents a list of previously created backups and replaces the current tracings with the selected one, allowing you to go 'back in time'.
To use this feature:
...
Offers ways to manage and inspect backups, with options to delete older backups.
...
#### Send to TrakEM2
Sends current paths to [TrakEM2](/plugins/trakem2/) instances attached to SNT.
...
- **Tracing Mode** Allows toggling between _Standard_ (the default) and _Live Preview_. In _Live Preview_ (also known as "rubber band" tracing), the path to the cursor position is continuously displayed as you move the mouse. Note that this option is only recommended for 2D images
...
- **Probability maps (p-maps)** Probability maps of classified images generated by [machine learning](./machine-learning) may not be 100% accurate. We find that, in many cases it is more effective to use p-maps as secondary layers, rather than attempting fully automated p-map reconstructions that typically require time-consuming _post-hoc_ corrections
...
{% capture finishtip%}
{% include key keys='Double Click' %} can always be use to finish a path directly.
{% endcapture %}
{% include notice icon="info" content=finishtip %}
...
- **Assign diameter (Ctrl+mouse wheel)**
Diameter assigned to path nodes via {% include key keys='Ctrl|Mouse Wheel' %}, which adjusts the cursor size until it matches the thickness of the neurite being traced. Set it to 0 to disable this option.

- **Centerline opacity (%)**
The render opacity (in percentage) for node diameters and segments connecting path nodes.

- **Out-of-plane opacity (%)**
The render opacity (in percentage) for path segments that are above/below the active Z-plane. It is only considered when tracing 3D images and [visibility filters](#path-display-filters) are set to _Only nodes within {x} nearby Z-slices_.
...
- **Additional Options...** Allows setting other preferences, namely:
  ...
  - Suffix filter defining which extensions/ filename patterns should be considered by the [Open Next/Prev commands](#save-tracings--open-next-image)
  - Whether {% include wikipedia title="Gzip" %} compression (lossless) should be used to reduce the storage footprint of ".traces" files.
  - How single-node soma-tagged paths should be displayed in the tracing image: Either as a circular node (the default), or with a triangular shape
...
The Legacy 3D Viewer is a functional tracing canvas and allows images to be traced interactively in 3D. SNTv5 has implemented several improvements that have restored/improved functionality relatively to earlier versions. However, the legacy viewer relies on libraries that are not actively maintained and _may_ not function reliably during complex tasks. Please consider using [Big Volume Viewer](#big-volume-viewer) instead. For usage instructions, see [Tracing using the Legacy 3D Viewer](/plugins/snt/walkthroughs#tracing-in-the-legacy-3d-viewer).
...
### Hide Paths (Hold {% include key key='H' %})
...
### Path Orientation (Hold {% include key key='O' %})
...
{% include img align="right" width="320" name="contextual menu" src="/media/plugins/snt/snt-contextual-menu.png" %}
...
### Undo Last Segment {% include key key='Z' %}
...
### Tag Active Node... {% include key key='T' %}
...
Uses selected Path(s) to train -- in [Labkit](/plugins/labkit) -- a random forest classifier algorithm for semantic segmentation aimed at classifying neurite-associated pixels. Described in [Machine Learning](machine-learning).

#### Load TWS With Selected Paths...

Uses selected Path(s) to train -- in [TWS](/plugins/tws) -- a random forest classifier algorithm for semantic segmentation aimed at classifying neurite-associated pixels. Described in [Machine Learning](machine-learning).
