...
Options to save/export all traced paths in the SWC and TRACES formats.
Note that it is also possible to export subsets of paths using the [Path Manager](#path-manager).
...
Unlike {% include bc path='File|Save Tracings|Save' %} ({% include key key='ctlcmd|S' %}), which overwrites the working file, this command always creates a new, uniquely named file.
Use it at meaningful milestones, e.g., before making structural edits you may want to undo.
