...
This page provides a comparison of various [scripting languages](/scripting/#supported-languages) via code examples.

Each example script provides equivalent functionality: a **command launcher** that opens a small dialog window, letting you type in a command for ImageJ to execute.

As the name of the command is typed, the color of the text changes from red to black if the command exists.

Each script consists of four parts:
...
While in Clojure one is able to declare types if desired, it's not required; the low computational requirements of the script do not invite to make it verbose unnecessarily. But Java demands type declarations just so that the script can be compiled and thus a binary .class file generated.
...
Note that [Clojure](https://clojure.org) is **not** a scripting language: Clojure compiles directly to JVM bytecode, and thus runs at native speed.
...
A second version, lispier, rewritten from the above by Clojure's author Rich Hickey. Note the use of the `some` function to check whether any given key in a list (`text` is the only key here) is contained in a set of keys (`commands`):
...
For some tutorial material on using BeanShell, see [BeanShell's Quickstart](https://www.beanshell.org/manual/quickstart.html#java-statements-and-expressions).
