...
Then create a new (maybe inner) class that extends
`ij3d.behaviors.InteractiveBehavior`.
...
In the example shown above, mouse presses with hold-down control key are
captured. The universe's Picker object is used to obtain the picked
point (i.e. the point where the mouse ray intersects the object). In the
example, the point's coordinates are just displayed on the screen then.

**Important:**  
Be careful to call `super.process(...)` when necessary to retain normal
(rotating, translating) behavior as desired.
(Return to the [Developer
Documentation](3D_Viewer:_Developer_Documentation "wikilink") page)  
(Return to the main [3D\_Viewer](3D_Viewer "wikilink") page)
