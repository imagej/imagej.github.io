...
ImageJ uses [Semantic Versioning](http://semver.org/). As of this writing, the project is still in beta, so the API is not finalized yet. But once we make the final 2.0.0 release, future versions will be fully compliant. See the [Architecture](Architecture#Versioning "wikilink") page for further details.
...
We try to follow best practices for maintaining a clean and organized Git history:
  ...
  - We avoid monster commits (with commit messages like "Many changes to several subsystems") in favor of well-separated, modular commits with one conceptual change at a time. Git's staging area feature makes this much easier (e.g., `git add -p`). Granular commits have many advantages; e.g., [`git``   ``bisect`](Pinpoint_regressions_with_Git "wikilink") becomes much more useful for understanding mysterious bugs.
...
We also have several comment tokens we use in various situations:
  ...
  - Anywhere the code may be unintuitive or surprising, we add an ["NB"](wikipedia_Nota_bene "wikilink") comment that provides an explanation.
  - For code that is considered "dirty" or less than ideal, but necessary from a practical perspective, we add a ["HACK"](wikipedia_Kludge#In_computer_science "wikilink") comment explaining it.
