...
{% include develop-menu content='tutorials' %}This article explains how to install, configure and use Eclipse to develop [ImageJ](ImageJ "wikilink") [components](components "wikilink") and [plugins](plugins "wikilink"). Directions correspond to Eclipse 4.4 Luna, and may need adjustment for other versions.
...
**Understand Java 6 vs. Java 8.** Eclipse should work on OS X with no further configuration. However, we recommend reading the [OS X section of the FAQ](FAQ#Mac_OS_X "wikilink"), as there are several Java-related issues on OS X. </tab> <tab name="Linux"> ![ x32px](Tux.png " x32px") **Linux**
...
Using your [Git client of choice](http://git-scm.com/downloads/guis), clone the source code which interests you:
  ...
  - If you want to work on an existing project, see the [list of sources](source_code "wikilink").
  - If you are creating your own project, see the [building a pom](building_a_pom "wikilink") guide.
...
One major problem when debugging [ImageJ 1.x](ImageJ_1.x "wikilink") plugins is that ImageJ 1.x expects all the plugins' *.jar* files to live in a sub-directory *plugins/* in the ImageJ root directory. We can trick ImageJ by setting the property *ij.dir* to the location of the *.jar* file generated by m2e. The [Fiji](Fiji "wikilink") project provides a convenience class *fiji.Debug* in the {% include github org='fiji ' repo='fiji-lib ' label='fiji-lib ' %} component which lets you do that without any pain:
...
The format of `plugin parameters` can be determined by using the [macro recorder](macro_recorder "wikilink"), or just pass `null` if your plugin opens a dialog. For more complex plugins that are not macro recordable, you can pass empty strings to the `run` method—it will still launch an ImageJ instance with your plugin on the classpath.
...
The format of such a `pom.xml` is described briefly on the [Maven](Maven "wikilink") page.
...
Note that doing this only allows you to view the source code, it does **not** allow you to edit it. If you need to edit these files, see the [snapshot coupling](Architecture#Using_snapshot_couplings_during_development "wikilink") section for more information.
