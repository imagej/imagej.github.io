...
Five different parameters can be adjusted:
  ...
  - Gamma, which is the spatial aspect ratio, and specifies the
...
As result, the script will display the set of filters, the filtered
version of the original image for each orientation, and the projections
(average, min, max, mean and variance) of the stack of filtered images.

## Example

This is an example of the script results using the Leaf sample image ()
and sigma = 8.0, gamma = 0.25, psi = 0.0, Fx = 3.0, nAngles = 5.
![Demonstration of a Gabor filter applied to the Leaf sample image. Five
orientations are shown on the right (0°, 36°, 72°, 108° and 144°). The
original Leaf picture is shown on the upper-left
