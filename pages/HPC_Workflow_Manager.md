...
![Figure 1: Example of a filled in login dialog.](hpc-workflow-manager-login.png "Figure 1: Example of a filled in login dialog.")
...
![Figure 2: Example of the main window of the HPC Workflow Manager, it displays all jobs ever submitted by the user, in this case, it is empty as it is the first time the plugin is used.](hpc-workflow-manager-main-empty.png "Figure 2: Example of the main window of the HPC Workflow Manager, it displays all jobs ever submitted by the user, in this case, it is empty as it is the first time the plugin is used.")
...
![ Figure 3: Context menu press right click on an empty row or empty table to display.](hpc_workflow_manager_context_menu.png " Figure 3: Context menu press right click on an empty row or empty table to display.")
...
![ Figure 4: Example of a new Macro job configuration.](hpc-workflow-manager-create-job.png " Figure 4: Example of a new Macro job configuration.")
...
![ Figure 5: A new Macro job has been created.](hpc-workflow-manager-created-job.png " Figure 5: A new Macro job has been created.")
...
![ Figure 6: Uploading files (in this case just the user’s script) is done.](hpc-workflow-manager-upload-job.png " Figure 6: Uploading files (in this case just the user’s script) is done.")
...
![ Figure 7: Job is queued.](hpc-workflow-manager-queued-job.png " Figure 7: Job is queued.")
...
![ Figure 8: The job is running and the progress indicators display the progress for each task on each compute-node.](hpc-workflow-manager-progress-running.png " Figure 8: The job is running and the progress indicators display the progress for each task on each compute-node.")
...
![ Figure 10: The red circle labeled zero (0) is a task performed by all nodes. It is first added, and then its progress is reported twice: once when it is zero and finally when it is done. The green circle labeled three (3) is first added and then its progress is reported as well, however, notice that all its related commands are inside the body of `if` statements. Notice that all calls of `parAddTask()` are before `parReportTasks()`.](hpc-workflow-manager-side-by-side-example.png " Figure 10: The red circle labeled zero (0) is a task performed by all nodes. It is first added, and then its progress is reported twice: once when it is zero and finally when it is done. The green circle labeled three (3) is first added and then its progress is reported as well, however, notice that all its related commands are inside the body of if statements. Notice that all calls of parAddTask() are before parReportTasks().")
...
![ Figure 11: Function autocompletion example.](hpc-workflow-manager-autocomplete.png " Figure 11: Function autocompletion example.")
