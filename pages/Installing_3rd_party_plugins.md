...
## For users
  ...
  - If the plugin is published on an [ImageJ update
    site](Update_Sites "wikilink"), you can run
...
{% include bc content='Help | Refresh Menus' %} .
  ...
  -   
    **Note:** On MacOSX, you need to

{% include key content='Ctrl' %} + {% include key content='click' %}

`on the `*`Fiji`*` icon in the Finder and select `*`Show`` 
 ``package``   ``contents`*` to see the `*`plugins/`*` folder.`
...
where {% include bc content='File' %}

`is the menu you want to add the item to -- at the moment, the`

only valid choices are {% include bc content='Plugins' %} , {% include
bc content='Plugins | <submenu>' %} , {% include bc content='File |
Import' %} ,
