...
See [Wikipedia's article on Java logging frameworks](wikipedia:Java_logging_framework "wikilink") for further reading.
...
Still, SLF4J is a nice thing, so the SciJava project does provide a {% include github org='scijava' repo='scijava-log-slf4j' path='src/main/java/org/scijava/log/slf4j/SLF4JLogService.java' label='LogService implementation backed by SLF4J' %}, which zealous developers can use to redirect all SciJava logging to the SLF4J framework, which they can then [further redirect](wikipedia:Fundamental_theorem_of_software_engineering "wikilink") using an SLF4J binding of their choosing\!
...
The [Fiji](Fiji "wikilink") distribution of ImageJ is a more complicated beast, because it ships many plugins, each of which may do its own thing with respect to logging. Rather than [herd cats](wikipedia:Herding_cats "wikilink"), Fiji simply ships each plugin's required logging framework so that it can function unimpeded.
