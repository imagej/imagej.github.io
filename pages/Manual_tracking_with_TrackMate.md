...
The previous TrackMate tutorial - [Manual editing of tracks using TrackMate](Manual_editing_of_tracks_using_TrackMate "wikilink") is dedicated to manually correcting the results of an automated process. This small tutorial here shows how to do a fully manual annotation with [TrackMate](TrackMate "wikilink").

## Setting up

![](TrackMate_ManualTrackingEntryPoint.png)

There is not much to do. We will use the same, simple dataset that for [Getting started with TrackMate](Getting_started_with_TrackMate "wikilink"). You can find it in {% include bc content='File | Open Samples | Tracks for TrackMate (807K)'%}.
...
All we have done so far was to create single spots, that are not part of any tracks. Tracks are created on the fly when you link several spots together. You can do it in [TrackScheme](TrackScheme "wikilink"), as explained elsewhere. Here is how to do it directly on the image. To go on, create a few spots above the bright blob of the source image. We need at least a couple of them in consecutive frames.

We need to add spots to the **selection**. The selection in TrackMate is a very useful tool for inspection, particularly because it is shared amongst all the possible views of a session, including *e.g.* [TrackScheme](TrackScheme "wikilink"). When you click in a spot, the selection is made of this spot, and all views are centered on the target spot.
...
![TrackMate\_GUIManualTracking.png](/images/pages/TrackMate_GUIManualTracking.png "TrackMate_GUIManualTracking.png")"
...
![TrackMate\_v2.1.0\_TrackMateTools.png](/images/pages/TrackMate_v2.1.0_TrackMateTools.png "TrackMate_v2.1.0_TrackMateTools.png")"
