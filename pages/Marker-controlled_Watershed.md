...
breadcrumb: Marker-controlled Watershed
layout: page
author: test author
...
{% include Infobox content="name = Marker-controlled Watershed |
software = IJPB-plugins | author = {| |style="vertical-align:top"
|![Marker-controlled flooding on the gradient image of the blobs
...
Marker-controlled Watershed is an ImageJ/Fiji plugin to segment
grayscale images of any type (8, 16 and 32-bit) in 2D and 3D based on
the marker-controlled watershed algorithm (Meyer and Beucher, 1990).
This algorithm considers the input image as a topographic surface (where
higher pixel values mean higher altitude) and simulates its flooding
from specific seed points or **markers**. A common choice for the
markers are the local minima of the gradient of the image, but the
method works on any specific marker, either selected manually by the
user or determined automatically by another algorithm.
...
Marker-controlled Watershed needs at least two images to run:
  ...
  - The **Marker** image: an image of the same dimensions as the input
    containing the seed points or markers as connected regions of
    voxels, each of them with a different label. They correspond usually
    to the local minima of the input image, but they can be set
    arbitrarily.

And it can optionally admit a third image:

  - The **Mask** image: a binary image of the same dimensions as input
    and marker which can be used to restrict the areas of application of
    the algorithm. Set to "None" to run the method on the whole input
    image.

Rest of parameters:

  - **Binary markers**: select to specify that markers are binary and
    need to be labeled.
  - **Calculate dams**: select to enable the calculation of watershed
    lines.
...
Output:
  ...
  - Labeled image containing the catchment basins and (optionally)
...
The Marker-controlled Watershed plugin is part of the
...
these installation steps, you will be installing as well the rest of
...
3.  David Legland, Ignacio Arganda-Carreras, Philippe Andrey;
