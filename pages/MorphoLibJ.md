...
content='Plugins | MorphoLibJ | Morphological filters'%}. The dialog let
...
reconstructed from binary images (Lehmann *et al.*, 2012\[4\]).
...
circularity values above 1 (Lehmann *et al.*, 2012\[5\]). The MorphoLibJ
library also considers the inverse of the circularity, referred to as
"elongation index". The values of elongation range from 1 for round
particles and increase for elongated particles.
...
DRIVE database (Staal *et al.*, 2004\[6\]). Left: original image with
...
where the users can perform the following set of editing tasks:
